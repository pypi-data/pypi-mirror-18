...
class CodeObjectTemplate(object):
    ...
    def __call__(self, scalar_code, vector_code, **kwds):
        ...
        **kwds
        ...
        '''
        if scalar_code is not None and len(scalar_code)==1 and list(scalar_code.keys())[0] is None:
            scalar_code = scalar_code[None]
        if vector_code is not None and len(vector_code)==1 and list(vector_code.keys())[0] is None:
        ...
        if len([k for k in list(module.__dict__.keys()) if not k.startswith('_')]):
...
class MultiTemplate(Mapping):
    ...
    def __init__(self, module):
        self._templates = {}
        for k, f in list(module.__dict__.items()):
