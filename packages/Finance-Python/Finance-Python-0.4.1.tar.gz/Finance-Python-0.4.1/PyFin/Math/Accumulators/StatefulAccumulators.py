...
class MovingTsMaxPos(SortedValueHolder):
    def __init__(self, window, dependency='x'):
        super(MovingTsMaxPos, self).__init__(window, dependency)
        self._runningTsMaxPos = np.nan

    def push(self, data):
        super(MovingTsMaxPos,self).push(data)
...
class MovingTsMinPos(SortedValueHolder):
    def __init__(self, window, dependency='x'):
        super(MovingTsMinPos,self).__init__(window, dependency)
        self._runningTsMinPos = np.nan

    def push(self, data):
        value = super(MovingTsMinPos,self).push(data)
...
class MovingRSV(SingleValuedValueHolder):
    def __init__(self, window, dependency='x'):
        ...
        self._runningMax = MovingMax(window, dependency)
        self._runningMin = MovingMinimum(window, dependency)

    def push(self, data):
        self._runningMax.push(data)
        self._runningMin.push(data)
        value = super(MovingRSV, self).push(data)
        if np.isnan(value):
            return np.nan
        else:
            self._runningRSV = (Identity(value) - self._runningMin) / (self._runningMax - self._runningMin)

    def result(self):
        return self._runningRSV.result()
