...
class LoginManager(object):
    ...
    def __init__(self, app=None, role='default', expires=3600, salt=''):
        ...
        self.__role = role
        self.__expires = expires + int(time.time())
        self.__salt = salt
    ...
    def __set_cookie(self, resp):
        ...
        if g.option is not None:
            ...
            if g.option == LoginManager.__SET:
                ...
                if user is not None:
                    info = [str(LoginManager.get_id(user)), str(self.__expires), LoginManager.__hash_generators.get(self.__role)(user)]
                    value = '-'.join(info)
                    
                    resp.set_cookie(key=self.__role, value=value, expires=self.__expires, httponly=True)
    ...
    # login_required 装饰器校验失败后调用的方法，可以分别制定以处理请求来自 WEB 和 API 的情况
    ...
    def login_required(self, permissions=0):
        def decorator(func):
            ...
            def wrapper(*args, **kw):
                ...
                if user is not None and (user.permissions & permissions) == permissions:
                    return func(*args, **kw)
