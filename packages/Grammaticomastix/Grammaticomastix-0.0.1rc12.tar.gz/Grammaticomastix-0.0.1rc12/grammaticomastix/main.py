...
def init_log():
    LOGSTR = {'disable_existing_loggers': False,
              'version': 1,
              'formatters': {'simple':
              ...
              'handlers': {'console': {'formatter': 'simple',
                           ...
                           'lastlogfile': {'level': 'DEBUG',
                                           'mode' : 'w',
                                           'formatter': 'simple',
                                           'encoding': 'utf8',
                                           'filename': 'grammaticomastix.lastlog',
                                           'class': 'logging.FileHandler',
                                          },
                           'errors.lastlogfile': {'level': 'ERROR',
                                                  'mode' : 'w',
                                                  'formatter': 'simple',
                                                  'encoding': 'utf8',
                                                  'filename': 'grammaticomastix.errors.lastlog',
                                                  'class': 'logging.FileHandler',
                                                 }},
              'loggers': {'my_module': {'level': 'DEBUG',
                                        'propagate': 'no',
                                        'handlers': ['console']}},
              'root': {'level': 'DEBUG',
                       'handlers': ['console',
    ...
    logging.config.dictConfig(LOGSTR)
...
def entrypoint():
    ...
    if globalvars.CMDLINEPARSER.interface=="qt5":
        ...
        main_linuxcli(src_string=globalvars.CMDLINEPARSER.sourcestring)        
...
def entrypoint_qt5():
    # si grammaticomastix-qt5 a été entré au clavier.
    init_log()
    LOGGER = logging.getLogger(__name__)
    LOGGER.info("{0} v. {1} : qt5 interface".format(PROJECT_NAME, VERSION))
    main_qt5()
