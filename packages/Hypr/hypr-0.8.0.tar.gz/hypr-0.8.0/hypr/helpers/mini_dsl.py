...
x=*y        x ends with y
x=*y*       x contains y
x=y*z       x starts with y and ends with z
            or any combination of the previous four predicates
...
def normalize_query(query):
    ...
    if not isinstance(query, tuple):
        return ((True, query, None),)           # simple value
    ...
    if isinstance(query[0], bool):              # tuple start with a bool
        if 1 < query_length < 4 and isinstance(query[1], bool):
            ...
            return ((query + (None,))[:3],)    # is a [partial] complexe value

    rv = []
    for value in query:
        if not isinstance(value, tuple):
            rv.append((True, value, None))
        else:
            rv.append((value + (None,))[:3])
