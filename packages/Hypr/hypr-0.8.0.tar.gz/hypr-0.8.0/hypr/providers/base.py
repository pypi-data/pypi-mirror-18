    ...
    * Decorative method :
    ...
    def add_propagation(cls, provider, *urls, methods=None, endpoint=None):
        ...
        if not isinstance(provider, _ProviderType):
            target = cls._app.router.get_provider(provider)
            if target is None:
                err_msg = '%s -> %s : unknown provider.'
                raise RuntimeError(err_msg % (cls.__name__, provider))
            if not urls:
                urls = tuple(cls._app.router.iter_rules(provider))
            provider = target

        provider._app = cls._app    # make the provider reachable
        if endpoint is None:
        ...
        # TODO: take benefit of this debug line
        # print(cls.__name__, ' -> ', provider.__name__, ' : ', methods)

        # get URLs from registered providers if none provided.
        if not urls and cls._app is not None:
            urls = tuple(cls._app.router.iter_rules(provider.__name__))
        if not urls:
