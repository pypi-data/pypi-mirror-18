...
HELP_COMMANDS = [{'command': 'config',
                 ...
                 {'command': 'build',
                  ...
                  'long_desc': None},
                 {'command': 'update',
                  'options': 'li update [--yes]',
                  'description': u'Roda em cada repositório clonado pelo comando `li config` o comando "git remote update && git fetch && git pull --all", desde que o repositorio esteja na branch prodution, staging, release ou master (repositórios que estejam em outros branchs são ignorados)\nEm adição ele clona novos repositórios que não estejam na pasta do projeto informado.\nUse a opção --yes ou -y para confirmar automaticamente as operações.',
                  'examples': None,
                  'long_desc': None},
                 {'command': 'list',
                  'options': 'li list [<libs>...]',
                  'description': u'Lista as aplicações existentes no Docker-Compose, indicando quais containers estão rodando e em qual branch eles estão.\nAlém disso mostra que versão do LI-Repo está instalado dentro do container.\nPara listar outras livrarias, digite elas após o comando, separando com espaço.\nPor questões de perfomance, a lista só mostra as versões de livrarias dos containers que estão rodando.',
