...
__date__    = '2016-December-4'
__url__     = 'https://engineering.purdue.edu/kak/distNonlinearLeastSquares/NonlinearLeastSquares-1.0.html'
...
USAGE:
    ...
    has two duties: (1) to synthetically generate noisy measurement data;
    and (2) to call on NonlinearLeastSquares to fit an optimized model to
    ...
    NonlinearLeastSquares instance includes:
        ...
        1) The observed data vector.  This vector is denoted X in the
           NonlinearLeastSquares class.  The same notation is used
           for the observed data vector in the usage-example class
           OptimizeSurfaceFit.
...
INTRODUCTION:
    ...
    vector norm of the different between X and Fvec.
    ...
    progress, GD will eventually get you to the point on the hyperplane
...
METHODS:
    ...
    (7)  set_jacobian_functionals_array():
         ...
         the number of measurements in the X vector and p is the number of
         parameters in the model.  If you are using nonlinear least-squares
         to fit optimal surfaces to noisy measurements over the xy-plane,
         each row of the X vector would correspond to one such measurement
         at some (x,y) coordinates. And the corresponding element in the
         argument jacobian_functionals_array (which should be a numpy
         chararray) would correspond to the partial derivative of the model
         functional that already has incorporated the (x,y) coordinates
         corresponding to that row and that is a partial derivative of the
         model with respect to the parameter corresponding to the column.

    (8)  set_num_measurements():

         This method sets the number of measurements in the X vector in the
         instance of the NonlinearLeastSquares on which the method is
         invoked.

    (9)  set_num_parameters():

         This method sets the number of model parameters that will be used
         in the nonlinear least-squares calculations.

    (10) set_X():

         This method sets the measured data vector, in the form of a numpy
         matrix consisting of only column, in an instance of
         NonlinearLeastSquares.
...
AN EXAMPLE OF A DOMAIN SPECIFIC CLASS THAT USES NonlinearLeastSquares:
    ...
    The constructor options for the OptimizeSurfaceFit class:
        ...
        data_array_size:                      
           ...
           If you set this constructor option to (16,16), the unit square
           will be sampled with a 16x16 grid in the unit square.

        datagen_functional:           

            When gen_data_synthetically is set to True, you must supply an
            algebraic expression in the form of a string that
            OptimizeSurfaceFit can use to generate the height data.  Just
            to show what such a string might look like, here is an
            example:
        ...
        debug:
            ...
            This flag is passed on to the instance of
            NonlinearLeastSquares.  When set to True, it cause that class
            to display a lot of useful information during each iteration of
            the nonlinear least-squares algorithm invoked.
    ...
    Here are the methods defined for OptimizeSurfaceFit:
        ...
        (3) display_function()
            ...
            that is provided, you can see how well the intermediate results
            for the model parameters create a surface fit the data points
            in the 3D space defined by the xyz-coordinates.
        ...
        (5) get_initial_params_from_file():
            ...
            If want to use model functions that have a large number of
            parameters, it might be easier to place their values in a text
            file and have OptimizeSurfaceFit get it from the file 
...
class NonlinearLeastSquares(object):
    ...
    def leven_marq(self):
        ...
        for iteration_index in range(self.max_iterations):
            if self.debug:
                print("\n\n============================================== LM ITERATION: %d" % iteration_index)
                print()
            ...
            if self.debug:
                ...
                print(str(g))
            if numpy.max(g) < 0.0000001: break
            ...
            if new_error_norm >= error_norm_with_iteration[-1]:
                ...
                if alambda > 1e5:
                    if self.debug:
                        print("\nIterations terminated at %d because alambda exceeded limit" % iteration_index)
                    break
        ...
        if self.display_function:
            self.display_function(new_fit_to_measurements, new_error_norm, iteration_index)
        result = {"error_norms_with_iterations" : error_norm_with_iteration[1:],
