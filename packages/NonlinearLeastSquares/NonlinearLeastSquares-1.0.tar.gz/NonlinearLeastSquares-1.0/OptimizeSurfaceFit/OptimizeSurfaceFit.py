...
##  OptimizeSurfaceFit is an example of a domain-specific class that you
##  would need to create in order to use the functionality of
##  NonlinearLeastSquares in your own code.  
##
##  The goal of OptimizeSurfaceFit is to fit a model surface to noisy
##  height data over the xy-plane in the xyz-coordinate frame, with the
##  model surface being described by a polynomial function.  Here are some
...
class OptimizeSurfaceFit(object):

    def __init__(self, *args, **kwargs):
        'constructor'                       
        if args:
            raise Exception('''The OptimizeSurfaceFit constructor can only be called with '''
        ...
        allowed_keys = 'initial_param_values_file','initial_param_values','model_functional_file','measured_data_file','model_functional','data_array_size','gen_data_synthetically','partials_for_jacobian','datagen_functional','how_much_noise_for_synthetic_data','optimizer','display_needed','debug'
        keywords_used = kwargs.keys()
        for keyword in keywords_used:
            if keyword not in allowed_keys:
                raise Exception("Wrong key used in constructor call --- perhaps spelling error")
        initial_param_values=initial_param_values_file=model_functional=model_functional_file=measured_data_file=gen_data_synthetically=data_array_size=partials_for_jacobian=datagen_functional=how_much_noise_for_synthetic_data=optimizer=display_needed=debug=None
    ...
    def set_constructor_options_for_optimizer(self, algo):
        '''
        This method conveys the following information from an instance of OptimizeSurfaceFit to an 
        instance of NonlinearLeastSquares:
            ...
            3)  The functionals for the partial derivatives to be used in the Jacobian matrix, 
                ...
                OptimizeSurfaceFit that was created.
...
#            algo.set_partials_for_jacobian(self.construct_jacobian_array_in_functional_form())
...
#        best_param_values = None
        if algo == 'lm':
#            best_param_values = self.optimizer.leven_marq()
            result_dict = self.optimizer.leven_marq()
        elif algo == 'gd':
#            best_param_values = self.optimizer.grad_descent()
...
#        return best_param_values
    ...
    def gen_data(self, datagen_functional):
        ...
        datagen_functional = str.replace(datagen_functional, 'y', 'i * ' + str(delta_y))
    ...
    def display_function(self, new_fit_to_measurements, new_error_norm, iteration_index):
        ...
        zz = [[new_fit_to_measurements_as_arr[i,j] for j in range(self.data_array_size[0])] for i in range(self.data_array_size[1])]
        ...
        fig.suptitle("iteration: " + str(iteration_index) + "     error: " + new_error_norm_str, fontsize=14, fontweight='bold')
        ...
        manager.window.wm_geometry("+500+500")
        plt.show()
#------------------------------     end of class def for OptimizeSurfaceFit    -----------------------------------
...
#                        datagen_functional = "7.8*(x - 0.5)**2 + 1.2*(y - 0.5)**2",
                        datagen_functional = "7.8*(x - 0.5)**3 + 2.2*(y - 0.5)**2",
#                        datagen_functional = "7.8*(x - 0.5)**4 + 2.2*(y - 0.5)**2",
                        data_array_size = (16,16), 
                        how_much_noise_for_synthetic_data = 0.5,  #  a fraction of the largest coeff in datagen_functional
#                        how_much_noise_for_synthetic_data = 1.0,  # a fraction of the largest coeff in datagen_functional
#                        model_functional = "a*(x-b)**2 + c*(y-d)**2",
                        model_functional = "a*(x-b)**3 + c*(y-d)**2",
#                        model_functional = "a*(x-b)**4 + c*(y-d)**2",

                        initial_param_values = {'a':2.0, 'b':0.4, 'c':0.8, 'd':0.4},
#                        initial_param_values = {'a':0.0, 'b':0.0, 'c':0.0, 'd':0.0},

#                        partials_for_jacobian = {'a':'(x-b)**2', 'b':'-2*a*(x-b)', 'c':'(y-d)**2', 'd':'-2*c*(y-d)'},
                        partials_for_jacobian = {'a':'(x-b)**3', 'b':'-3*a*(x-b)**2', 'c':'(y-d)**2', 'd':'-2*c*(y-d)'},

#                        partials_for_jacobian = {'a':'(x-b)**4', 'b':'-4*a*(x-b)**3', 'c':'(y-d)**2', 'd':'-2*c*(y-d)'},
                    )

    '''
    The next statement conveys the following information from the surface_fitter object constructed above
    to the optimizer object:

        1)  The measurement vector X.

        2)  The initial values to be used for the parameters of the model function.

        3)  The functionals for the partial derivatives to be used in the Jacobian matrix, assuming 
            that the option 'partials_for_jacobian' was set in the instance of OptimizeSurfaceFit that
            was created.

        4)  The F vector, which is a vector of the predicted values, all in functional form, for each
            of the data points in the measurement vector X.

        5)  The display function to be used for plotting the partial and the final results if such
            results can be displayed in the form of a 2D or 3D graphic with Python's matplotlib 
            library.
    ...
    surface_fitter.calculate_best_fitting_surface('lm')
#    surface_fitter.calculate_best_fitting_surface('gd')
