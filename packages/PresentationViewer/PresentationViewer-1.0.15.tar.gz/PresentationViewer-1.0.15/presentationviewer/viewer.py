...
import uno
from com.sun.star.uno import RuntimeException as uno_RuntimeError
...
def load( url, port=2002 ):
    ...
    # loadComponentFromURL isn't loading the PPT file...
    desk = desktop(port)
    impress = desk.loadComponentFromURL(url, "_blank", 0, properties(Hidden=False))
    if not impress:
        raise RuntimeError( "LibreOffice was unable to load the presentation" )
    return impress
...
def view_one_file( filename, default_duration=20, loop=False, display=-1, port=2002 ):
    """View a single file on the (already existing) captive display"""
    impress = None
    try:
        ...
        if impress:
            ...
            except uno_RuntimeError as err:
                # already closed...
                log.info( "Unable to set document unmodified, likely already closed" )
            else:
                log.info( 'Closing impress file: %s', filename )
                try:
                    impress.close( )
                except uno_RuntimeError as err:
                    log.info( "Unable to close impress, likely already closed" )

def view( filename, default_duration=20, loop=False, display=-1, port=2002 ):
    """Main viewing operation (view a single file on a captive display and exit)
    ...
    try:
        try:
            view_one_file( filename, default_duration=default_duration, loop=loop, display=display, port=port )
        except DisposedException as err:
            log.info( 'Binary connection lost, likely user close' )
    finally:
        log.info( 'Closing captive client' )
        # Request that the application close...
        try:
            desktop(port).terminate()
        except uno_RuntimeError as err:
            # desktop was already closed because e.g. the user hit <esc>
            pass 
        ...
        try:
            ...
            log.warn( "Unable to kill the captive Impress process: %s", err )
...
def main():
    ...
    if options.port is None:
        options.port = 2002 + options.display
