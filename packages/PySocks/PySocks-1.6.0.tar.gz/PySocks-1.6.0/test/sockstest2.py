...
def socket_SOCKS5_test():
    s = socks.socksocket()
    s.set_proxy(socks.SOCKS5, "127.0.0.1", 9151)
    s.connect(("httpbin.org", 80))
    s.sendall(raw_HTTP_request())
    status = s.recv(2048).splitlines()[0]
    assert status.startswith(b"HTTP/1.1 200")
...
def urllib2_HTTP_test():
    ...
    status = urllib2.urlopen("http://httpbin.org/ip").getcode()
    assert status == 200
...
def urllib2_handler_HTTP_test():
    ...
    status = opener.open("http://httpbin.org/ip").getcode()
    assert status == 200
...
def main():
    ...
    #socket_HTTP_test()
    print("1/13")
    socket_SOCKS4_test(); 1/0
    print("2/13")
    socket_SOCKS5_test(); 1/0
    print("3/13")
    if not PY3K:
