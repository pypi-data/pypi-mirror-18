...
def handle_sh_margin_details(start=''):
    ...
    margin_details = []
    if sh_margin_details_df is None or sh_margin_details_df.empty:
...
def handle_sz_margin_details(start=''):
    ...
    sz_margin_details = []
    if sz_margin_details_df is None or sz_margin_details_df.empty :
        return sz_margin_details
    rqdata.init()
    recent_day = date.today() + timedelta(days=-1)
    stock_list = list((sz_margin_details_df['stockCode'] + '.XSHE').unique())
    closing_px_df = rqdata.get_price(stock_list,
                                     start_date='2010-03-31',
                                     end_date=str(recent_day), adjusted=False)['ClosingPx']
...
def handle_sh_margins(start=None):
    ...
    sh_margins = []
    if sh_margins_df is None or sh_margins_df.empty :
        return sh_margins
...
def handle_sz_margins(start=None):
    ...
    sz_margins = []
    if sz_margins_df is None or sz_margins_df.empty :
        return sz_margins
