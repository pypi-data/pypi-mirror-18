...
from ..config import get_config_one
...
def update_margin_details(update_mode, config_path=None):
    database_url = get_config_one(config_path).get('tushare')
    flag = input('you are inserting in to ' +database_url['destination']
                 + database_url['database'] + '/' +database_url['margin_details'] + '.(y/n)')
    if flag != 'y':
        return
...
def update_margins(update_mode, config_path=None):
    database_url = get_config_one(config_path).get('tushare')
    flag = input(
        'you are inserting in to '+database_url['destination'] + database_url['database'] + '/' +
        database_url['margins'] + '.(y/n)')
    if flag != 'y':
        return
