...
def fetch_sh_margins(start=None, retry_count=3):
    """
    fetch summarized RQmargin data in ShangHai exchange from tushare
    :param start:
    :param retry_count:
    :return:
    """
...
def fetch_sh_margin_details(start='', retry_count=100):
    """
    fetch detailed  RQmargin data in ShangHai exchange from tushare
...
def fetch_sz_margins(start=None, retry_count=100):
    """
    fetch summarized RQmargin data in ShenZhen exchange from tushare, and tushare
    ...
    if int((today_date_str - start_date).days) > 364:
        ...
        sz_margins_df = ts.sz_margins(start=start,end=str(date.today()),
                                      retry_count=retry_count)
...
def fetch_sz_margin_details(start='', retry_count=100):
    """
     fetch detailed RQmargin data in ShenZhen exchange from tushare, and tushare
