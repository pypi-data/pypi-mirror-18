...
from ._launcher import Launcher as _la
...
class IsoThread:
    ...
    def __init__(self, target, name='Thread No.' + str(len(threads)), args: tuple = None,
                 kwargs: dict = None, required_module: tuple = None):
    ...
    def _prepare(self):
        ...
        return _la(self.src_name, name=self.name, args=self.args,
                   kwargs=self.kwargs, required=self.mds)
    ...
    def result(self):
        ...
        try:
            ...
            self.status = 'finished'
            del IsoThread.threads[IsoThread.threads.index(self)]
            return temp
