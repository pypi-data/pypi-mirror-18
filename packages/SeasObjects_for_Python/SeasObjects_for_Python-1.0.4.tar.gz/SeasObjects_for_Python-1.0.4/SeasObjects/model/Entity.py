...
class Entity(Obj):
	...
	def __init__(self, uri = None):
		...
		self.interfaces = []
		self.relatedUrls = {}
		self.setType(RESOURCE.SEAS_ENTITY)
	...
	def serialize(self, model):
		...
		# zone
		if self.hasZone():
			entity.addProperty(model.createProperty( PROPERTY.SEAS_ZONE ), self.zone.serialize(model))
		
		# address
		if self.hasAddress():
			entity.addProperty(model.createProperty( PROPERTY.VCARD_HASADDRESS ), self.address.serialize(model))
		
		# coordinate
		if self.hasCoordinates():
			entity.addProperty(model.createProperty( PROPERTY.SEAS_LOCATION ), self.coordinates.serialize(model))
		...
		for iAddrs in self.interfaces:
			entity.addProperty(model.createProperty( PROPERTY.SEAS_INTERFACE ), iAddrs.serialize(model))
		
		# set owners
		for owner in self.owners:
			entity.addProperty(model.createProperty( PROPERTY.SEAS_OWNER ), owner.serialize(model))
			
		# set creators
		for creator in self.creators:
			entity.addProperty(model.createProperty( PROPERTY.DC_CREATOR ), creator.serialize(model))
		...
		# websites
		if self.hasWebsite():
			websiteProp = model.createProperty( PROPERTY.VCARD_HASURL )
			for website in self.relatedUrls["siteurls"]:
				entity.addProperty( websiteProp, website );
			
		# logos
		if self.hasLogo():
			logoProp = model.createProperty( PROPERTY.VCARD_HASLOGO )
			for logo in self.relatedUrls["logos"]:
				entity.addProperty( logoProp, logo );
			
		# photos
		if self.hasPhoto():
			photoProp = model.createProperty( PROPERTY.VCARD_HASPHOTO )
			for photo in self.relatedUrls["photos"]:
				entity.addProperty( photoProp, photo );
			
		# sounds
		if self.hasSound():
			soundProp = model.createProperty( PROPERTY.VCARD_HASSOUND )
	...
	def parse(self, resource):
		...
		if isinstance(resource, Resource):
			...
			if predicate == PROPERTY.SEAS_LOCATION:
				...
				# coordinates
			if predicate == PROPERTY.VCARD_HASADDRESS:
				try:
					self.setLocation(Address().parse(statement.getResource()))
				except:
			...
			# creators
			if predicate == PROPERTY.DC_CREATOR:
				try:
			...
			# owner
			if predicate == PROPERTY.SEAS_OWNER:
				try:
			...
			# logo
			if predicate == PROPERTY.VCARD_HASLOGO:
				try:
			...
			# photo
			if predicate == PROPERTY.VCARD_HASPHOTO:
				try:
			...
			# sound
			if predicate == PROPERTY.VCARD_HASSOUND:
				try:
