...
import csv
from os import path
APP_PATH=path.normpath(path.join(path.dirname(path.dirname(path.abspath(__file__)))))
file_path =path.join(APP_PATH,'lib\data\\abook.csv')
...
if __name__ == '__main__':
    x = Way2sms()
    print x.login('8737855269','nitin')
    print x.send_sms(raw_input('Message'),raw_input('number'))
