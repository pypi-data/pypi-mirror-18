...
class Registry(object):
    ...
    def map_key(self, key):
        ...
        if type(key) is not str:
            ...
            if key in list(self.abbreviations.keys()):
    ...
    class RegistryKey(object):
        ...
        def open(self, key, subkey):
            ...
            try:
                self._hkey = wreg.OpenKey(hkey, subkey, 0, wreg.KEY_ALL_ACCESS)
        ...
        def create(self, key, subkey):
            ...
            if not hkey: raise RegistryError("could not find registry key for %s" % key)
            try:
                self._hkey = wreg.CreateKeyEx(hkey, subkey, 0, wreg.KEY_ALL_ACCESS)
                self._keyname = subkey
            except EnvironmentError as e:
                raise RegistryError(e)
        ...
        def openkey(self, index):
            ...
            if subkey:
                ...
                r.setkey(wreg.OpenKey(self._hkey, subkey, 0, wreg.KEY_ALL_ACCESS))
                r.setkeyname(subkey)
                return r
            else:
        ...
        def setvalue(self, name='', value='', var_type=''):
            """ Set the value of an item inside the current key,
            given its name"""

            try:
                if not var_type:
                    var_type = self.types[type(value)]
            except KeyError as e:
                raise TypeError(value)

            try:
                if name:
                    wreg.SetValueEx(self._hkey, name, 0, var_type, value)
                else:
                    wreg.SetValue(self._hkey, '', var_type, value)
            except (WindowsError, EnvironmentError) as e:
                raise RegistryError(e)
    ...
    while True:
        try:
            ...
            if keyname == '2.4':
                ...
                print()
                keyVersion, keyVersion.getkeyname()
                keyPath = keyVersion.openkey(2)
                print()
                keyPath, keyPath.getkeyname()
                print()
                'Install path is %s' % keyPath.getvalue()
                keyPath.close()
                keyVersion.close()
                break
        except RegistryError:
