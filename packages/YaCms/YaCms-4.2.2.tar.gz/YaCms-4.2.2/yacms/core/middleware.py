...
from yacms.core.models import SitePermission
from yacms.core.management.commands.createdb import (DEFAULT_USERNAME,
                                                         DEFAULT_PASSWORD)
from yacms.utils.cache import (cache_key_prefix, nevercache_token,
                                   cache_get, cache_set, cache_installed)
from yacms.utils.device import templates_for_device
from yacms.utils.deprecation import MiddlewareMixin, get_middleware_setting
from yacms.utils.sites import current_site_id, templates_for_host
from yacms.utils.urls import next_url
...
class AdminLoginInterfaceSelectorMiddleware(MiddlewareMixin):
    ...
    def process_view(self, request, view_func, view_args, view_kwargs):
        login_type = request.POST.get("yacms_login_interface")
...
class UpdateCacheMiddleware(MiddlewareMixin):
    """
    Response phase for YaCms's cache middleware. Handles caching
...
class FetchFromCacheMiddleware(MiddlewareMixin):
    """
    Request phase for YaCms cache middleware. Return a response
    ...
    def process_request(self, request):
        ...
        if response and request.method == "POST":
            if resolve(request.get_full_path()).url_name == "fb_do_upload":
                ...
                # when /admin is restricted to SSL using YaCms's SSL
...
class RedirectFallbackMiddleware(MiddlewareMixin):
    ...
    YaCms's approach for determining the current site.
    """
