...
from yacms.core.middleware import FetchFromCacheMiddleware
from yacms.core.templatetags.yacms_tags import initialize_nevercache
from yacms.utils.cache import cache_installed
from yacms.utils.sites import current_site_id, override_current_site_id
...
from yacms.core.admin import BaseDynamicInlineAdmin
from yacms.core.fields import RichTextField, MultiChoiceField
from yacms.core.managers import DisplayableManager
from yacms.core.models import (CONTENT_STATUS_DRAFT,
                                   CONTENT_STATUS_PUBLISHED)
from yacms.forms.admin import FieldAdmin
from yacms.forms.models import Form
from yacms.pages.models import Page, RichTextPage
from yacms.utils.importing import import_dotted_path
from yacms.utils.tests import (TestCase, run_pyflakes_for_package,
                                             run_pep8_for_package)
from yacms.utils.html import TagCloser
...
class CoreTests(TestCase):
    ...
    @skipUnless("yacms.mobile" in settings.INSTALLED_APPS and
                "yacms.pages" in settings.INSTALLED_APPS,
    ...
    def test_syntax(self):
        ...
        """
        warnings = []
        warnings.extend(run_pyflakes_for_package("yacms"))
        warnings.extend(run_pep8_for_package("yacms"))
        if warnings:
            self.fail("Syntax warnings!\n\n%s" % "\n".join(warnings))

    def test_utils(self):
        """
        Miscellanous tests for the ``yacms.utils`` package.
        """
        self.assertRaises(ImportError, import_dotted_path, "yacms")
        self.assertRaises(ImportError, import_dotted_path, "yacms.NO")
        self.assertRaises(ImportError, import_dotted_path, "yacms.core.NO")
        try:
            import_dotted_path("yacms.core")
        except ImportError:
            self.fail("yacms.utils.imports.import_dotted_path"
                      "could not import \"yacms.core\"")

    @skipUnless("yacms.pages" in settings.INSTALLED_APPS,
                "pages app required")
    ...
    def test_cache_installed(self):
        ...
        test_contexts = [
            (False,
             ['yacms.core.middleware.FetchFromCacheMiddleware']),
            (True,
             ['yacms.core.middleware.UpdateCacheMiddleware',
              'yacms.core.tests.SubclassMiddleware']),
            (True,
             ['yacms.core.middleware.UpdateCacheMiddleware',
              'yacms.core.middleware.FetchFromCacheMiddleware']),
        ]
