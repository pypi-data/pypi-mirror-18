...
from yacms.generic.managers import CommentManager, KeywordManager
from yacms.core.models import Slugged, Orderable
from yacms.conf import settings
from yacms.utils.models import get_user_model_name
...
class ThreadedComment(Comment):
    ...
    so we can't inherit from ``SiteRelated`` in ``yacms.core``, and
    therefore need to set the site on ``save``. ``CommentManager``
    inherits from yacms's ``CurrentSiteManager``, so everything else
    ...
    def avatar_link(self):
        from yacms.core.templatetags.yacms_tags import gravatar_url
