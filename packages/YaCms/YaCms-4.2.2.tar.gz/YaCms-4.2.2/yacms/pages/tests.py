...
from yacms.pages.admin import PageAdminForm
from yacms.urls import PAGES_SLUG
from yacms.utils.sites import override_current_site_id
from yacms.utils.tests import TestCase
...
class PagesTests(TestCase):
    ...
    def setUp(self):
        ...
        from yacms.core.request import _thread_local
    ...
    def test_login_required(self):
        ...
        accounts_installed = ("yacms.accounts" in settings.INSTALLED_APPS)
        ...
        if accounts_installed:
            # For an inaccessible page with yacms.accounts we should
        ...
        if accounts_installed:
            with override_settings(LOGIN_URL="yacms.accounts.views.login"):
                response = self.client.get(public_url, follow=True)
                self.assertEqual(response.status_code, 200)
                response = self.client.get(private_url, follow=True)
                self.assertEqual(response.status_code, 200)
    ...
    def test_exact_page_processor_for(self):
        ...
        from yacms.pages.page_processors import processor_for
        from yacms.pages.views import page as page_view
    ...
    def test_page_slug_has_correct_lang(self):
        ...
        from yacms.utils.urls import slugify
