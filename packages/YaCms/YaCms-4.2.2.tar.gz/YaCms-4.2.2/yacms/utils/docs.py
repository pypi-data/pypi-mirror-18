...
from yacms import __version__
from yacms.conf import registry
from yacms.utils.importing import import_dotted_path, path_for_import
...
def build_settings_docs(docs_path, prefix=None):
    ...
    ``yacms.conf.registry`` into RST format for use in docs,
...
def build_deploy_docs(docs_path):
    try:
        ...
        return
    project_template_path = path_for_import("yacms.project_template")
...
def build_changelog(docs_path, package_name="yacms"):
    """
...
def build_modelgraph(docs_path, package_name="yacms"):
    """
    Creates a diagram of all the models for yacms and the given
