...
class OIDCAuthMiddleware(object):
    ...
    def authenticate(self, environ, start_response):
        ...
        if self.reauthentication_necessary(session):
            ...
            self.logger.info('Session state set: {}'.format(session['state']))
    ...
    def callback(self, environ, start_response):
        ...
        self.logger.info('Query string: {}'.format(environ['QUERY_STRING']))

        try:
            code, state = self.parse_auth_response(environ)
            self.logger.info('code: {}, state: {}'.format(code, state))
    ...
    def parse_auth_response(self, environ):
        ...
        self.logger.info('state from auth_response: {}'.format(state))

        self.logger.info('state in session: {}'.format(session.get('state')))
