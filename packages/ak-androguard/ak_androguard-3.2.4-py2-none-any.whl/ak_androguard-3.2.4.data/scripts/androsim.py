...
option_4 = {'name': ('-d', '--display'),
            'help': 'display all information about methods', 'action': 'count'}
option_5 = {'name': (
    '-n', '--new'), 'help': 'calculate the final score only by using the ratio of included methods', 'action': 'count'}
option_6 = {'name': ('-e', '--exclude'),
            'help': 'exclude specific class name (python regexp)', 'nargs': 1}
option_7 = {'name': (
    '-s', '--size'), 'help': 'exclude specific method below the specific size (specify the minimum size of a method to be used (it is the length (bytes) of the dalvik method)', 'nargs': 1}
option_8 = {'name': ('-x', '--xstrings'),
            'help': 'display similarities of strings', 'action': 'count'}
option_9 = {'name': ('-v', '--version'),
            'help': 'version of the API', 'action': 'count'}
option_10 = {'name': ('-l', '--library'),
             'help': 'use python library (python) or specify the path of the shared library)', 'nargs': 1}

options = [option_0, option_1, option_2, option_4, option_5,
...
def check_one_file(a, d1, dx1, FS, threshold, file_input, view_strings=False, new=True, library=True):
    ...
    el = elsim.Elsim(ProxyDalvik(d1, dx1), ProxyDalvik(
        d2, dx2), FS, threshold, options.compressor, libnative=library)
    el.show()
    ...
    if view_strings:
        ...
        # ProxyDalvikStringOne(d2, dx2), FILTERS_DALVIK_SIM_STRING, threshold,
        # options.compressor, libnative=library )
        els.show()
        print("\t--> strings: %f%% of similarities" %
              els.get_similarity_value(new))
    ...
    for root, dirs, files in os.walk(directory, followlinks=True):
        if files != []:
            for f in files:
                ...
                check_one_file(a, d1, dx1, FS, threshold,
                               real_filename, view_strings, new, library)
...
def main(options, arguments):
    if options.input != None:
        ...
        if os.path.isdir(options.input[1]) == False:
            check_one_file(a, d1, dx1, FS, threshold, options.input[
                           1], options.xstrings, new, library)
        else:
            check_one_directory(a, d1, dx1, FS, threshold, options.input[
                                1], options.xstrings, new, library)
