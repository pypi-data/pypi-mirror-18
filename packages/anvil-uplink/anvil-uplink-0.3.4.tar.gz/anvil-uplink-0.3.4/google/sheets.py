...
class Sheet(ApiItem):
    ...
    def __getattr__(self, name):
        if name == "worksheets":
            return list(self.list_worksheets())
        else:
            raise AttributeError("Could not find attribute: %s" % name)
...
def login():
    return google.auth.login(['http://spreadsheets.google.com/feeds/'])
...
description: |
  ...
  ```python
  ws = db.worksheets["Sheet 1"]
  ```

  Once you have an object representing a Google Sheet, you can access its individual worksheets through the `worksheets` attribute.

  ```python
  print db.worksheets["Sheet 1"].fields
  ...
  for r in db.worksheets["Sheet 1"].rows:
  ...
  You can change this behaviour by setting `google.sheets.add_missing_fields` to `False`, in which case an Exception will be raised if you try to set a non-existent field.
