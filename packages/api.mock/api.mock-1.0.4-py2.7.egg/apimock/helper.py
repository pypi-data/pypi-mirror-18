...
class PushHelper(object):
    ...
    # 空文件, 标志新推送的配置文件, APP 检测到会删除该文件
    CONFIG_FLAG_NAME = '.new'
    ...
    def __init__(self, config_path=None, verbose=False):
        ...
        self.CONFIG_FILE, self.CONFIG_MOCK_DIR, self.CONFIG_FLAG = \
            PushHelper._build_paths(self.CONFIG_PATH)
    ...
    def _build_paths(cls, target):
        ...
        :return: (config_file, config_mock_dir, config_flag) tuple
                    配置文件、 mock 数据文件夹和标志文件的路径
        """
        config_file = os.path.join(target, cls.CONFIG_FILE_NAME)
        config_mock_dir = os.path.join(target, cls.CONFIG_MOCK_DIR_NAME)
        config_flag = os.path.join(target, cls.CONFIG_FLAG_NAME)
        return config_file, config_mock_dir, config_flag
    ...
    def init(self):
        ...
        if os.path.exists(self.CONFIG_FLAG_NAME):
            warning('Found file: %s' % self.CONFIG_FLAG_NAME)
        else:
            self._create_empty_file(self.CONFIG_FILE)
    ...
    def _check_config(self):
        ...
        if not self._check_file_exist(self.CONFIG_FLAG, verbose=False):
            self._create_empty_file(self.CONFIG_FLAG)

        return remote

    @staticmethod
    def _check_json_file(path, verbose=True):
        """
        检查 JSON 文件的格式
        :param path: 文件路径
    ...
    @staticmethod
    def _check_file_exist(path, verbose=True):
        """
        检查文件是否存在
        :param path: 文件路径
    ...
    @staticmethod
    def _check_regex(regex, verbose=True):
        """
        检查正则表达式的格式
        :param regex: 正则表达式
    ...
    def _push(self, device, remote):
        ...
        config_file, mock_dir, flag = self._build_paths(remote)

        device.push(self.CONFIG_FILE, config_file)
        device.push(self.CONFIG_MOCK_DIR, mock_dir)
        device.push(self.CONFIG_FLAG, flag)
