...
from appdynamics.agent.core import conditions, expressions, naming
    ...
    def match_backend(self, backend_properties, match_rule):
        ...
        for key in backend_properties:
            ...
            if match_rule.HasField(field_name):
                if not conditions.match(str(backend_properties[key]), getattr(match_rule, field_name)):
                    return False
        return True
    ...
    def match(self, entry_type, request, base_name=None):
        ...
        for rule in entry_point_config['custom_definitions']:
            if conditions.match_http(request, rule['condition']):
                ...
                custom_bt_name = naming.get_http_bt_name(custom_naming_scheme, request,
