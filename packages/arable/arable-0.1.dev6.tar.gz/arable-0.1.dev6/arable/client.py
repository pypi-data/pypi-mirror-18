...
class ArableClient(object):
    ...
    param_options = {"devices", "deployments", "location", "start", "end",
                     "order", "limit", "format", "select", "measure", "meta"}
    ...
    def get_deployments_by_geo(self, lat=0, lng=0, distance=1000):
    ...
    def get_deployments_by_tag(self, tag):
        self.check_connection()
        url = ArableClient.base_url + "/deployments"
        params = {'tag': tag}
        r = requests.get(url, params=params, headers=self.header)
    ...
    def query(self, **kwargs):
        """Query Arable pod data.
            ...
            :param location : optional; name of location to retrieve data for;
            devices and deployments lists are ignored if this parameter is
            provided
