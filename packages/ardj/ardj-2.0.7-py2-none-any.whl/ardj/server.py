...
from ardj.database import connect, commit
...
class TrackController(DataController):
    ...
    def on_post(self, track_id):
        ...
        try:
            form = web.input(title="", artist="", labels="")
            ...
            return self.send_json({"message": "Changes saved.",
                                   "redirect": "/artist/" + utf(track["artist"])})
...
def get_application():
    app = web.application((
        ...
        r"/track/(\d+)/rocks", RocksController,
        r"/track/(\d+)/sucks", SucksController,
        r"/track/(\d+)/download", DownloadController,
        r"/skip", SkipController,
...
def cmd_serve(arg=None, *args):
    ...
    socket = config.get("web_socket", "127.0.0.1:8080")
