    ...
    def touch_by_track(cls, track_id):
        ...
        for playlist in cls.get_all():
            ...
            if name and playlist.match_track(track):
                log_debug('Track {} touches playlist "{}".', track_id, name.encode("utf-8"))
...
class Track(dict):
    ...
    def get_labels(self):
        ...
        if "labels" not in self:
            rows = db.fetch("SELECT label FROM labels WHERE track_id = ?", (self["id"], ))
    ...
    def get_by_id(cls, track_id):
        ...
        row = db.fetchone(sql, (int(track_id), ))
        if not row:
            return None
    ...
    def get_last(cls):
        ...
        row = connect().fetchone(sql)
        return Track([(cls.fields[k], v) for k, v in enumerate(row)])

    def get_last_vote(self, sender):
        votes = connect().fetchone("SELECT vote FROM votes WHERE track_id = ? AND email = ? ORDER BY track_id DESC", (self["id"], sender, ))
    ...
    def refresh_tags(self, filepath):
        ...
        if duration != self["length"]:
            log_info(u"Updating track {} duration to {}.", self["id"], duration)
            self["duration"] = duration
            write = True

        artist = tags.get("artist", "Unknown Artist")
        if artist != self["artist"]:
            log_info(u"Updating track {} artist to {}.", self["id"], artist)
            self["artist"] = artist
            write = True

        title = tags.get("title", "Untitled")
        if title != self["title"]:
            log_info(u"Updating track {} title to {}.", self["id"], title)
            self["title"] = title
            write = True

        if write:
            db = connect()
            db.execute("UPDATE tracks SET artist = ?, title = ?, length = ? WHERE id = ?", (artist, title, duration, self["id"]))
            db.commit()
    ...
    def write_tags(self):
        ...
        if new_tags:
            log_info("Writing new tags to {}.", utf(self["filename"]))
...
def get_last_track_id():
    ...
    """
    row = connect().fetchone("SELECT id FROM tracks ORDER BY last_played DESC LIMIT 1")
    if row:
        return row[0]
...
def get_urgent():
    ...
    data = connect().fetch('SELECT labels FROM urgent_playlists WHERE expires > ? ORDER BY expires', (int(time.time()), ))
    if data:
...
def add_vote(track_id, email, vote, update_karma=False):
    ...
    db = connect()
    row = db.fetchone("SELECT last_played, weight FROM tracks WHERE id = ?", (track_id, ))
    if row is None:
        return None
    ...
    vote_count = db.fetchone("SELECT COUNT(*) FROM votes WHERE track_id = ? "
    ...
    real_weight = db.fetchone('SELECT weight FROM tracks WHERE id = ?',
        (track_id, ))[0]
    db.commit()
...
def add_file(filename, add_labels=None, owner=None, quiet=False, artist=None, title=None, dlink=None):
    ...
    if not quiet:
        log_info('Adding from {}', utf(filename))
    ...
    rel_path = os.path.relpath(filename,
        config.get_path("musicdir"))

    db = connect()
    track_id = db.execute('INSERT INTO tracks (artist, title, filename, length, last_played, owner, weight, real_weight, count, download) VALUES (?, ?, ?, ?, ?, ?, 1, 1, 0, ?)', (artist, title, rel_path, duration, 0, owner or 'ardj', dlink, ))
    for label in labels:
        db.execute('INSERT INTO labels (track_id, label, email) VALUES (?, ?, ?)', (track_id, label, (owner or 'ardj').lower(), ))
    db.commit()
...
def get_track_id_from_queue():
    ...
    """
    db = connect()
    row = db.fetchone('SELECT id, track_id FROM queue ORDER BY id LIMIT 1')
    if row:
    ...
    with Sticky() as sticky:
        ...
        if sticky["playlist"] != pl_name:
            if sticky["label"] is not None:
                log_info("Playlist changed to {}, dropping sticky label \"{}\".", pl_name, sticky["label"])
                sticky["label"] = None
        ...
        if sticky["label"]:
            log_info("Using sticky label {}", utf(sticky["label"]))
            return

        # This playlist has no sticky labels, nothing to do.
        if not playlist.get("sticky_labels"):
            log_debug("Sticky: playlist {} has no sticky_labels.", utf(sticky["playlist"]))
            return

        # Find intersecting labels.
        track = Track.get_by_id(track_id)
        if track is None:
            log_debug("Sticky: track {} not found -- no labels to pick from.", track_id)
            return

        # No intersection, nothing to do.
        labels = list(set(track.get_labels()) & set(playlist["sticky_labels"]))
        if not labels:
            log_debug("Sticky: track {} has no labels to stick to playlist {}.", track["id"], utf(sticky["playlist"]))
            return

        # Store the new sticky label.
        sticky["label"] = random.choice(labels)
        log_info("New sticky label: {}", utf(sticky["label"]))
...
def get_sticky_label(playlist):
    ...
    with Sticky() as sticky:
        ...
        if playlist.get("name", "unnamed") != sticky["playlist"]:
            if sticky["label"]:
                log_debug("Sticky: playlist changed to {}, forgetting old label {}.", utf(playlist.get("name")), utf(sticky["label"]))
            return []

        if not sticky["label"]:
            return []

        log_debug("Sticky: forcing label {}", sticky["label"])
...
def get_random_row(rows, strategy=None):
    ...
    log_debug("Picked track {} using strategy '{}'.", track_id, utf(strategy))
    return track_id
...
def get_prerolls_for_track(track_id):
    ...
    db = connect()
    by_artist = db.fetch("SELECT t1.id FROM tracks t1 INNER JOIN tracks t2 ON t2.artist = t1.artist INNER JOIN labels l ON l.track_id = t1.id WHERE l.label = 'preroll' AND t2.id = ? AND t1.weight > 0 AND t1.filename IS NOT NULL", (track_id, ))
    by_label = db.fetch("SELECT t.id, t.title FROM tracks t WHERE t.weight > 0 AND t.filename IS NOT NULL AND t.id IN (SELECT track_id FROM labels WHERE label IN (SELECT l.label || '-preroll' FROM tracks t1 INNER JOIN labels l ON l.track_id = t1.id WHERE t1.id = ?))", (track_id, ))
...
def add_preroll(track_id, labels=None):
    ...
    log_debug("Looking for prerolls for track {} (labels={})", track_id, labels)

    db = connect()
    row = db.fetchone("SELECT COUNT(*) FROM labels WHERE track_id = ? AND label LIKE 'preroll-%'", (track_id, ))
    if row and row[0]:
        log_debug("Track {} is a preroll itself.", track_id)
        return track_id
    ...
    log_debug("Found {} prerolls.", len(prerolls))
    ...
    if prerolls:
        ...
        log_debug("Will play track {} (a preroll).", track_id)
...
def get_next_track_id(update_stats=True):
    ...
    if track_id:
        want_preroll = False
        if debug:
            log_debug('Picked track {} from the queue.', track_id)
    ...
    if not track_id:
        ...
        if labels:
            ...
            if debug and track_id:
                log_debug('Picked track {} from the urgent playlist.', track_id)

    if not track_id:
        for playlist in Playlist.get_active():
            if debug:
                log_debug('Looking for tracks in playlist "{}"', utf(playlist.get('name', 'unnamed')))
    ...
    if not track_id:
        log_debug("Falling back to just any random track from the database.")

        rows = db.fetch("SELECT id, weight, artist, count, last_played FROM tracks WHERE weight > 0")
    ...
    if track_id:
        ...
        if update_stats:
            count = db.fetch('SELECT count FROM tracks WHERE id = ?', (track_id, ))[0][0] or 0
            db.execute('UPDATE tracks SET count = ?, last_played = ? WHERE id = ?', (count + 1, int(time.time()), track_id, ))
...
def shift_track_weight(track_id):
    log_debug("Shifting weight for track {}", track_id)
    db = connect()
    weight, real_weight = db.fetchone("SELECT weight, real_weight FROM tracks WHERE id = ?", (track_id, ))
...
def find_by_artist(artist_name):
    db = connect()
    rows = db.fetch('SELECT id FROM tracks WHERE artist = ? COLLATE unicode', (artist_name, ))
    return [row[0] for row in rows]


def find_by_filename(pattern):
    db = connect()
    rows = db.fetch('SELECT id FROM tracks WHERE filename LIKE ? COLLATE unicode', (pattern, ))
    return [row[0] for row in rows]
...
def find_by_title(title, artist_name=None):
    ...
    if artist_name is None:
        rows = db.fetch('SELECT id FROM tracks WHERE title = ? COLLATE unicode', (title, ))
    else:
        rows = db.fetch('SELECT id FROM tracks WHERE title = ? COLLATE unicode AND artist = ? COLLATE unicode', (title, artist_name, ))
    return [row[0] for row in rows]
...
class MediaFolderScanner(object):
    ...
    def run(self):
        ...
        Label.delete_by_name(self.temp_label)

        count = 0
        for fn in fs:
            if fn not in db:
                try:
                    t = Track.from_file(fn)
                    log_info("New track: {}: \"{}\" by {}", t["id"], utf(t["title"]), utf(t["artist"]))
                    count += 1
                except Exception, e:
                    log_exception("{}", e)
    ...
    def find_tracks(self):
        result = {}
        for track in Track.find_all(deleted=True):
            if track["filename"]:
...
def count_available():
    ...
    count = connect().fetch("SELECT COUNT(*) FROM tracks WHERE weight > 0")
    return count[0][0]
...
def cmd_mark_hitlist(arg=None, *args):
    ...
    weight = db.fetch_cell('SELECT real_weight FROM tracks WHERE id IN (SELECT track_id FROM labels WHERE label = ?) ORDER BY real_weight DESC LIMIT 19, 1', (check_label, ))
    if weight:
