...
def on_ban(args, sender):
    ...
    db = connect()
    count = db.fetchone('SELECT COUNT(*) FROM tracks WHERE artist = ? AND weight > 0', (args, ))[0]
    if not count:
...
def on_queue(args, sender):
    ...
    if args == 'flush -a' and is_admin:
        ...
        tracks = tracks[:1]
        have_tracks = db.fetchone('SELECT COUNT(*) FROM queue WHERE owner = ?', (sender, ))[0]
...
def on_dump(args, sender):
    ...
    votes = db.fetchone('SELECT vote FROM votes WHERE track_id = ? AND email = ?', (track['id'], sender, ))
