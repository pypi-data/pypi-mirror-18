...
class Model(dict):
    ...
    def where(cls, cond, params=None):
        fields_sql = ", ".join(cls.fields)
        sql = "SELECT %s FROM %s WHERE %s" % (fields_sql, cls.table_name, cond)
        return cls._fetch_rows(sql, params or [])
...
class Track(Model):
    ...
    def get_by_path(cls, path):
        rows = cls.where("`filename` = ? ORDER BY `id` LIMIT 1", [path])
...
class Task(Model):
    ...
    def get_by_task(cls, task):
        rows = cls.where("`task` = ?", [task])
        return rows[0] if rows else None

    @classmethod
    def pick_one(cls):
        rows = cls.where("`run_after` < ? ORDER BY `run_after`, `id` LIMIT 1", [int(time.time())])
        return rows[0] if rows else None
