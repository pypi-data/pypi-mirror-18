...
class AnotherSAMPTT(BaseNestedSets):
    ...
    def parents(self, include_self=False):
        ...
        table = self.get_basetree()._get_tree_table(self.__mapper__)
        pk = getattr(table.c, self.get_pk_column().name)
        lidx = getattr(table.c, cls.get_basetree().FIELD_LEFT_IDX)
        ridx = getattr(table.c, cls.get_basetree().FIELD_RIGHT_IDX)

        return session.query(table).filter(lidx<=self.id).filter(ridx>=self.id)
