...
def wrapper_func(i):
	print "OK",MPI.COMM_WORLD.Get_rank(),i, abcsampler.rank, abcsampler.sim_comm.Get_rank()
	#out=sim_pool.map(abcsampler.test,[1.1,2.2])
	out=abcsampler.sim_pool.map(abcsampler.test,[1.1,2.2])
	#abcsampler.sim_pool.close
	#abcsampler.test(3.33)
	return i

def test2(i):
	print MPI.COMM_WORLD.Get_rank(),i
	return i
...
class ABC_class(object):
	...
	def __init__(self,nparam,npart,data,tlevels,niter,priors,**kwargs):
		...
		if self.rank in self.abc_ranks: #BOTH 0 and 1 run this
                                self.pool = MpiPool(comm=self.abc_comm)

		sim_ranks = [1,2,3]
		sim_group = self.group.Incl(sim_ranks)
		self.sim_comm = self.comm.Create(sim_group)
		if self.rank in sim_ranks: #1,2 and 3 run this
                                self.sim_pool = MpiPool(comm=self.sim_comm)
				#out=self.sim_pool.map(test2,[1.1,2.2])
			        #self.sim_pool.close()


		if self.rank in self.abc_ranks:
			self.pool.map(wrapper_func,[self.rank]*3)
			self.pool.close()
		else:
			self.sim_pool.worker()
		if self.rank==1:
			self.sim_pool.close()

	def test(self,i):
		print "YUP"
		print "INSIDE",MPI.COMM_WORLD.Get_rank(),i, self.rank
		return i
