...
class Accounts(Dynamics):
    ...
    def save(self,account):
        ...
        self.print_message( "Account " + account['name'] + " was saved in dynamodb.")

    def get_from_dynamics(self, param_modifiedon=''):
        """Get all the accounts with modifiedon greater than the latest
        ...
        self.query(params={'$filter': 'modifiedon gt ' + modifiedon })
    ...
    def to_delete_in_dynamo(self, persist= False):
        ...
        response=self.table.query( KeyConditionExpression= Key('bbug_company_id').eq( self.bbug_company_id),
                                  FilterExpression=Attr('bbug_updated_at').lt(limit_date.strftime('%Y-%m-%d') ))
        if persist:
            for account in response['Items']:
                self.table.delete_item(Key={'bbug_company_id': account['bbug_company_id'], 'accountid': account['accountid'] })
        return response['Items']
