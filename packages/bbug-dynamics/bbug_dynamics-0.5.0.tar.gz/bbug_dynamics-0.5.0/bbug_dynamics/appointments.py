import adal, httplib, urllib, json, boto3, datetime, pytz
...
class Appointments(Dynamics):
    ...
    def save_booking_in_dynamo(self,bbug_event):
        ...
        self.print_message("Bookingbug Event " + str(bbug_event['id']) + " was saved in dynamodb.")
    ...
    def to_delete_in_dynamo(self, persist= False):
        limit_date  = datetime.today() - relativedelta(days=self.days_to_delete_old_data)
        response=self.table.query( KeyConditionExpression= Key('bbug_company_id').eq( self.bbug_company_id),
                                  FilterExpression=Attr('dynamics_updated_at').lt(limit_date.strftime('%Y-%m-%d') ))
        if persist:
            for booking in response['Items']:
                self.table.delete_item(Key={'bbug_company_id': booking['bbug_company_id'], 'id': booking['id'] })
        return response['Items']
    ...
    def udate_appointment_in_dynamics(self,appointment,booking_appointment,booking):
        ...
        if self.response.status==204:
            ...
            self.print_message("Updated appointment " + booking_appointment['activityid'] + " for booking " + booking_appointment['id'])
        else:
            self.print_message("Error cannot update the appointment "+booking_appointment['activityid'] +" for booking: " + str(booking['id']))
            self.print_message("Response status: " + str(self.response.status))
            self.print_message("Response : " + self.response.read())
    def create_appointment_in_dynamics(self,appointment,booking):
        ...
        if self.response.status==204:
            ...
            self.save_booking_in_dynamo(booking)
            self.print_message("Created appointment for booking " + str(booking['id']))
        else:
            self.print_message("Error cannot create an appointment for booking: " + str(booking['id']))
            self.print_message("Response status: " + str(self.response.status))
            self.print_message("Response : " + self.response.read())
    ...
    def delete_appointment_in_dynamics(self, booking):
        ...
        if booking_appointment:
            ...
            if self.response.status==204:
                ...
                self.print_message("Deleted appointment " + activityid )
            else:
                self.print_message("Error, cannot delete the appointment " + booking_appointment['activityid'] )
        else:
            self.print_message("Error, the acitvityid not found")
