...
def test_accounts_query():
    ...
    accounts.query(params={ '$filter': 'modifiedon gt ' +
                        date_after_month.strftime('%Y-%m-%d') })
    assert len(accounts.process_response()) == 0

def test_find_account():
    accounts = init_accounts()
    accounts.query(params={'$top': '1'})
    data = accounts.process_response()
    assert len(data) == 1
    assert 'accountid' in data[0]
    accounts.find_dynamics_object(data[0]['accountid'])
    account = accounts.process_response()
    assert data[0]['accountid']== account['accountid']
    assert data[0]['versionnumber']==account['versionnumber']


def test_acounts_get_from_dynamics():
    # reset date in test account
    table_modifiedon=boto3.resource('dynamodb').Table('dynamics_accounts_greater_modifiedon')
    table_modifiedon.put_item(Item={ 'bbug_company_id': 'test', 'modifiedon': '2010-01-01'})

    table_modifiedon.put_item(Item={ 'bbug_company_id': 'localhost__37000', 'modifiedon': datetime.today().strftime('%Y-%m-%d')} )
