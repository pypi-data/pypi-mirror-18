...
class CatalogPackage(SubCommand):
    ...
    def populateParserArgs(self, subparser):
        ...
        subparser.add_argument('-o', '--os', action='store', dest='os',
                               default='centos', choices=['any', 'centos', 'rhel'],
                               help='Determins which type of EPIC installation '
                               'this catalog entry should be available for. '
                               'For example, rhel catalog entry will not show '
                               'on an EPIC installated on CentOS.')
    ...
    def run(self, pargs):
        ...
        if imageDict.has_key('import_url'):
            ...
            print("WARN: No image to include in the bundle.")
            image = 'undefined'
            imageMd5 = 'undefined'
        ...
        cmdList = [pkgScript, '--config', paramFile]
        if pargs.verbose:
            cmdList.append('--verbose')

        try:
            print("Packaging the catalog entry for %s." %(pargs.os))
