...
class MethodPane(beatle.app.ui.pane.BasePane, ui.MethodPane):
    """Implements method pane editor"""
    # command ids:
    _delete = IdentifiersHandler.register("ID_DELETE")
    _undo  = IdentifiersHandler.register("ID_UNDO")
    _redo  = IdentifiersHandler.register("ID_REDO")
    _copy  = IdentifiersHandler.register("ID_COPY")
    _cut  = IdentifiersHandler.register("ID_CUT")
    _paste  = IdentifiersHandler.register("ID_PASTE")
    _save  = IdentifiersHandler.register("ID_SAVE")
    
    _closePane  = IdentifiersHandler.register('close-pane')
    #_saveCode = IdentifiersHandler.new()
    #_undoCode = IdentifiersHandler.register('undo')
    #_redoCode = IdentifiersHandler.register('redo')
    #_copyCode = IdentifiersHandler.register('copy')
    #_cutCode = IdentifiersHandler.register('cut')
    #_pasteCode = IdentifiersHandler.register('paste')
    #_deleteCode = IdentifiersHandler.register('delete')
    _toggleFull = IdentifiersHandler.register('fullscreen')
    _navigator = IdentifiersHandler.new()
    ...
    def Refresh(self):
        ...
        if text == self._object._content:
            # This is a temporally patch for self updates
    ...
    def Popup(self, editor, text=None):
        ...
        if _class is not None:
            _vars.update({"this": {
                'type': _class,
                'static': self._object._static,
                'volatile': False,
                'const': self._object._const_method,
                'register': False,
                'ptr': True,
                'ref': False,
                'array': False
        ...
        if text:
            ...
            if type(_type) is model.cc.Class:
                ...
                dlg = NavigatorDialog(editor, p, type=_type)
            ...
            p = editor.PointFromPosition(pos)
            p = editor.ClientToScreen(p)
            dlg = NavigatorDialog(editor, p, vars=_vars)
            if dlg.ShowModal() == wx.ID_OK:
                editor.AddText(dlg.value+";\n")
                editor.AutoIndent(True)
    ...
    def OnFullScreen(self, event):
        ...
        if self._fullScreen:
            ...
            self._fullScreen = beatle.app.ui.pane.FullScreen(self)
        wx.SafeYield()
