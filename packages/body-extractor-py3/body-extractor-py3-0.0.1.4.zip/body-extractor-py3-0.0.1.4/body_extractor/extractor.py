...
class BodyExtractor():
    ...
    def _preprocess(self):
        ...
        regTitle = re.search(b'<title>[\s\S]*?</title>',self.html)
        if regTitle is not None:
            titleTag = regTitle.group()
            self.title = titleTag[7:len(titleTag)-8].decode('utf-8')
...
if __name__ == '__main__':
    ...
    #print(extractor.wordCount)
    #print(len(extractor.lines[extractor.maxIndex]))
