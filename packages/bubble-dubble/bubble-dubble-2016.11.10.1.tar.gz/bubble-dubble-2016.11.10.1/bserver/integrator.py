...
class Integrator:
    ...
    def adjustCorrections(self, interface, shape):
        ...
        if self.interface.detector == 'Frelon':
            ...
            self.distortion.init(self.interface.spline, shape)
        else:
            self.darkSample.init(None)
            self.darkBackground.init(None)
            self.flatfield.init(None)
            self.distortion.init(None, None)
    ...
    def __call__(self, filename, image, interface, catchException=True):
        self.adjustCorrections(interface, image.array.shape)
        if catchException:
