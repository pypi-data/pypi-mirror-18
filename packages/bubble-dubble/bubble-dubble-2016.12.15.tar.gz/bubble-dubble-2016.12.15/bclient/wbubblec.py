...
class WBubble(QtWidgets.QDialog, Ui_WBubble):
    ...
    def frelonMessage(self, det):
        if det == 'Frelon' and platform.system() == 'Linux':
            self.speedCheckBox.setChecked(True)
            # noinspection PyCallByClass,PyArgumentList,PyTypeChecker
            QtWidgets.QMessageBox.warning(self, 'Problem with frelon', 'There is a problem with the Frelon detector'
                                                                       ' on Windows and MacOS. To avoid it the speed '
                                                                       'mode on the Server tab has been switched on. '
                                                                       'It is strongly recommended to keep it on. The'
