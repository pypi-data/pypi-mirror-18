...
class HgRepo(Repo):
    ...
    def pushed(self, tries=0):
        try:
            return 'no changes found' in self.api_repo.hg_command('outgoing')
        except hgapi.hgapi.HgException as e:
            if 'no changes found' in str(e):
                return True

            if 'repository does not exist' in str(e):
                return False

            print('\nwhen hg {} push...'.format(self.dirname))
            print(e)
...
class GitRepo(Repo):
    ...
    def pushed(self):
        try:
            ...
            return 'Everything up-to-date' in err
        except gitapi.gitapi.GitException:
            pass
