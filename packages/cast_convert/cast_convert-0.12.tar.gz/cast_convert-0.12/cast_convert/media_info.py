...
from .exceptions import StreamNotFoundException, FFProbeException
from .chromecast_compat import CAST_COMPAT
from .preferences import CONVERT_TO_CODEC, DEFAULT_CONTAINER
...
NOT_VIDS = set('ansi mjpg png subrip mjpeg bintext tiff'.split())
...
def get_media_info(filename: str) -> dict:
    try:
        ...
        raise FFProbeException("Error accessing %s." % filename) from e

    if not json:
        raise FFProbeException("File %s cannot be read by ffprobe." % filename)

    return json


def mediainfo(filename: str) -> Dict:
    output = getoutput('mediainfo -f "{filename}"'.format(filename=filename))
    return dict((item.strip() for item in line.split('   : '))
                for line in output.split('\n')[1:] if line and ' : ' in line)


def is_mp4_streamable(filename: str) -> bool:
    return mediainfo(filename)['IsStreamable'] == 'Yes'
...
def update_transcoding_info(stream_type: str, media_info: Options, transcoding_info: Options):
    ...
    try:
        ...
        if not is_compatible(stream_type, codec):
            new_codec = CONVERT_TO_CODEC[stream_type]

            if stream_type == "container":
                new_codec = DEFAULT_CONTAINER if codec not in new_codec else False

            transcoding_info[stream_type] = new_codec
...
def is_video(path: str) -> bool:
    ...
    except (FFProbeException, StreamNotFoundException) as e:
        return False
...
def mime_is_video(path: str) -> bool:
