...
from .media_info import is_video, mime_is_video
...
def consume_video_queue(queue: Queue, threads: int = THREADS, debug: bool = False):
    while True:
        filename = queue.get()
        file_is_vid = mime_is_video(filename)

        if debug:
            debug_print(filename, 'is vid', file_is_vid)

        if file_is_vid:
            if debug:
                debug_print("Getting", filename, "size...")
...
class AddedFileHandler(PatternMatchingEventHandler):
    ...
    def add_to_queue(self, event, filename: str):
        if self.debug:
            debug_print('Event received:', event)
            debug_print(filename, 'seen', filename in self.seen)

        if filename not in self.seen:
            if self.debug:
                debug_print('Adding to queue:', filename)
    ...
    def on_moved(self, event):
        ...
        self.add_to_queue(event, filename)
