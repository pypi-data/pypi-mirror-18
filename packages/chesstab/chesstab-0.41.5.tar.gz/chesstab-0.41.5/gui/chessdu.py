...
from basesup.tools import dialogues, callthreadqueue
...
# sequential updates to indexes can be.  The factor 3 is appropriate when at
...
class ChessDeferredUpdate(ChessException):
    ...
    def estimate_games_in_import(self):
        ...
        if not positions_per_game:
            ppp = 'Not a number'
        else:
            ppp = str(pieces_per_game // positions_per_game)
                ...
                if not status:
                    if backup:
                        self.report.append_text(''.join(
                            ('Problems encountered doing import. Backups are ',
                             'retained for recovery.')))
                    else:
                        self.report.append_text(''.join(
                            ('Problems encountered doing import. No backups ',
                             'available.')))
                elif backup:
...
def write_error_to_log():
    ...
    try:
        f = open(os.path.join(sys.argv[1], ERROR_LOG), 'ab')
        f.write(
            ''.join(
                ('\n\n\n',
                 ' '.join(
