...
def strip_omitted_docs(env):
    ...
    for doc in found_docs:
        ...
        with open(env.doc2path(doc), 'r') as fh:
            line = ''
            for line in fh:
                if line.strip():
                    break
...
def remove_from_toc(app, doctree):
    ...
    """
    # FIXME: removing entries now seems to leave broken PREV/NEXT links before & after
    #        the omitted entries.  either need to patch that as well, or move this up
    #        into our TocTree.run() call, where we'd have to deal w/ nodes up there.
