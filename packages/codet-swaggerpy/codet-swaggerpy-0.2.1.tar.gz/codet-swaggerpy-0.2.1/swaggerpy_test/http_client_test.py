...
class SynchronousClientTestCase(unittest.TestCase):
    ...
    def test_real_post(self):
        ...
        self.assertEqual(b"foo=bar", httpretty.last_request().body)
    ...
    def test_basic_auth(self):
        ...
        resp = uut.request('GET', "http://swagger.py/client-test",  params={'foo': 'bar'})
        encode = base64.b64encode(b'unit:peekaboo')
        self.assertEqual(200, resp.status_code)
        self.assertEqual('expected', resp.text)
        self.assertEqual({'foo': ['bar']},  httpretty.last_request().querystring)
        self.assertEqual('Basic %s' % encode.decode('utf-8'), httpretty.last_request().headers.get('Authorization'))

    @httpretty.activate
    def test_api_key(self):
        httpretty.register_uri(httpretty.GET, "http://swagger.py/client-test", body='expected')
    ...
    def test_auth_leak(self):
        ...
        self.assertEqual('expected', resp.text)
        self.assertEqual({'foo': ['bar']}, httpretty.last_request().querystring)
        self.assertIsNone(httpretty.last_request().headers.get('Authorization'))
