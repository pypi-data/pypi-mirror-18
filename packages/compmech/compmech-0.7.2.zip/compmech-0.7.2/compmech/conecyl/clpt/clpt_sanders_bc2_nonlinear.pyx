...
from compmech.conecyl.clpt.clpt_commons_bc2 cimport cfwx, cfwt, cfN, cfv
    ...
    for i in range(npts):
        ...
        for k2 in range(i0, m2+i0):
            ...
            for l2 in range(j0, n2+j0):
                ...
                q12 = cosa*sink2x*sinl2t*(cosa*v - w0t - wt)/(r*r)
                q13 = cosa*cosl2t*sink2x*(cosa*v - w0t - wt)/(r*r)
                q14 = cosl2t*l2*sink2x*(-cosa*v + w0t + wt)/(r*r)
                q15 = l2*sink2x*sinl2t*(cosa*v - w0t - wt)/(r*r)
                q22 = -cosa*sink2x*sinl2t*(w0x + wx)/r
                q23 = -cosa*cosl2t*sink2x*(w0x + wx)/r
        ...
        for i2 in range(i0, m2+i0):
            ...
            for j2 in range(j0, n2+j0):
                ...
                p20 = (L*cosj2t*j2*sini2x*(A12*r + B12*cosa) + sinj2t*(A16*r + B16*cosa)*(-L*sina*sini2x + pi*cosi2x*i2*r))/(L*r)
                p21 = (L*cosj2t*j2*sini2x*(A22*r + B22*cosa) + sinj2t*(A26*r + B26*cosa)*(-L*sina*sini2x + pi*cosi2x*i2*r))/(L*r)
                p22 = (L*cosj2t*j2*sini2x*(A26*r + B26*cosa) + sinj2t*(A66*r + B66*cosa)*(-L*sina*sini2x + pi*cosi2x*i2*r))/(L*r)
                p30 = (-L*sini2x*(cosj2t*sina*(A16*r + B16*cosa) + j2*sinj2t*(A12*r + B12*cosa)) + pi*cosi2x*cosj2t*i2*r*(A16*r + B16*cosa))/(L*r)
                p31 = (-L*sini2x*(cosj2t*sina*(A26*r + B26*cosa) + j2*sinj2t*(A22*r + B22*cosa)) + pi*cosi2x*cosj2t*i2*r*(A26*r + B26*cosa))/(L*r)
                p32 = (-L*sini2x*(cosj2t*sina*(A66*r + B66*cosa) + j2*sinj2t*(A26*r + B26*cosa)) + pi*cosi2x*cosj2t*i2*r*(A66*r + B66*cosa))/(L*r)
    ...
    for i in range(npts):
        ...
        for k2 in range(i0, m2+i0):
            ...
            for l2 in range(j0, n2+j0):
                ...
                q22 = cosa*sink2x*sinl2t/r
                q23 = cosa*cosl2t*sink2x/r
                # kG_02
                c += 1
        ...
        for i2 in range(i0, m2+i0):
            ...
            for j2 in range(j0, n2+j0):
                ...
                # p_2
                p20 = -Nxt*cosa*sini2x*sinj2t
                p21 = -Ntt*cosa*sini2x*sinj2t
                p22 = Ntt*cosa*sini2x*sinj2t
                p30 = -Nxt*cosa*cosj2t*sini2x
                p31 = -Ntt*cosa*cosj2t*sini2x
                p32 = Ntt*cosa*cosj2t*sini2x
    ...
    for i in range(npts):
        ...
        for i2 in range(i0, m2+i0):
            ...
            for j2 in range(j0, n2+j0):
                ...
                # p_2
                p20 = cosa*sini2x*sinj2t*(A12*(cosa*v - w0t - wt) - A16*r*(w0x + wx))/r
                p21 = cosa*sini2x*sinj2t*(A22*(cosa*v - w0t - wt) - A26*r*(w0x + wx))/r
                p22 = cosa*sini2x*sinj2t*(A26*(cosa*v - w0t - wt) - A66*r*(w0x + wx))/r
                p30 = cosa*cosj2t*sini2x*(A12*(cosa*v - w0t - wt) - A16*r*(w0x + wx))/r
                p31 = cosa*cosj2t*sini2x*(A22*(cosa*v - w0t - wt) - A26*r*(w0x + wx))/r
                p32 = cosa*cosj2t*sini2x*(A26*(cosa*v - w0t - wt) - A66*r*(w0x + wx))/r
    ...
    for i in range(npts):
        ...
        for j2 in range(j0, n2+j0):
            ...
            for i2 in range(i0, m2+i0):
                ...
                ettL += (c[col+2]*(0.5*cosa*sini2x*sinj2t*(cosa*v/r - wt/r)/r - cosa*sini2x*sinj2t*w0t/(r*r))
                         +c[col+3]*(0.5*cosa*cosj2t*sini2x*(cosa*v/r - wt/r)/r - cosa*cosj2t*sini2x*w0t/(r*r))
                ...
                gxtL += (c[col+2]*(-cosa*sini2x*sinj2t*w0x/r - 0.5*cosa*sini2x*sinj2t*wx/r)
                         +c[col+3]*(-cosa*cosj2t*sini2x*w0x/r - 0.5*cosa*cosj2t*sini2x*wx/r)
                ...
                ktt0 += (c[col+2]*cosa*cosj2t*j2*sini2x/(r*r)
                ...
                kxt0 += (c[col+2]*cosa*sinj2t*(-L*sina*sini2x + pi*cosi2x*i2*r)/(L*(r*r))
        ...
        for j2 in range(j0, n2+j0):
            ...
            for i2 in range(i0, m2+i0):
                ...
                fint[col+2] = beta*(fint[col+2]) + alpha*((L*sini2x*(cosj2t*j2*(MttL*cosa + NttL*r) + sinj2t*(-NxtL*r*sina + (cosa*cosa)*v*(Ntt0 + NttL) - cosa*(MxtL*sina + Ntt0*(w0t + wt) + NttL*w0t + NttL*wt + Nxt0*r*w0x + Nxt0*r*wx + NxtL*r*w0x + NxtL*r*wx))) + pi*cosi2x*i2*r*sinj2t*(MxtL*cosa + NxtL*r))/(L*r))
                fint[col+3] = beta*(fint[col+3]) + alpha*((-L*j2*sini2x*sinj2t*(MttL*cosa + NttL*r) + cosj2t*(L*sini2x*(-NxtL*r*sina + (cosa*cosa)*v*(Ntt0 + NttL) - cosa*(MxtL*sina + Ntt0*(w0t + wt) + NttL*w0t + NttL*wt + Nxt0*r*w0x + Nxt0*r*wx + NxtL*r*w0x + NxtL*r*wx)) + pi*cosi2x*i2*r*(MxtL*cosa + NxtL*r)))/(L*r))
