...
from compmech.conecyl.fsdt.fsdt_commons_bc1 cimport cfwx, cfwt, cfN
    ...
    for i in range(npts):
        ...
        for i2 in range(i0, m2+i0):
            ...
            for j2 in range(j0, n2+j0):
                ...
                p0200 = pi*A16*cosi2x*i2*r*sinj2t/L + sini2x*(A12*cosj2t*j2 - A16*sina*sinj2t)
                p0201 = pi*A26*cosi2x*i2*r*sinj2t/L + sini2x*(A22*cosj2t*j2 - A26*sina*sinj2t)
                p0202 = pi*A66*cosi2x*i2*r*sinj2t/L + sini2x*(A26*cosj2t*j2 - A66*sina*sinj2t)
                p0300 = pi*A16*cosi2x*cosj2t*i2*r/L - sini2x*(A12*j2*sinj2t + A16*cosj2t*sina)
                p0301 = pi*A26*cosi2x*cosj2t*i2*r/L - sini2x*(A22*j2*sinj2t + A26*cosj2t*sina)
                p0302 = pi*A66*cosi2x*cosj2t*i2*r/L - sini2x*(A26*j2*sinj2t + A66*cosj2t*sina)
    ...
    for i in range(npts):
        ...
        for i2 in range(i0, m2+i0):
            ...
            for j2 in range(j0, n2+j0):
                ...
                # p_2
                p0400 = (L*cosj2t*j2*sini2x*(A12*(w0t + wt) + A16*r*(w0x + wx)) + pi*cosi2x*i2*r*sinj2t*(A11*r*(w0x + wx) + A16*(w0t + wt)))/(L*r)
                p0401 = (L*cosj2t*j2*sini2x*(A22*(w0t + wt) + A26*r*(w0x + wx)) + pi*cosi2x*i2*r*sinj2t*(A12*r*(w0x + wx) + A26*(w0t + wt)))/(L*r)
                p0402 = (L*cosj2t*j2*sini2x*(A26*(w0t + wt) + A66*r*(w0x + wx)) + pi*cosi2x*i2*r*sinj2t*(A16*r*(w0x + wx) + A66*(w0t + wt)))/(L*r)
                p0500 = (-L*j2*sini2x*sinj2t*(A12*(w0t + wt) + A16*r*(w0x + wx)) + pi*cosi2x*cosj2t*i2*r*(A11*r*(w0x + wx) + A16*(w0t + wt)))/(L*r)
                p0501 = (-L*j2*sini2x*sinj2t*(A22*(w0t + wt) + A26*r*(w0x + wx)) + pi*cosi2x*cosj2t*i2*r*(A12*r*(w0x + wx) + A26*(w0t + wt)))/(L*r)
                p0502 = (-L*j2*sini2x*sinj2t*(A26*(w0t + wt) + A66*r*(w0x + wx)) + pi*cosi2x*cosj2t*i2*r*(A16*r*(w0x + wx) + A66*(w0t + wt)))/(L*r)
    ...
    for i in range(npts):
        ...
        for j2 in range(j0, n2+j0):
            ...
            for i2 in range(i0, m2+i0):
                ...
                ett0 += (c[col+0]*sina*sini2x*sinj2t/r
                         ...
                         +c[col+4]*sini2x*cosa*r*sinj2t/(r*r)
                         +c[col+5]*sini2x*cosa*cosj2t*r/(r*r))
                ...
                gtz0 += (-c[col+3]*cosa*cosj2t*sini2x/r
        ...
        for j2 in range(j0, n2+j0):
            ...
            for i2 in range(i0, m2+i0):
                ...
                fint[col+2] = beta*(fint[col+2]) + alpha*((L*sini2x*(NttL*cosj2t*j2 - sinj2t*(NxtL*sina + QtL*cosa)) + pi*NxtL*cosi2x*i2*r*sinj2t)/L)
                fint[col+3] = beta*(fint[col+3]) + alpha*((-L*NttL*j2*sini2x*sinj2t + cosj2t*(-L*sini2x*(NxtL*sina + QtL*cosa) + pi*NxtL*cosi2x*i2*r))/L)
