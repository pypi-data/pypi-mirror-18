...
cdef void cfuvw(double *c, int m1, int m2, int n2, double r2,
    ...
    for j2 in range(j0, n2+j0):
        ...
        for i2 in range(i0, m2+i0):
            ...
            phix += c[col+3]*cosbi*cosbj
            phit += c[col+4]*sinbi*sinbj
...
cdef cfg(double[:, ::1] g, int m1, int m2, int n2,
    ...
    for i2 in range(i0, m2+i0):
        ...
        for j2 in range(j0, n2+j0):
            ...
            g[3, col+3] = cosbi*cosbj
            g[4, col+4] = sinbi*sinbj
                    ...
                    for l2 in range(j0, n2+j0):
                        ...
                        if k2==i2 and l2==j2:
                            if i2!=0:
                                ...
                                k0r[c] = row+0
                                k0c[c] = col+2
                                k0v[c] += 0.25*cosa*(pi*A12*(-L*sin(2*pi*i2*xa/L) + L*sin(2*pi*i2*xb/L) + 2*pi*i2*(xa - xb)) + A22*L**2*sina*(cos(2*pi*i2*xa/L) - cos(2*pi*i2*xb/L))/(i2*r))/L
                                c += 1
                                k0r[c] = row+0
                                k0c[c] = col+3
                            ...
                            k0r[c] = row+0
                            k0c[c] = col+2
                            k0v[c] += (cosa*i2*(pi*A12*i2*r*cos(pi*i2*xb/L) + A22*L*sina*sin(pi*i2*xb/L))*sin(pi*k2*xb/L) - cosa*(i2*(-pi*A12*k2*r*cos(pi*k2*xa/L) + A22*L*sina*sin(pi*k2*xa/L))*sin(pi*i2*xa/L) + k2*(pi*A12*i2*r*sin(pi*i2*xb/L) - A22*L*sina*cos(pi*i2*xb/L))*cos(pi*k2*xb/L) + (pi*A12*i2**2*r*sin(pi*k2*xa/L) + A22*L*k2*sina*cos(pi*k2*xa/L))*cos(pi*i2*xa/L)))/(r*(i2 - k2)*(i2 + k2))
                            c += 1
                            k0r[c] = row+0
                            k0c[c] = col+3
                            ...
                            k0r[c] = row+2
                            k0c[c] = col+0
                            k0v[c] += cosa*(i2*(-pi*A12*k2*r*sin(pi*k2*xa/L) + A22*L*sina*cos(pi*k2*xa/L))*cos(pi*i2*xa/L) + i2*(pi*A12*k2*r*sin(pi*k2*xb/L) - A22*L*sina*cos(pi*k2*xb/L))*cos(pi*i2*xb/L) + k2*(pi*A12*k2*r*cos(pi*k2*xa/L) + A22*L*sina*sin(pi*k2*xa/L))*sin(pi*i2*xa/L) - k2*(pi*A12*k2*r*cos(pi*k2*xb/L) + A22*L*sina*sin(pi*k2*xb/L))*sin(pi*i2*xb/L))/(r*(i2 - k2)*(i2 + k2))
                            c += 1
                            k0r[c] = row+2
                            k0c[c] = col+1
                            k0v[c] += L*cosa*j2*(A22 + A44)*(i2*sin(pi*k2*xa/L)*cos(pi*i2*xa/L) - i2*sin(pi*k2*xb/L)*cos(pi*i2*xb/L) - k2*sin(pi*i2*xa/L)*cos(pi*k2*xa/L) + k2*sin(pi*i2*xb/L)*cos(pi*k2*xb/L))/(r*(i2**2 - k2**2))
