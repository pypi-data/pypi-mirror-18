...
from credits.digest import digest
...
class SingleKeyProof(Proof):
    ...
    def verify(self, state):
        ...
        challenge_digest = digest([
            bytes.fromhex(self.challenge),
            self.address,
            self.nonce
        ])
        if not self.verifying_key.verify(challenge_digest, bytes.fromhex(self.signature)):
        ...
        if self.nonce < known_nonce:
            error = "SingleKeyProof nonce (%s) is less than current nonce (%s) for %s" % (
                self.nonce,
                known_nonce,
                address,
            )
    ...
    def sign(self, signing_key):
        ...
        signature = signing_key.sign(challenge_digest).hex()
