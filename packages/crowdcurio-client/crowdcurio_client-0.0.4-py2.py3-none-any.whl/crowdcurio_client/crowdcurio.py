    ...
    def http_request(
        ...
        if response.status_code >= 400:
            raise CrowdCurioAPIException(
                '{0} Error: {1}'.format(
                    response.status_code, JSON.loads(response.content)['errors']
                )
            )
        return response
...
class CrowdCurioObject(object):
    ...
    def save(self):
        ...
        response, _ = save_method(
            self.url(self.id),
            json={'data': {'type': self._api_slug.capitalize(), 'id': c_id,'attributes': self._savable_dict(
        ...
        if self.object_index >= self.object_count:
            if self.next_href:
                if "page" in self.next_href:
                   ...
                   response, _ = CrowdCurio.client().get(self.next_href, params={'page':page})
                else:
                    response, _ = CrowdCurio.client().get(self.next_href)
                self.set_page(response)
            else:
                raise StopIteration
