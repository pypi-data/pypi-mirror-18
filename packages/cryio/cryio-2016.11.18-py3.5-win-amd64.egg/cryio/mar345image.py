...
class Mar345Image:
    ...
    def _parse_binary(self):
        ...
        if self.binary_header.format == MAR_FORMAT_COMPRESSED:
            ...
            buf = _cryio.decode_mar(self._capsule, dim1, dim2, of, self._mmap[MAR_HEADER_SIZE:s], self._mmap[ds:])
            self.array = np.frombuffer(buf, np.uint32)
            self.array = self.array.reshape(self.binary_header.dim1, dim2)
