...
class CbfImage(CbfHeader):
    ...
    def parse_binary(self, cbfmap):
        ...
        self.array = np.frombuffer(_cryio.decode_cbf(self._cbf_, pixels, self.cbf_str), np.int32, pixels)
        self.array.flags.writeable = True
        self.array = self.array.reshape(shapeX, shapeY)
    ...
    def save_cbf(self, filepath, **kwargs):
        ...
        pixels = shapeX * shapeY
        array = self.array.reshape((pixels,))
        ...
        self.cbf_str = _cryio.encode_cbf(self._cbf_, array.astype(np.int32).tostring())
        self.binary_header_dict['X-Binary-Size'] = len(self.cbf_str)
        self.binary_header_dict['Content-MD5'] = base64.b64encode(hashlib.md5(self.cbf_str).digest()).decode()
