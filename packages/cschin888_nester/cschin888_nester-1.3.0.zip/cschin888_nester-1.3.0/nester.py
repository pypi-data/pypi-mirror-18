...
def print_lol(the_list, indent=False, level=0):
    """This function takes a positional argument called "the_list", which is
       ...
       A second argument called "indent". If true, enable tab-stop 
       A third argument called "level" for how many tab-stops when a
       nested list is encountered."""

    for each_item in the_list:
        if isinstance(each_item, list):
            print_lol(each_item, indent, level+1) #recursive call
        else:
            if indent:
