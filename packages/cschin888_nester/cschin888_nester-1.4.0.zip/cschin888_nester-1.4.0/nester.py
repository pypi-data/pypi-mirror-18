...
def print_lol(the_list, indent=False, level=0, fh=sys.stdout):
    """This function takes a positional argument called "the_list", which is
       ...
       A third argument called "level" (which default to 0) is used to insert
       tab-stops when a nested list is encountered.
       A fourth argument is file handle"""

    for each_item in the_list:
        if isinstance(each_item, list):
            print_lol(each_item, indent, level+1, fh) #recursive call
        else:
            if indent:
