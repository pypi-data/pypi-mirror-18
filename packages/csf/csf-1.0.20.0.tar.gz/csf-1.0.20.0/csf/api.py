...
def get_stock_hist_bar(code, freq="d", start_date=None, end_date=None, adjusted="f", field=None):    """获取股票历史时间段K线数据    Parameters    ----------    code:str
    ...
    dfs = __get_dataframe("/api/hq/stock/inc", params)
    ...
    return __return_format(dfs, field=None, renames=renames, ret_fields=ret_fields, index=index)def get_supply_chain_relation(sam, lang="szh"):    """产业链关系 （限制在数库行业四级和五级）    Parameters    ----------    sam:str
        samcode
    lang:str,default "szh"
        语言类别 枚举值：szh、en    Returns    -------    prime_cd:str 主科目编码
    ...
    rtyp:str 关联科目相对主科目的关系类型：A：生产工具
                                             ...
                                             T：服务    Examples    --------    >>> import csf
    >>> csf.config.set_token("ACCESS_KEY","SECRET_KEY")
    >>> csf.get_supply_chain_relation(sam="PS00501",lang="szh")    """    params = dict()
    ...
    if not __chk_type(lang, [str]):
        raise TypeError("get_supply_chain_relation() arg lang must be a str type")

    params["lang"] = __format_param("lang", lang)
    ...
    return __return_format(dfs, field=None, renames=renames, ret_fields=ret_fields, index=index)def get_stock_basic_info(codes, field=None, lang="szh"):    """股票基本信息    Parameters    ----------    codes:str  or  list
        股票代码，一个/多个，最多10个
    field:list,default None
        返回字段
    lang:str,default "szh"
    ...
    listing:str 上市状态    Examples    --------    >>> import csf
    >>> csf.config.set_token("ACCESS_KEY","SECRET_KEY")
    >>> csf.get_stock_basic_info(codes=["601398","002183"],lang="szh")    """    params = dict()
    ...
    if not __chk_type(lang, [str]):
        raise TypeError("get_stock_basic_info() arg lang must be a str type")
    ...
    return __return_format(dfs, field=field, renames=renames, ret_fields=ret_fields, index=index)def get_stock_csf_industry(codes, field=None, lang="szh"):    """根据股票代码查询股票数库行业分类    Parameters    ----------    codes:str  or  list
        股票代码，一个/多个 , 最多100个
    field:list,default None
        返回字段
    lang:str,default "szh"
    ...
    >>> csf.config.set_token("ACCESS_KEY","SECRET_KEY")
    >>> csf.get_stock_csf_industry(codes="601398",lang="szh")    """    params = dict()
    ...
    if not __chk_type(lang, [str]):
        raise TypeError("get_stock_csf_industry() arg lang must be a str type")
    ...
    return __return_format(dfs, field=field, renames=renames, ret_fields=ret_fields, index=index)def get_stock_industry(codes, field=None, lang="szh"):    """根据股票代码查询股票申万行业分类    Parameters    ----------    codes:str  or  list
        股票代码，一个/多个 , 最多100个
    field:list,default None
        返回字段
    lang:str,default "szh"
    ...
    level3_code:str 三级行业编码    Examples    --------    >>> import csf
    >>> csf.config.set_token("ACCESS_KEY","SECRET_KEY")
    >>> csf.get_stock_industry(codes="601398",lang="szh")    """    params = dict()
    ...
    if not __chk_type(lang, [str]):
        raise TypeError("get_stock_industry() arg lang must be a str type")
    ...
    return __return_format(dfs, field=field, renames=renames, ret_fields=ret_fields, index=index)def get_stock_ipo_info(code, field=None, lang="szh"):    """根据股票代码查询股票首次上市信息    Parameters    ----------    code:str  or  list
        股票代码，一个
    field:list,default None
        返回字段
    lang:str,default "szh"
    ...
    invt:str 募投内容    Examples    --------    >>> import csf
    >>> csf.config.set_token("ACCESS_KEY","SECRET_KEY")
    >>> csf.get_stock_ipo_info(code="601398",lang="szh")    """    params = dict()
    ...
    if not __chk_type(lang, [str]):
        raise TypeError("get_stock_ipo_info() arg lang must be a str type")
    ...
    return __return_format(dfs, field=field, renames=renames, ret_fields=ret_fields, index=index)def get_stock_dividend(code, field=None, lang="szh"):    """根据股票代码查询股票分红信息    Parameters    ----------    code:str 
        股票代码，一个
    field:list,default None
        返回字段
    lang:str,default "szh"
    ...
    >>> csf.config.set_token("ACCESS_KEY","SECRET_KEY")
    >>> csf.get_stock_dividend(code="601398",lang="szh")    """    params = dict()
    ...
    if not __chk_type(lang, [str]):
        raise TypeError("get_stock_dividend() arg lang must be a str type")
    ...
    return __return_format(dfs, field=field, renames=renames, ret_fields=ret_fields, index=index)def get_stock_rightissue(code, lang="szh"):    """根据股票代码查询股票配股信息    Parameters    ----------    code:str 
        股票代码，一个
    lang:str,default "szh"
        语言类别 枚举值：szh、en    Returns    -------    code:str 股票代码
    secu_name:str 股票简称
    ...
    >>> csf.config.set_token("ACCESS_KEY","SECRET_KEY")
    >>> csf.get_stock_rightissue(code="601398",lang="szh")    """    params = dict()
    ...
    if not __chk_type(lang, [str]):
        raise TypeError("get_stock_rightissue() arg lang must be a str type")
    ...
    return __return_format(dfs, field=None, renames=renames, ret_fields=ret_fields, index=index)def get_st_stock_today(date=None, lang="szh"):    """查询指定日期特别处理的股票    Parameters    ----------    date:str,default None
        查询日期 格式：yyyy-MM-dd
    lang:str,default "szh"
        语言类别 枚举值：szh、en    Returns    -------    code:str 股票代码
    secu_name:str 股票简称
    ...
    >>> csf.config.set_token("ACCESS_KEY","SECRET_KEY")
    >>> csf.get_st_stock_today(date="2016-04-08",lang="szh")    """    params = dict()
    ...
    if not __chk_type(lang, [str]):
        raise TypeError("get_st_stock_today() arg lang must be a str type")
    ...
    return __return_format(dfs, field=None, renames=renames, ret_fields=ret_fields, index=index)def get_csf_ind_stocks(ind_code, date=None, lang="szh"):    """查询数库行业分类包含的股票    Parameters    ----------    ind_code:str
        行业代码，一个
    date:str,default None
        日期，为None取当前日期 
    lang:str,default "szh"
    ...
    secu_name:str 股票简称    Examples    --------    >>> import csf
    >>> csf.config.set_token("ACCESS_KEY","SECRET_KEY")
    >>> csf.get_csf_ind_stocks(ind_code="CSF_20",lang="szh")    """    params = dict()
    ...
    if not __chk_type(lang, [str]):
        raise TypeError("get_csf_ind_stocks() arg lang must be a str type")
    ...
    return __return_format(dfs, field=None, renames=renames, ret_fields=ret_fields, index=index)def get_stock_list(markets=["1001","1002","1003","1012","1008"], lang="szh"):    """证券列表    Parameters    ----------    markets:str  or  list,default ["1001","1002","1003","1012","1008"]
    ...
    name:str 股票简称    Examples    --------    >>> import csf
    >>> csf.config.set_token("ACCESS_KEY","SECRET_KEY")
    >>> csf.get_stock_list(markets="1001",lang="szh")    """    params = dict()
    ...
    if not __chk_type(lang, [str]):
        raise TypeError("get_stock_list() arg lang must be a str type")
    ...
    return __return_format(dfs, field=None, renames=renames, ret_fields=ret_fields, index=index)def get_index_component(index_code, date=None, field=None, lang="szh"):    """常用指数（非数库）成份股    Parameters    ----------    index_code:str
    ...
    >>> csf.config.set_token("ACCESS_KEY","SECRET_KEY")
    >>> csf.get_index_component(index_code="000906",date="2016-04-05",lang="szh")    """    params = dict()
    ...
    if not __chk_type(lang, [str]):
        raise TypeError("get_index_component() arg lang must be a str type")
    ...
    ret_fields = list()
    ret_fields.append("code")
    renames["lang为szh时，中文名称，lang为en时，英文名称"] = "name"
    ret_fields.append("name")
    ret_fields.append("weight")
    ret_fields.append("market_cap")
    renames["lang为szh时，中文名称，lang为en时，英文名称"] = "industry"
    ret_fields.append("industry")
    renames = renames if renames else None

    dfs = __get_dataframe("/api/sdk/market/index/stock/weight", params)
    return __return_format(dfs, field=field, renames=renames, ret_fields=ret_fields, index=index)def get_csf_index_com(index_code, date=None, field=None, lang="szh"):    """数库指数成份股    Parameters    ----------    index_code:str
    ...
    industry:str 股票所属行业（默认数库二级行业）    Examples    --------    >>> import csf
    >>> csf.config.set_token("ACCESS_KEY","SECRET_KEY")
    >>> csf.get_csf_index_com(index_code="AAAEAB",date="2016-04-28",lang="szh")    """    params = dict()
    ...
    if not __chk_type(lang, [str]):
        raise TypeError("get_csf_index_com() arg lang must be a str type")
    ...
    return __return_format(dfs, field=field, renames=renames, ret_fields=ret_fields, index=index)def get_index_info(index_code, field=None, lang="szh"):    """常用指数（非数库）基本信息    Parameters    ----------    index_code:str or list
    ...
    basepoint:int 基点    Examples    --------    >>> import csf
    >>> csf.config.set_token("ACCESS_KEY","SECRET_KEY")
    >>> csf.get_index_info(index_code=["000001","000003.SH"],lang="szh")    """    params = dict()
    ...
    if not __chk_type(lang, [str]):
        raise TypeError("get_index_info() arg lang must be a str type")
    ...
    return __return_format(dfs, field=field, renames=renames, ret_fields=ret_fields, index=index)def get_csf_index_info(index_code, field=None, lang="szh"):    """数库指数基本信息(含京东大数据指数)    Parameters    ----------    index_code:str or list
    ...
    edt :str 最新一期换期结束日期    Examples    --------    >>> import csf
    >>> csf.config.set_token("ACCESS_KEY","SECRET_KEY")
    >>> csf.get_csf_index_info(index_code="CAP500",lang="szh")    """    params = dict()
    ...
    if not __chk_type(lang, [str]):
        raise TypeError("get_csf_index_info() arg lang must be a str type")
    ...
    return __return_format(dfs, field=None, renames=renames, ret_fields=ret_fields, index=index)def get_macro_data(marco_code, start_date="1990-01-01", end_date=None, timesort="D", lang="szh"):    """宏观数据    Parameters    ----------    marco_code:str
        指标代码
    ...
    type:str 类型    Examples    --------    >>> import csf
    >>> csf.config.set_token("ACCESS_KEY","SECRET_KEY")
    >>> csf.get_macro_data(marco_code="A10000011",start_date="2005-01-01",end_date="2016-06-01",timesort="D",lang="szh")    """    params = dict()
    ...
    if not __chk_type(lang, [str]):
        raise TypeError("get_macro_data() arg lang must be a str type")
    ...
    return __return_format(dfs, field=None, renames=renames, ret_fields=ret_fields, index=index)def get_notice_by_type(notice_type, start_date, end_date, field=None, lang="szh"):    """返回过去一段时间内某类型的公告数据    Parameters    ----------    notice_type:str
    ...
    pdt:str 公告日期    Examples    --------    >>> import csf
    >>> csf.config.set_token("ACCESS_KEY","SECRET_KEY")
    >>> csf.get_notice_by_type(notice_type="260201",start_date="2010-01-01",end_date="2016-01-01",lang="szh")    """    params = dict()
    ...
    if not __chk_type(lang, [str]):
        raise TypeError("get_notice_by_type() arg lang must be a str type")
    ...
    return __return_format(dfs, field=field, renames=renames, ret_fields=ret_fields, index=index)def get_notice_by_stock(code, start_date=None, end_date=None, field=None, lang="szh"):    """返回某只股票过去一段时间内的公告数据    Parameters    ----------    code:str
        股票代码，只能输入一个股票代码
    start_date:str,default None
        开始日期
    end_date:str,default None
    ...
    pdt:str 公告日期    Examples    --------    >>> import csf
    >>> csf.config.set_token("ACCESS_KEY","SECRET_KEY")
    >>> csf.get_notice_by_stock(code="600628",start_date="2010-01-01",end_date="2016-01-01",lang="szh")    """    params = dict()
    ...
    if not __chk_type(lang, [str]):
        raise TypeError("get_notice_by_stock() arg lang must be a str type")
    ...
    dfs = __get_dataframe("/api/sdk/announce", params)
    return __return_format(dfs, field=field, renames=renames, ret_fields=ret_fields, index=index)def get_notice_by_date(date=None, field=None, lang="szh"):    """获取某一天披露的所有股票的所有公告数据    Parameters    ----------    date:str,default None
        查询日期
    field:list,default None
        返回字段
    lang:str,default "szh"
    ...
    pdt:str 公告日期    Examples    --------    >>> import csf
    >>> csf.config.set_token("ACCESS_KEY","SECRET_KEY")
    >>> csf.get_notice_by_date(date="2016-02-01",lang="szh")    """    params = dict()
    ...
    if not __chk_type(lang, [str]):
        raise TypeError("get_notice_by_date() arg lang must be a str type")
