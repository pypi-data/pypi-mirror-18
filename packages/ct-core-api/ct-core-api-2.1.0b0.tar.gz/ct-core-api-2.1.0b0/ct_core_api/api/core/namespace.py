...
class APIAuthNamespace(APINamespace):
    ...
    def login_required(self, oauth_scopes):
        ...
        * All of the above requirements are put into OpenAPI Specification with
        ...
        def decorator(func_or_class):
            ...
            # Accumulate OAuth2 scopes if @login_required decorator is applied several times
            api_doc = getattr(protected_func, '__apidoc__', {})
            if 'security' in api_doc and '__oauth__' in api_doc['security']:
                _oauth_scopes = (oauth_scopes + api_doc['security']['__oauth__']['scopes'])
            else:
            ...
            def oauth_protection_decorator(func):
                ...
                """
                oauth_protected_func = self.OAUTH2_PROVIDER.require_oauth(*_oauth_scopes)(func)

                @wraps(oauth_protected_func)
                def wrapper(self, *args, **kwargs):
                    latest_oauth_decorator_id = getattr(
                        getattr(self, func.__name__), '__latest_oauth_decorator_id__', None)
