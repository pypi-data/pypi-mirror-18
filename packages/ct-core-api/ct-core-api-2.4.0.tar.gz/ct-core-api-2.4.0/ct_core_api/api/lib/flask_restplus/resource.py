...
class Resource(OriginalResource):
    """
    Extended Flask-RESTPlus Resource to add options method
    """
    ...
    def options(self, *args, **kwargs):
        ...
        performed on this endpoint, e.g. to decide whether to display a button
        in your UI.
        ...
        # methods to provide information about what methods `current_user` can use.
        ...
        for method_func in method_funcs:
            if getattr(method_func, '_access_restriction_decorators', None):
                if not hasattr(method_func, '_cached_fake_method_func'):
                    def fake_method_func(*args, **kwargs):  # noqa
                        return True
