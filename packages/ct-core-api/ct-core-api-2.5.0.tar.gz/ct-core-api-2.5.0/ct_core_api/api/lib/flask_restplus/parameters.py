...
__all__ = ['Parameters', 'PostFormParameters', 'PatchJSONParameters']
...
class PostFormParameters(Parameters):
    def __init__(self, **kwargs):
        super(PostFormParameters, self).__init__(**kwargs)
        self.default_locations = ('form',)
...
class PatchJSONParameters(Parameters):
    ...
    def __init__(self, **kwargs):
        super(PatchJSONParameters, self).__init__(many=True, **kwargs)
    ...
    def perform_patch(cls, operations, obj, state=None):
        ...
        for operation in operations:
            if not cls._process_patch_operation(operation, obj=obj, state=state):
                _logger.info("{} patching has stopped because of unknown operation {}".format(obj.__name__, operation))
