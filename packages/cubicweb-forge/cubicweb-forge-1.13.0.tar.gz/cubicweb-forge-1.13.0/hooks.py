...
class SetModificationDateAfterAddComment(Hook):
    ...
    def __call__(self):
        ...
        if entity.e_schema == 'Comment':
            entity = entity.cw_adapt_to('ITree').root()
        entity.cw_set(modification_date=datetime.now(tz=utc if CW322 else None))
...
class TicketProgressHook(Hook):
    ...
    def __call__(self):
        if 'load' in self.entity.cw_edited or 'load_left' in self.entity.cw_edited:
            try:
