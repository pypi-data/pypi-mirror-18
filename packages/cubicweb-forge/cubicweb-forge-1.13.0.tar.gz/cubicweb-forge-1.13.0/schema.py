...
from yams.constraints import IntervalBoundConstraint, Attribute
...
Ticket.add_relation(Float(description=_('load for this ticket in day.man'),
                    ...
                    name='load')
# use two separate constraints so the >= 0 is enforced even when load is NULL
Ticket.add_relation(Float(description=_('remaining load for this ticket in day.man'),
                          constraints=[IntervalBoundConstraint(minvalue=0),
                                       IntervalBoundConstraint(maxvalue=Attribute('load'))]),
                    name='load_left')
...
class License(EntityType):
    ...
    shortdesc = String(required=False, fulltextindexed=True,
                       description=_('disclaimer of the license'))
    longdesc = RichString(required=False, fulltextindexed=True,
                          description=_("full license's text"))
...
nosy_list_subjects = ('Email', 'Project', 'Version', 'Ticket',
...
class see_also(RelationDefinition):
    symmetric = True
    subject = ('Project', 'Ticket', 'Card', 'File', 'Email')
    object = ('Project', 'Ticket', 'Card', 'File', 'Email')
...
class tags(RelationDefinition):
    subject = 'Tag'
    object = ('Project', 'Version', 'Ticket',
              'License', 'MailingList',
              'Card', 'File', 'Email')


class filed_under(RelationDefinition):
    subject = ('Project', 'Card', 'File')
    object = 'Folder'
