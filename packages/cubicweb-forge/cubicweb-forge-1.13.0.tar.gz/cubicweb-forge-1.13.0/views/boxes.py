...
class VersionIDownloadableAdapter(EntityAdapter):
    ...
    __select__ = (is_instance('Version') &
                  score_entity(lambda x: x.project.downloadurl
                               and x.cw_adapt_to('IWorkflowable').state == 'published'))
...
class FollowupSideboxView(component.EntityCtxComponent):
    ...
    title = _('ticket traceability chain')
    context = 'incontext'
    ...
    def render_body(self, w):
        ...
        for elt in reversed(list(parent)[1:]):
            w(self.wrapper % ('other',
                              elt.view('incontext'),
                              elt.cw_adapt_to('IWorkflowable').printable_state))
        w(self.wrapper % ('current', ticket.view('incontext'),
                          ticket.cw_adapt_to('IWorkflowable').printable_state))
