...
if CW322:
    class PlanSerializableAdapter(ISerializableAdapter):
        __select__ = is_instance('Plan')

        def serialize(self):
            data = super(PlanSerializableAdapter, self).serialize()
            data['recipe'] = self.entity.recipe.cw_adapt_to(self.__regid__).serialize()
            data['state'] = self.entity.execution_status
            return data
