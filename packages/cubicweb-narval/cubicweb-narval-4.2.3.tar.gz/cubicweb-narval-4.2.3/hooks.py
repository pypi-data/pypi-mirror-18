...
class RunnablePlanOp(hook.Operation):
    def precommit_event(self):
        plan = self.plan
        if not (plan.recipe and plan.recipe.may_be_started()):
...
class SetScriptOnPlanHook(hook.Hook):
    ...
    def __call__(self):
        plan = self._cw.entity_from_eid(self.eidfrom)
        if plan.script is None:
            script = self._cw.entity_from_eid(self.eidto).script
            plan.cw_set(script=script)
