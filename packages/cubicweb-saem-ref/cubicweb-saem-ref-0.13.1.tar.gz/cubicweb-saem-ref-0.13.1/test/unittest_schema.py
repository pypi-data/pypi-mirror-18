...
class AuthorityRecordTC(CubicWebTC):
    ...
    def test_seda_originating_agency_to_redirection(self):
        """in the seda cube, seda_originating_agency_to relation targets an Agent. In saem_ref, we
        want to target an AuthorityRecord.
        """
        with self.admin_access.repo_cnx() as cnx:
            profile = testutils.setup_profile(cnx)
            unit, unit_alt, unit_alt_seq = testutils.create_archive_unit(profile)
            content = cnx.create_entity('SEDAContent', seda_content=unit_alt_seq)
            cnx.create_entity('SEDATitle', seda_title=content)
            cnx.commit()
            # attempt to link an agent raise a TypeResolverException since this is not possible
            # anymore according to saem_ref'schema
            agent = testutils.agent(cnx, u'agent')
            with self.assertRaises(TypeResolverException):
                cnx.create_entity('SEDAOriginatingAgency',
                                  seda_originating_agency_from=content,
                                  seda_originating_agency_to=agent)
            # but it goes fine with an authority record
            record = testutils.authority_record(cnx, u'record')
            cnx.create_entity('SEDAOriginatingAgency',
                              seda_originating_agency_from=content,
