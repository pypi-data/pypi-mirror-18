...
if cwversion >= (3, 21, 99):
    from cubicweb.entities.adapters import ISerializableAdapter

    class VersionSerializableAdapter(ISerializableAdapter):
        __select__ = is_instance('Version')

        def serialize(self):
            data = super(VersionSerializableAdapter, self).serialize()
            data['state'] = self.entity.cw_adapt_to('IWorkflowable').state
            return data
...
class VersionIUserFriendlyUniqueTogether(IUserFriendlyUniqueTogether):
    ...
    def raise_user_exception(self):
        ...
        raise ValidationError(self.entity.eid, dict((col, msg) for col in self.exc.rtypes))
