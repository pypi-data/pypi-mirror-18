...
class RelationChange(hook.Hook):
    ...
    def __call__(self):
        if self.rtype in ('concerns', 'version_of', 'subproject_of'):
            ...
            ProjectModificationDateUpdateFromEntity.get_instance(self._cw).add_data(self.eidto)
...
class TicketVersionChange(hook.Hook):
    ...
    def __call__(self):
        ProjectModificationDateUpdateFromEntity.get_instance(self._cw).add_data(self.entity.eid)
...
class ProjectModificationDateUpdate(hook.DataOperationMixIn, hook.Operation):
    ...
    def precommit_event(self):
        now = datetime.now(tz=utc if CW322 else None)
...
class VersionPublishHook(hook.Hook):
    ...
    def __call__(self):
        ...
        if version.publication_date is None:
            version.cw_set(publication_date=date.today())
...
# not necessary on:
#
# * "secondary" relations: assigned_to, done_in, depends_on
