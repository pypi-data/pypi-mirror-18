...
class RevisionAdded(hook.Hook):
    """Detect that a new revision is added to prepare relevant operation
    """
    __regid__ = 'trackervcs.revision-to-ticket-state.add'
    ...
    def __call__(self):
        ...
        if not repo.reverse_source_repository:
            return
        SearchRevisionTicketOp.get_instance(self._cw).add_data(self.eidfrom)


class RevisionUpdated(hook.Hook):
    """Detect that a revision is touched to prepare relevant operation

    We ignore non-phase change.
    """
    __regid__ = 'trackervcs.revision-to-ticket-state.update'
...
class SetSourceRepositoryOp(hook.Operation):
    ...
    def precommit_event(self):
        if not self.project.source_repository and self.projectenv.repository:
            ...
            self.project.cw_set(source_repository=repo)
    ...
    def __call__(self):
        ...
        for ticket in tickets(self._cw, eids, source_repo.eid):
            ...
            if all(x.eid != ticket.eid for x in patch.patch_ticket):
                patch.cw_set(patch_ticket=ticket)


# sync repo ownership with project #############################################

class ProjectRepoOwner(hook.Hook):
    """Add/remove repository owner when project owner is added/removed"""
    __regid__ = 'trackervcs.repository-owner-sync'
    __select__ = hook.Hook.__select__ & hook.match_rtype('owned_by', frometypes=('Project',))
    ...
    def __call__(self):
        ...
        if 'add' in self.event:
            ...
            action = 'DELETE'
            cond = ''
        self._cw.execute('%s REPO owned_by U WHERE P source_repository REPO, '
                         'P eid %%(p)s, U eid %%(u)s, %s REPO owned_by U' % (action, cond),
