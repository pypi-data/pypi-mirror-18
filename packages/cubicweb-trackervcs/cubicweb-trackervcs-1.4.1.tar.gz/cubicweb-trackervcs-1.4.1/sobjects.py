def registration_callback(vreg):
    ...
    class PatchAddedView(vcreview.PatchAddedView):
        def subject(self):
            patch = self.cw_rset.get_entity(0, 0)
            if not patch.repository.project:
                return super(PatchAddedView, self).subject()
    ...
    class PatchStatusChangeView(vcreview.PatchStatusChangeView):
        def subject(self):
            patch = self.cw_rset.get_entity(0, 0)
            if not patch.repository.project:
                return super(PatchStatusChangeView, self).subject()
            repo_title = patch.repository.project.dc_title()
    ...
    class ReviewerSetNotificationView(vcreview.ReviewerSetNotificationView):
        def subject(self):
            patch = self.cw_rset.get_entity(0, 0)
            if not patch.repository.project:
                return super(ReviewerSetNotificationView, self).subject()
            repo_title = patch.repository.project.dc_title()
            return self._cw.__('[%(repo)s] patch assigned: %(patch)s') % {
                'repo': repo_title,
                'patch': patch.dc_title()}

    vreg.register_and_replace(PatchAddedView, vcreview.PatchAddedView)
