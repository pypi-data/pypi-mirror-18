...
def reprocess(id,queue,options):
    while True:
        try:
            filehash=queue.get(False)
        except:
            time.sleep(1)
            logger.debug("[Pool Worker-%d] No jobs in queue..sleeping"%(id))
            continue
        #have a job..do it
        result=SubmitJob(options).reprocess(filehash)
        status=result.pop('status')
        if status=='success':
            logger.info("[Pool Worker-%d] Submitted file %s"%(id,filehash))
            while not task_done(filehash,options): #wait for task to finish
                time.sleep(15)
        else:
            logger.error("[Pool Worker-%d] Unable to Submit file %s"%(id,filehash))
...
class PooledReprocessVisitor(SubmitJob):
    ...
    def close(self):
        if self.queue.qsize()==0:
            ...
            time.sleep(1)
            self.close()
    def visit(self,filehash, result):
        #addToQueue
        self.queue.put(filehash)
    def okToProcess (self, filehash):
        return True
