...
class BackupTask(object):
    ...
    def __repr__(self):
        if self.state == 'finished':
            return 'FINISHED {0}'.format(self.backup_date)

        elif self.state == 'failed':
            return 'FAILED {0}'.format(self.started)

        else:
            return '{0} {1}'.format(self.state.upper(), self.started)
...
class BackupdLog(LogFile):
    """Parse backupd log entries

    Parse log entries from backupd from system.log
    """
    lineloader = BackupdLogEntry

    def __init__(self):
        super(BackupdLog, self).__init__('/var/log/system.log')
        self.backups = []
