...
class Access(object):
    ...
    def headers(self, headers):
        ...
        assert "Authentication" in headers.keys(), "Authentication must be present in the headers"
        self._headers = headers

    def new_authentication(self, authentication_list, synchronous=False):
        """
    ...
    def check_date(self, url):
        ...
        if response is None:
            ...
            # No last date
            return None
        elif int(response[0]["_id"]) <= MIN_BATCH_ID:
            logging.info("Data found, but was older than minimum thresholds.")
            # Data older than what is acceptable
            return None
        else:
    ...
    def post_lines(self, dest_url, data, verbose=False):
    ...
    def get(self, url, query=None, fields=None, sort='', agg=False, pg=1, limit=1000):
        ...
        :type query: list
        :param pg: (optional)int(page_number) Defaults to 1
        :type pg: int
        :param limit: (optional)int(page_size) Defaults to the maximum value 1000
        :type limit: int
        :param sort: (optional)string(sort_condition)
        :type sort: str
        :param agg: (optional)bool Use aggreagation endpoint. Default False.
        :type agg: bool
        ...
        # Pagination index
        page = 0
        max_page = self.head(url + "?q=" + json.dumps(query))["X-Count"]
        ...
        while page < max_page:
            ...
            while i <= 2 and getting:
                try:
                    response = requests.get(qurl, headers=self.headers)
    ...
    def _sub_post(self, url, data_set):
