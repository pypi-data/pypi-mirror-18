...
class ReplaceCluster(_QObject):
    ...
    def _init(self):
        self.nodes = None
        self.cluster = None
        self.environment = mdk_protocol.OperationalEnvironment()
...
class Cluster(_QObject):
    ...
    def _init(self):
        ...
        self._registeredVersions = _List([])
    ...
    def matchingVersionRegistered(self, version):
        """
        ...
        """
        idx = 0;
        while ((idx) < (len(self._registeredVersions))):
            if (mdk_util.versionMatch(version, (self._registeredVersions)[idx])):
                return True
    ...
    def add(self, node):
        ...
        kdx = 0;
        foundVersion = False;
        while ((kdx) < (len(self._registeredVersions))):
            if (((self._registeredVersions)[kdx]) == ((node).version)):
                foundVersion = True
                break;
        ...
        if (not (foundVersion)):
            (self._registeredVersions).append((node).version);
        ...
        if ((len((self)._waiting)) > (0)):
            ...
            while ((jdx) < (len(waiting))):
                ...
                if (mdk_util.versionMatch((req).version, (node).version)):
                    if (not (((((req).factory).promise).value()).hasValue())):
        ...
        while ((idx) < (len(self.nodes))):
            if (((((self.nodes)[idx]).address) == ((node).address)) or ((((self.nodes)[idx]).getId()) == ((node).id))):
                (self.nodes)[idx] = (node);
                return
    ...
    def _getField(self, name):
        ...
        if ((name) == (u"_registeredVersions")):
            return (self)._registeredVersions
    ...
    def _setField(self, name, value):
        ...
        if ((name) == (u"_registeredVersions")):
            (self)._registeredVersions = _cast(value, lambda: _List)
...
class Node(_QObject):
    ...
    def getId(self):
        ...
        return _cast((self.properties).get(u"datawire_nodeId"), lambda: unicode)
...
class Discovery(_QObject):
    ...
    def _getServices(self, environment):
        ...
        """
        if (not (((environment).name) in (self.services))):
            (self.services)[(environment).name] = ({});

        return (self.services).get((environment).name)
    ...
    def resolve(self, service, version, environment):
        ...
        if (not ((cluster).matchingVersionRegistered(version))):
            fallback = (environment).getFallback();
            if ((fallback) != (None)):
                fallbackCluster = self._getCluster(service, fallback);
                if (not ((fallbackCluster).matchingVersionRegistered(version))):
    ...
    def onMessage(self, origin, message):
        if ((self._notificationCallback) != (None)):
            (self._notificationCallback)(message) if callable(self._notificationCallback) else (self._notificationCallback).call(message);
    ...
    def _getField(self, name):
        ...
        if ((name) == (u"_notificationCallback")):
    ...
    def _setField(self, name, value):
        ...
        if ((name) == (u"_notificationCallback")):
            (self)._notificationCallback = _cast(value, lambda: quark.UnaryCallable)
