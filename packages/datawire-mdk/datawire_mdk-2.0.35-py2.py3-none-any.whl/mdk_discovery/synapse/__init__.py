...
class Synapse(_QObject):
    ...
    def __init__(self, directory_path, environment):
        self._init()
        (self)._directory_path = directory_path
        (self)._environment = environment

    def create(self, subscriber, runtime):
        return _SynapseSource(subscriber, (self)._directory_path, runtime, (self)._environment)
...
class _SynapseSource(_QObject):
    ...
    def __init__(self, subscriber, directory_path, runtime, environment):
        self._init()
    ...
    def _update(self, service, nodes):
        ...
        """
        ((self).dispatcher).tell(self, mdk_discovery.ReplaceCluster(service, (self).environment, nodes), (self).subscriber);
    ...
    def onMessage(self, origin, message):
        ...
        if ((typeId) == (u"mdk_runtime.files.FileContents")):
            ...
            while ((idx) < ((json).size())):
                ...
                (node).id = (quark.concurrent.Context.runtime()).uuid()
