...
def createLogEvent(ctx, procUUID, level, category, text):
    """
...
class FakeTracer(_QObject):
    ...
    def log(self, event):
        (self.messages).append({u"level": (event).level, u"category": (event).category, u"text": (event).text, u"context": ((event).context).traceId});
...
class Tracer(_QObject):
    ...
    def log(self, event):
        ...
        """
        (self.logger).trace((u"CTX ") + (((event).context).toString()));
        (self._client).log(event);
