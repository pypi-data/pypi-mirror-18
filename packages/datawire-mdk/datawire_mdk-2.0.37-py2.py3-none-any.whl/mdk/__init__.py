...
def _parseEnvironment(environment):
    """
    Convert 'name' or 'fallback:name' into an Environment.
    """
    name = environment;
    fallback = _cast(None, lambda: unicode);
    if (((environment).find(u":")) != (-(1))):
        fallback = ((environment).split(u":"))[0]
        name = ((environment).split(u":"))[1]

    result = mdk_protocol.OperationalEnvironment();
    (result).name = name
...
class MDKImpl(_QObject):
    ...
    def __init__(self, runtime):
        ...
        self._environment = _parseEnvironment((((runtime).getEnvVarsService()).var(u"MDK_ENVIRONMENT")).orElseGet(u"sandbox"))
        ...
        if ((self._wsclient) != (None)):
            self._openclose = mdk_protocol.OpenCloseSubscriber(self._wsclient, self.procUUID, self._environment)
    ...
    def getDiscoveryFactory(self, env):
        ...
        if ((config).startswith(u"datawire:")):
            ...
            if ((config).startswith(u"synapse:path=")):
                result = mdk_discovery.synapse.Synapse((config)[(13):(len(config))], (self)._environment)
            else:
    ...
    def session(self):
        session = SessionImpl(self, None, (self)._environment);
    ...
    def join(self, encodedContext):
        session = SessionImpl(self, encodedContext, (self)._environment);
        if ((self._defaultTimeout) != (None)):
            (session).setDeadline(self._defaultTimeout);
    ...
    def _setField(self, name, value):
        ...
        if ((name) == (u"_environment")):
            (self)._environment = _cast(value, lambda: mdk_protocol.OperationalEnvironment)
...
class LoggedMessageId(_QObject):
    """
    ...
    def _init(self):
        self.traceId = None
        self.causalLevel = None
        self.environment = None
        self.environmentFallback = None

    def __init__(self, traceId, causalLevel, environment, environmentFallback):
        self._init()
    ...
    def _getClass(self):
        return u"mdk.LoggedMessageId"

    def _getField(self, name):
        if ((name) == (u"traceId")):
            return (self).traceId

        if ((name) == (u"causalLevel")):
            return (self).causalLevel

        if ((name) == (u"environment")):
            return (self).environment

        if ((name) == (u"environmentFallback")):
    ...
    def _setField(self, name, value):
        ...
        if ((name) == (u"causalLevel")):
            (self).causalLevel = _cast(value, lambda: _List)

        if ((name) == (u"environment")):
            (self).environment = _cast(value, lambda: unicode)

        if ((name) == (u"environmentFallback")):
            (self).environmentFallback = _cast(value, lambda: unicode)


LoggedMessageId.mdk_LoggedMessageId_ref = None
LoggedMessageId.quark_List_quark_int__ref = None
class SessionImpl(_QObject):
    ...
    def __init__(self, mdk, encodedContext, localEnvironment):
        self._init()
        ...
        if (((encodedContext) == (None)) or ((encodedContext) == (u""))):
            ...
            (self._context).environment = localEnvironment
        else:
    ...
    def _log(self, level, category, text):
        if ((SessionImpl._inLogging).getValue()):
            return _cast(None, lambda: LoggedMessageId)

        (SessionImpl._inLogging).setValue(True);
        evt = mdk_tracing.createLogEvent(self._context, (self._mdk).procUUID, level, category, text);
        if ((((self._mdk)._tracer) != (None)) and (self._enabled(level))):
            ((self._mdk)._tracer).log(evt);

        (SessionImpl._inLogging).setValue(False);
        return LoggedMessageId((self._context).traceId, (((evt).context).clock).clocks, ((self._context).environment).name, ((self._context).environment).fallbackName)

    def critical(self, category, text):
        return self._log(u"CRITICAL", category, text)

    def error(self, category, text):
        return self._log(u"ERROR", category, text)

    def warn(self, category, text):
        return self._log(u"WARN", category, text)

    def info(self, category, text):
        return self._log(u"INFO", category, text)

    def debug(self, category, text):
        return self._log(u"DEBUG", category, text)
    ...
    def _resolve(self, service, version):
        ...
        return (((self._mdk)._disco).resolve(service, version, (self).getEnvironment())).andThen(quark._BoundMethod(self, u"_resolvedCallback", _List([])))
    ...
    def start_interaction(self):
        ...
        lmid = self.info(u"MDK", u"Starting interaction.");
    ...
    def _start_interaction(self):
        ...
        (interactionReport).startTimestamp = int(round((1000.0) * ((((self._mdk)._runtime).getTimeService()).time())))
        (interactionReport).session = (self._context).traceId
        (interactionReport).environment = (self._context).environment
    ...
    def fail_interaction(self, message):
        ...
        text = u"no dependent services involved";
        if ((len(involved)) > (0)):
            text = (u"involved: ") + ((u", ").join(involved))

        (self).error(u"interaction failure", ((text) + (u"\n\n")) + (message));
    ...
    def finish_interaction(self):
        ...
        (report).endTimestamp = int(round((1000.0) * ((((self._mdk)._runtime).getTimeService()).time())))
        lmid = self.info(u"MDK", u"Finished interaction.");
