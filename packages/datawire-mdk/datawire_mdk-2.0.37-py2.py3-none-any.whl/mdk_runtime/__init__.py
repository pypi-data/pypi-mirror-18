...
class QuarkRuntimeWSActor(_QObject):
    ...
    def onMessage(self, origin, message):
        ...
        messageId = (quark.reflect.Class.get(_getClass(message))).id;
        if (((messageId) == (u"quark.String")) and (((self).state) == (u"CONNECTED"))):
        ...
        if (((messageId) == (u"mdk_runtime.WSClose")) and (((self).state) == (u"CONNECTED"))):
            self.logTS(u"   close-ish, switching to DISCONNECTING state");
            (self).state = u"DISCONNECTING"
            ((self).socket).close();
            return

        if (((messageId) == (u"mdk_runtime.WSClose")) and (((self).state) == (u"CONNECTING"))):
            (self).state = u"DISCONNECTING"
    ...
    def onWSConnected(self, socket):
        ...
        if (((self).state) != (u"CONNECTING")):
            self.logTS(((u"Connection event when transitioned out of CONNECTING.") + (u"Current state: ")) + ((self).state));
            (socket).close();
            return
...
def defaultRuntime():
    ...
    """
    runtime = MDKRuntime();
    (runtime).dispatcher = mdk_runtime.actors.MessageDispatcher(mdk_runtime.actors._QuarkRuntimeLaterCaller())
...
def fakeRuntime():
    ...
    (runtime).dispatcher = mdk_runtime.actors.MessageDispatcher(mdk_runtime.actors._ManualLaterCaller())
