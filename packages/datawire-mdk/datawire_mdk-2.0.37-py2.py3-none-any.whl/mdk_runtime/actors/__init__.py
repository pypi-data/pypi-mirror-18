...
_CallLater.mdk_runtime_actors__CallLater_ref = None
class _QuarkRuntimeLaterCaller(_QObject):
    def _init(self):
        pass
    def __init__(self): self._init()

    def schedule(self, t):
        (quark.concurrent.Context.runtime()).schedule(t, 0.0);

    def runAll(self):
        pass

    def _getClass(self):
        return u"mdk_runtime.actors._QuarkRuntimeLaterCaller"
    ...
    def _setField(self, name, value):
        pass
_QuarkRuntimeLaterCaller.mdk_runtime_actors__QuarkRuntimeLaterCaller_ref = None
class _ManualLaterCaller(_QObject):
    def _init(self):
        self.tasks = _List([])
    ...
    def _getClass(self):
        return u"mdk_runtime.actors._ManualLaterCaller"
...
_ManualLaterCaller.quark_List_quark_Task__ref = None
_ManualLaterCaller.mdk_runtime_actors__ManualLaterCaller_ref = None
class MessageDispatcher(_QObject):
    ...
    def onExecute(self, runtime):
        ...
        while ((len((self)._queued)) > (0)):
            ...
            while ((idx) < (len(toDeliver))):
                ...
                success = _cast((quark.concurrent.Context.runtime()).callSafely(deliver, False), lambda: bool);
                if (not (success)):
                    (self.logger).warn((u"FAILURE when delivering ") + (_toString((toDeliver)[idx])));
    ...
    def _setField(self, name, value):
        if ((name) == (u"callLater")):
            (self).callLater = _cast(value, lambda: _CallLater)
