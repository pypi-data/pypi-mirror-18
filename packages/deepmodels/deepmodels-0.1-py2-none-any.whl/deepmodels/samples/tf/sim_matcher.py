...
class SimpleMatcher(base_model.BaseDeepModel):
  ...
  def train_model(self,
                  train_anchor_batch,
                  train_pos_batch, train_neg_batch,
                  model_params, train_params):
    # get embedding for all batches.
    all_batch = tf.concat(0, [train_anchor_batch, train_pos_batch, train_neg_batch])
    ...
    # compute loss.
    triplet_loss = dm_losses.triplet_loss(anchor_feats, pos_feats, neg_feats,
                                          0.2, loss_type=commons.LossType.TRIPLET_L2)
