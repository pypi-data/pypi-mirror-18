...
class MyImgClassifer(DMClassifier):
  ...
  def build_model(self, inputs, model_params):
    ...
    """
    net, endpoints = model_zoo.build_net(self.train_net_type, inputs,
  ...
  def simple_ft_model(self, inputs, model_params):
    ...
    with tf.variable_scope("simple_ft"):
      with slim.arg_scope([slim.fully_connected],
        ...
        net = slim.fully_connected(inputs, model_params.cls_num, activation_fn=None, scope="logits")
    return net
  ...
  def simple_finetune(self, train_img_batch, train_label_batch,
    ...
    clf_loss = slim.losses.softmax_cross_entropy(pred_logits, labels,
                                                 scope="clf_loss")
    tf.scalar_summary("losses/clf_loss", clf_loss)
    model_fn = model_zoo.get_default_net_weights_fn(self.train_net_type)
    init_fn = slim.assign_from_checkpoint_fn(
        model_fn, self.vars_to_restore)
    base_model.print_variable_names(self.vars_to_train, "to train")
    base_model.train_model_given_loss(slim.losses.get_total_loss(),
                                      self.vars_to_train,
                                      train_params, init_fn=init_fn)
  ...
  def test_simple_finetune(self, test_img_batch, test_label_batch,
    ...
    metric_args = {
        "eval/mean_clf_accuracy": slim.metrics.streaming_accuracy(
            pred_cls, test_label_batch)
    }
...
def main(_):
  ...
  if FLAGS.task in {0, 1, 2, 3}:
    ...
    batch_data = data_provider.clf_input_from_image_fns(FLAGS.input_meta, 3,
    ...
    if FLAGS.task in {0, 1}:
      train_test_params = commons.TrainTestParams(log_dir=FLAGS.log_dir,
      ...
      if FLAGS.task == 0:
        ...
        model_params.custom["train_scopes"] = ["InceptionV3/Logits", "InceptionV3/AuxLogits"]
        clf.train(img_batch, label_batch, model_params, train_test_params)
      else:
        if clf.train_net_type == commons.NetNames.INCEPTION_V3:
          model_params.custom["restore_scopes_exclude"] = ["InceptionV3/Logits", "InceptionV3/AuxLogits"]
          model_params.custom["train_scopes"] = ["InceptionV3/Logits", "InceptionV3/AuxLogits"]
      ...
      train_test_params = commons.TrainTestParams(log_dir=FLAGS.log_dir, batch_size=FLAGS.batch_size,
                                                  samp_num=samp_num, max_epochs=FLAGS.test_ratio)
      train_test_params.save_summaries_secs = 60
