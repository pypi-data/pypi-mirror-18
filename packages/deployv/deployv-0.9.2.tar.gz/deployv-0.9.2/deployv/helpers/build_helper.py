...
class BuildImage(object):
    ...
    def __init__(self, config, version, working_folder='/tmp',
        ...
        if full_stack_image:
            self.base_image_name = utils.clean_string(
                self._config.get('container_config').get('image_name'))
        elif self._config.get('container_config').get('base_image_name'):
            self.base_image_name = utils.clean_string(
                self._config.get('container_config').get('base_image_name'))
        else:
            self.base_image_name = utils.clean_string(
    ...
    def _create_working_folder(self):
        ...
        try:
            shutil.copytree(
                ...
                utils.encode(path.join(self.working_folder, 'files')))
        except OSError:
            _logger.error('The specified working folder already exists and it is not empty,'
                          ' use --force to remove it')
            raise
        instance_path = path.join(self.working_folder, 'files', 'instance')
    ...
    def _clone_dependencies(self, instance_path, extra_path):
        ...
        """
        utils.clone_repo('https://github.com/Vauxoo/odoo.git',
                         self.version,
    ...
    def build(self):
        ...
        """
        res = {}
        _logger.info('Building instance for: %s', self._config.get('instance').get('customer_id'))
        try:
