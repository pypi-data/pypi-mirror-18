...
class SqllogBase(db.Model):
    ...
    def __init__(self, action, target):
        ...
        self.operator_id = cls.operator_id_handle() if callable(cls.operator_id_handle) else 0
        self.operator_role = cls.operator_role_handle() if callable(cls.operator_role_handle) else ""
        self.operator_name = cls.operator_name_handle(self.operator_id, self.operator_role) \
            if callable(cls.operator_name_handle) else "System"
        ...
        if action == "update":
            ...
            if field_finds:
                ...
                for k, v in log.changes.items():
                    kstr = tpl.replace("{key}", k).replace("{before}", json.dumps(v[0])).replace("{after}", json.dumps(v[1]))
                    chstr.append(kstr)
    ...
    @classmethod
    def operator_id_handle(cls):
        return operator_id_handle()

    @classmethod
    def operator_role_handle(cls):
        return operator_role_handle()

    @classmethod
    def operator_name_handle(cls, optid, optrole):
        return "System"

    # @hybrid_property
    # def changes(self):
    #     data = json.loads(self.__diff_data)
    #     ds = list()
    #     if isinstance(data, dict):
    #         for k, v in data.items():
    #             ds.append(dict(key=k, before=v[0], after=v[1]))
    #     return ds
