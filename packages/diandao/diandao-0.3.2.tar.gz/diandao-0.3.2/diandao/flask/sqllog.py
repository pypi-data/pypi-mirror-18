...
from diandao.flask import operator_id as operator_id_handle, operator_role as operator_role_handle, app
...
class SqllogBase(db.Model):
        ...
        if action == "update":
            ...
            if field_finds:
                ...
                for k, v in log.changes.items():
                    kstr = tpl.replace("{key}", k).replace("{before}", json.dumps(v[0])).replace("{after}",
                                                                                                 json.dumps(v[1]))
                    chstr.append(kstr)
    ...
    def operator_handle(cls, target, action, **kwargs):
        return (operator_id_handle(), operator_role_handle(), "System")
