...
class PronooChannel(object):
    ...
    def needRefresh(self):
        if self.age()>=self._refreshRate:
            if time.time()>=self._timeoutRetry:
                return True
    ...
    # TODO
    def set(self, value, quality, stamp=0):
        if not stamp:
            stamp=time.time()
...
class Pronoo(object):
    ...
    def query(self, items=None):
        request=[]
        if items:
            if not isinstance(items, list):
                items=[items]
            for item in items:
                request.append({self.identifierType(item): item})
            data={'request': json.dumps(request)}

            data=self.post('Query', data)
            payload=PronooPayload(data)
