...
class PronooChannel(object):
    ...
    def getPendingPublishData(self, clear=True):
        if self._pendingPublishData:
            self._timeoutPublish=time.time()+10.0
            data=self._pendingPublishData
            if clear:
                self._pendingPublishData=None
            return data
        ...
        try:
            if channels:
                ...
                if not isinstance(channels, list):
                    channels=[channels]
                for channel in channels:
                    data={'data': channel.getPendingPublishData(False)}
                ...
                if r and r.status_code==requests.codes.ok:
                    for channel in channels:
                        # clear request
                        channel.getPendingPublishData(True)
