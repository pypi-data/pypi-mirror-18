...
class PronooChannel(object):
    ...
    def manager(self):
        ...
        if self._valid and time.time()>=self._timeoutWatchdog:
            self.logging.error('channel(%s) Watchdong!' % self.name)
            self._valid=False
    ...
    def clearPendingPublishData(self):
        self.logger.debug('channel(%s):clearPendingPublishData' % self.name)
        self._pendingPublishData=None
...
class Pronoo(object):
                    ...
                    if payload.isType('data'):
                        for data in payload.content:
                            try:
                                ...
                                channel.load(pdata)
                                print pdata.data
                                print "VALUE", channel.value, channel.pdata.quality.isGood()
                                print "-"*60
                            except:
    ...
    def registerChannel(self, channel):
        self.logger.debug('registering channel(%s)' % channel.name)
        try:
