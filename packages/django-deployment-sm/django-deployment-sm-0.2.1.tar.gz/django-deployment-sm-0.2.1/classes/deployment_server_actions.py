...
class DeploymentServerActions(object):
    ...
    def _execute_manage_py_command(self, to_deploy_commit_hash, manage_py_command):
        return sudo(
            '{python_virtualenv} {manage_py_full_path} {manage_py_command}'.format(
                python_virtualenv=self.python_path,
                manage_py_full_path=self.manage_py_full_path(to_deploy_commit_hash),
    ...
    def _set_symbolic_link(self, source_path, destination_path):
        return sudo(
            'ln -s {source_path} {destination_path}'.format(
    ...
    def update_virtualenv(self):
        return self.execute_with_host(self._update_virtualenv)

    def _update_virtualenv(self):
        return sudo('{pip} install -r {requirements_txt}'.format(
            pip=self.pip_path,
            requirements_txt=os.path.join(self.deployed_path, self.config_item.requirements_txt)
        ))
    ...
    def get_current_revision(self):
        ...
        return return_value.replace('/', '') if return_value else return_value
