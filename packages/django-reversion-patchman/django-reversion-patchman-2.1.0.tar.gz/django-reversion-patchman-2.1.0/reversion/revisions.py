...
def _push_frame(manage_manually, using, comment=""):
    if is_active():
...
def add_single_revision(obj, using=None, delete=False):
    _push_frame(True, using)
    add_to_revision(obj, using, delete)
    try:
...
def _add_to_revision(obj, using, model_db, explicit, delete=False):
    ...
    version = Version(
        ...
        serialized_data = "" if delete else serializers.serialize(
    ...
    if version_options.ignore_duplicates and explicit and not delete:
...
@contextmanager
def _create_revision_context(manage_manually, using, comment=""):
    _push_frame(manage_manually, using, comment)
    try:
...
def create_revision(manage_manually=False, using=None, comment=""):
    from reversion.models import Revision
    using = using or router.db_for_write(Revision)
    return _ContextWrapper(_create_revision_context, (manage_manually, using, comment))
...
def _post_delete_receiver(sender, instance, using, **kwargs):
    if is_registered(sender) and is_active() and not is_manage_manually():
        add_to_revision(instance, model_db=using, delete=True)
