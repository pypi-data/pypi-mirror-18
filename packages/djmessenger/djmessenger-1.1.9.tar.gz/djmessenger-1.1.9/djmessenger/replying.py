...
class CommonReplier(Serializable, Loggable):
        ...
        try:
            ...
            try:
                ...
                if status.status_code == 200:
                    ...
                    self.logger.debug(
                        'Sent message %s failed with status code %d'
                        'and failure details %s' %
                        (data, status.status_code, status.content))
                reply.save()
            except Exception as e:
                self.logger.exception(
                    'Failed to create Sending object because %s' % e)
        except FBUserProfile.DoesNotExist:
            self.logger.debug(
                'Recipient PSID not found in database table djm_user,'
    ...
    def preprocess_outgoing_string(self, psid, localized_text):
        ...
        try:
            ...
            self.logger.exception(
                'Failed to process outgoing string for %s because'
                ' %s' % (localized_text, e))
            text = localized_text.base_string
        return text
