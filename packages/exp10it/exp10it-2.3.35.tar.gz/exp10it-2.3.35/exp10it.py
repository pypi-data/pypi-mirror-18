...
tar -jxvf phantomjs-2.1.1-linux-x86_64.tar.bz2 && cd phantomjs-2.1.1-linux-x86_64 && cp bin/phantomjs /usr/bin")
        ...
        try:
            driver.get(url)
            import random
            driver.get_screenshot_as_file("/tmp/PhantomJSPic"+get_http_domain_from_url(url).split("/")[-1]+str(random.random()))
            code = None
            title = driver.title
...
scan_finished varchar(50) not null default 0)" % sub_table_name
                ...
                if eval(get_key_value_from_config_file('config.ini', 'default', 'scan_way')) in [1, 2, 3, 4]:
                    execute_sql_in_db(sql_pang, eval(
                        get_key_value_from_config_file('config.ini', 'default', 'db_name')))
...
def crawl_scan(target):
    ...
    if eval(get_key_value_from_config_file('config.ini', 'default', 'scan_way')) == 1:
        ...
        result = execute_sql_in_db(
            sql, eval(get_key_value_from_config_file('config.ini', 'default', 'db_name')))
        if len(result) > 0:
            for each in result:
                if each[0] != "":
...
def get_sqlmap_result_and_save_result(url):
    ...
    if len(sql_result1) > 0:
        ...
        tmp_table_name = eval(get_key_value_from_config_file(
            'config.ini', 'default', 'first_targets_table_name'))
    else:
...
ruby-dev build-essential libgmp-dev zlib1g-dev")
    ...
    for each_table in table_name_list:
        set_scan_finished("cms_scaned", eval(get_key_value_from_config_file(
            'config.ini', 'default', 'db_name')), each_table, target)
