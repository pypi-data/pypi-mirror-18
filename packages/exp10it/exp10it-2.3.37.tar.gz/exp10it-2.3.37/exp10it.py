...
def mail_msg_to(msg, mailto, subject, user, password, format='plain'):
    ...
    if mailto is None:
        ...
        if re.search(r"mailto",content):
            ...
            update_config_file_key_value("config.ini",'mail','mailto',mailto)
        if re.search(r"user",content):
            ...
            update_config_file_key_value("config.ini",'mail','user',user)
        if re.search(r"password",content):
            ...
            update_config_file_key_value("config.ini",'mail','password',password)
    import smtplib
...
def bing_search(query, search_type):
    ...
    if re.search(r"bingApiKey",content):
        key=eval(get_key_value_from_config_file("config.ini",'default','bingApiKey'))
    else:
        print("please input your bing api key:")
        key=input()
        update_config_file_key_value("config.ini",'default','bingApiKey',"'"+key+"'")
