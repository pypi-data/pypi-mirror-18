...
def database_init():
    ...
    config_file_abs_path = os.getcwd() + "/config.ini"
    ...
    if re.search(r"db_server",content):
        ...
        while 1:
            ...
            if check_string_is_ip(db_server) is True:
                update_config_file_key_value(config_file_abs_path,'default','db_server',"'"+db_server+"'")
                break
            else:
...
default[y]:>", end=' ')
    ...
    if re.search(r"targets_table_name",content):
        targets_table_name=eval(get_key_value_from_config_file(config_file_abs_path,'default','targets_table_name'))
    else:
...
table name,n|N to input your own table name as targets' table name. default[y]:>", end=' ')
        ...
        update_config_file_key_value(config_file_abs_path,'default','targets_table_name',"'"+targets_table_name+"'")
    print("targets_table_name:" + targets_table_name)

    # 优先级更高的目标表
    if re.search(r"first_targets_table_name",content):
        first_targets_table_name=eval(get_key_value_from_config_file(config_file_abs_path,'default','first_targets_table_name'))
    else:
...
and their info:>", end=' ')
        ...
        update_config_file_key_value(config_file_abs_path,'default','first_targets_table_name',"'"+first_targets_table_name+"'")
...
def exp10itScanner():
    ...
    database_init()
    while 1:
        target = get_one_target_from_db(eval(get_key_value_from_config_file("config.ini",'default','db_name')),
                eval(get_key_value_from_config_file("config.ini",'default','first_targets_table_name')))
        if target == None:
            target = get_one_target_from_db(eval(get_key_value_from_config_file("config.ini",'default','db_name')),
                    eval(get_key_value_from_config_file("config.ini",'default','targets_table_name')))
