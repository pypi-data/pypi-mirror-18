...
tar -jxvf phantomjs-2.1.1-linux-x86_64.tar.bz2 && cd phantomjs-2.1.1-linux-x86_64 && cp bin/phantomjs /usr/bin")
        ...
        try:
            ...
            driver.get_screenshot_as_file(
                "/tmp/PhantomJSPic" + get_http_domain_from_url(url).split("/")[-1] + str(random.random()))
            code = None
            title = driver.title
...
def get_user_and_pass_form_from_html(html):
    ...
    form_action_url_pattern = re.compile(r'''form action=('|")?([^'" ]{,20})('|")?''', re.I)
...
def crack_admin_login_url(
    ...
    def crack_admin_login_url_inside_func(url, username, pass_dict_file):
        ...
        if has_yanzhengma[0] == True:
            # 如果有验证码开一个线程
            max_workers_num = 1
        else:
            max_workers_num = 20
        with futures.ThreadPoolExecutor(max_workers=max_workers_num) as executor:
...
class MyScanner(object):
    ...
    def __init__(self, target, single_xxx_scan):
        ...
        http_domain_xxx_scaned = get_scan_finished(
            xxxValue + "_scaned", eval(get_key_value_from_config_file('config.ini', 'default', 'db_name')), main_target_table_name, target)
        ...
        if http_domain_xxx_scaned == 1 and pang_domains_xxx_scaned == 1 and sub_domains_xxx_scaned == 1:
            ...
            if eval(get_key_value_from_config_file('config.ini', 'default', 'scan_way')) == 1:
                ...
                if len(result) > 0:
                    for each in result:
                        if each[0] != "":
                            xxx_scaned = get_scan_finished(xxxValue + "_scaned", eval(get_key_value_from_config_file('config.ini', 'default', 'db_name')), target.split(
                                "/")[-1].replace(".", "_") + "_pang", each[0])
                            if xxx_scaned == 0:
                                single_xxx_scan(each[0])
                                set_scan_finished(xxxValue + "_scaned", eval(get_key_value_from_config_file('config.ini', 'default', 'db_name')), target.split(
                                    "/")[-1].replace(".", "_") + "_pang", each[0])
                ...
                if len(result) > 0:
                    for each in result:
                        if each[0] != "":
                            pang_list.append(each[0])
                            xxx_scaned = get_scan_finished("script_type_scaned", eval(get_key_value_from_config_file('config.ini', 'default', 'db_name')), target.split(
                                "/")[-1].replace(".", "_") + "_pang", each[0])
                            if xxx_scaned == 0:
                                single_xxx_scan(each[0])
                                set_scan_finished(xxxValue + "_scaned", eval(get_key_value_from_config_file('config.ini', 'default', 'db_name')), target.split(
...
def auto_attack(target):
    ...
    input("stop here cause this need a input and i finished single_crack_admin_page_scan function")
