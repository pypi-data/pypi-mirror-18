...
def execute_sql_in_db(sql, db_name="mysql"):
    ...
    a=os.path.abspath(__file__).split("/")[:-1]
    config_ini_file_abs_path="/".join(a)+"/config.ini"
    try:
        import MySQLdb
    except:
    ...
    try:
        ...
        conn = MySQLdb.connect(
            eval(get_key_value_from_config_file(config_ini_file_abs_path, 'default', 'db_server')),
            eval(get_key_value_from_config_file(config_ini_file_abs_path, 'default', 'db_user')),
            eval(get_key_value_from_config_file(config_ini_file_abs_path, 'default', 'db_pass')),
            db=db_name,
            port=3306,
            charset="utf8")
        conn.autocommit(1)
