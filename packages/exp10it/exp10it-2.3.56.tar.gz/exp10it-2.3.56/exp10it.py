...
tar -jxvf phantomjs-2.1.1-linux-x86_64.tar.bz2 && cd phantomjs-2.1.1-linux-x86_64 && cp bin/phantomjs /usr/bin")
        ...
        if proxy!="":
            ...
            service_args_value=['--ignore-ssl-errors=true', '--ssl-protocol=any','--proxy=%s' % proxyValueWithType,'--proxy-type=%s' % proxyType]
...
def get_user_and_pass_form_from_html(html):
    ...
    form_action_url_pattern = re.compile(r'''form.*action=('|")?([^'" ]{,20})('|")?.*>[\s\S]{,1000}(<input .*type=('|")?password.*>)''', re.I)
