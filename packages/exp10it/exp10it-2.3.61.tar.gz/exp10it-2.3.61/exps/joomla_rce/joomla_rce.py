...
if target[:4]!="http":
    ...
    return_string=""
    command="cd hackUtils && python hackUtils.py -r %s > hackUtils_log_tmp_file" % target
    print(command)
    os.system(command)
    with open("hackUtils/hackUtils_log_tmp_file","r+") as f:
        output1=f.read()
    ...
    command="cd joomla_exp && python joomla_exp.py %s > joomla_exp_log_tmp_file" % (target+"/")
    print(command)
    os.system(command)
    with open("joomla_exp/joomla_exp_log_tmp_file","r+") as f:
        output2=f.read()
