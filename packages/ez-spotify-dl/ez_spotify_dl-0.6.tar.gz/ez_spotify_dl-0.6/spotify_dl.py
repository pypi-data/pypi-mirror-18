...
import os, requests, shelve, hashlib, json, subprocess, shutil, argparse, urllib
...
class Spotify(object):
    ...
    def playlist_tracks(self, playlist):
        url = playlist.get('tracks', {}).get('href') if isinstance(playlist, dict) \
            else api + '/users/%s/playlists/%s/tracks' % (self.__user_id, playlist)
    ...
    for playlist in spotify.playlists().get('items'):
        ...
        if playlists == 'ALL' or playlist.get('name') in playlists:
            ...
            for track_item in tracks.get('items', []):
                ...
                download(folder, normalize(playlist.get('name')), music_identifier)
...
def download(dest_folder, playlist_name, music_identifier):

    playlist_folder = os.path.join(dest_folder, playlist_name)
    if not os.path.exists(playlist_folder):
        os.mkdir(playlist_folder)

    destfile = os.path.join(playlist_folder, '%s.mp3' % music_identifier)

    store = Store('downloaded')
    key = hashlib.md5(music_identifier.encode('u8')).hexdigest()
...
def callback():
    ...
    <div style="padding: 20px; margin: 0 auto; display: table; border-radius: 5px; background: #eee;">
        ...
        <p style="color: #444; font-family: Verdana;">For more information execute ./spotify_dl.py -h</p>
    </div>
    '''
...
def main():
    ...
    parser.add_argument('-d', '--download-folder', help='Absolute folder path to where you want saving the downloaded musics.')
    parser.add_argument('--playlists', help='Playlists (separated by comma) what you wanna to download. DEFAULT: ALL', default='ALL')
    ...
    if args.authorize:
        ...
        if user_id:
            print('\n')
            print('##########################################################')
            print('## Open you web browser and go to http://localhost:5000 ##')
            print('##########################################################')
            print('\n')
            app.run()
    else:
        ...
        if not folder:
            folder = raw_input('Insert the absolut path where you want to save the downloaded playlists: ')
            folder = folder.strip()

        playlists = args.playlists if args.playlists == 'ALL' else args.playlists.split(',')

        if its_ok(creds.check):
            download_tracks(folder, playlists)
