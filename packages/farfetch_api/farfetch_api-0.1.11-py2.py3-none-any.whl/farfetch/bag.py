...
class BagApi(object):
    ...
    def get_bag(self, id):
        url = self.base_url + "bags/%s" % id
        contents = self.get(url)
        return contents

    def item_bag(self, id, item_id):
        url = self.base_url + "bags/%s/items/%s" % (id, item_id)
        contents = self.get(url)
        return contents

    def add_item_bag(self, id, item):
        url = self.base_url + "bags/%s/items" % id
        contents = self.post(url, data_j = item)
        return contents

    def update_item_bag(self, id, item_id, item_update):
        url = self.base_url + "bags/%s/items/%s" % (id, item_id)
        contents = self.patch(url, data_j = item_update)
        return contents

    def remove_item_bag(self, id, item_id):
        url = self.base_url + "bags/%s/items/%s" % (id, item_id)
        contents = self.delete(url)
        return contents

    def merge_bags(self, id, id_source):
        url = self.base_url + "bags/%s/merge/%s" % (id, id_source)
        contents = self.post(url)
        return contents

    def create_checkout_order_bag(self, id):
        url = self.base_url + "bags/%s/checkoutOrders" % id
        contents = self.post(url)
        return contents
