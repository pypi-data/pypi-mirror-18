...
class NodeService( AbstractService ):
    ...
    def list_nodes(self, retries=0):
        ...
        ----------
        :type retries: int
        :param retries: When this service is used as SDK, the client has the option to
            retry the node listing until a PM shows up in the list.
        '''
        ...
        if isinstance(j_nodes, FdsError):
            ...
            if j_nodes.error.status_code == requests.codes.service_unavailable:
                # On any service start, there is a delay where the service tries to
                # dynamically find the OM. As a result of that delay, the node list
                # might not include that service. If there are no PM services found by
                # the /nodes service, the /nodes service returns a 503 to us. We should
                # retry for a while to see if a PM shows up.
                ...
                while count < retries:
                    ...
                    time.sleep(9)
                    j_nodes = self.rest_helper.get( self.session, url )
                    if not isinstance(j_nodes, FdsError):
                        break;
                    elif j_nodes.error.status_code != requests.codes.service_unavailable:
                        break;
            else:
                return j_nodes
