...
class ZonePlugin( AbstractPlugin):
    ...
    def create_create_command(self, subparser):
        ...
        __create_parser.add_argument( "-" + AbstractPlugin.zone_iface, help="Interface name on which to create the VIP",required=False)
    ...
    def validate_zone_input(self, zone, zone_service_ids):
        ...
        if zone.iface is not None:
            try:
                addr = netifaces.ifaddresses(zone.iface)
            except ValueError:
                print "Error: The interface name specified is invalid"
                return False
            if netifaces.AF_INET not in addr:
                print "Error: The interface specified: " + zone.iface + " is not up."
                return False
