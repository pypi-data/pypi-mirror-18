...
from pylib.security import tokens
...
def cookie_remover(key, secure=False):
    """
    Creates a pro hook for removin a cookie using the given key
    from the dict.
    @type Hook => (Request -> Response -> Resource -> _)
    @sig cookie_fixer :: Str -> Hook
    """
    def hook(req, resp, res):
        resp.set_cookie(key, 'disabled', expires=tokens.preset_day(1), secure=secure)
    return hook
