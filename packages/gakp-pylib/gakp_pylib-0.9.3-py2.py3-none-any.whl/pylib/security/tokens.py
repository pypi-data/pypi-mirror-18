...
def preset_hour(hours):
    """
    Calculates the datetime `hours` before now
    @sig preset_hour :: Int -> Date
    """
    return datetime2.utcnow() - datetime.timedelta(hours=hours)
...
def preset_day(days):
    """
    Calculates the date `days` before now.
    @sig preset_day :: Int -> Date
    """
    return datetime2.utcnow() - datetime.timedelta(days=days)
...
def stamp(ttl, days=True):
    ...
    @sig stamp :: Int -> Bool -> {Str: datetime}
    """
