...
# get log file
set logfile = /root/LOGS/mcast-log
...
#
if ($db != "uploaded_db") set db = /root/db/fasta_databases/$db
...
# run mcast 
#
echo "<ul><li>/root/bin/mcast $bfile $* $motifs $db </li></ul><hr>" >> index.html
#
eval "/root/bin/mcast $bfile $* $motifs $db >mcast.html"
