#
# ImageViewAggGtk.py -- classes for the display of FITS files in Gtk surfaces
...
class ImageViewAggGtk(ImageViewAgg):
    ...
    def redraw(self, whence=0):
        if not self.defer_redraw:
            super(ImageViewAggGtk, self).redraw(whence=whence)
            return
...
class ImageViewEvent(ImageViewAggGtk):

    def __init__(self, logger=None, rgbmap=None, settings=None):
        ImageViewAggGtk.__init__(self, logger=logger, rgbmap=rgbmap,
                                 settings=settings)
