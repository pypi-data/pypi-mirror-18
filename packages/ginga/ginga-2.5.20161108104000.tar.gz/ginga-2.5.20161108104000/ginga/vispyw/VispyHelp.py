#
# VispyHelp.py -- help classes for the PIL drawing
#
...
def get_vispy_colormap(rgbmap):
    ...
    # create a clone of this ColorDist object, customized to a hash size
    # equivalent to the color map size
    cdist = rgbmap.get_dist()
    cdst = cdist.__class__(clen, colorlen=clen)
    cdst.calc_hash()
    controls = cdst.hash_array(np.arange(clen)) / float(clen - 1)
    # vispy will complain if this is not strictly met
    controls[0], controls[-1] = 0.0, 1.0

    colors = np.asarray(clst)[rgbmap.get_sarr()]

    assert len(colors) == len(controls), \
           ValueError("lengths of controls (%d) and colors (%d) not consistent" % (
        len(controls), len(colors)))

    cmap = Colormap(colors, controls=controls, interpolation='linear')
    return cmap
