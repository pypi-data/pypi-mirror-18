...
from ginga.gtkw import GtkMain
...
def fv_start(args, logfile=None, loglevel=20, logstderr=False):
...
def main(options, args):
    ...
    fv_start(args, logfile=options.logfile,
...
def fv(logfile=None, loglevel=20, logstderr=False):
    logger = logging.getLogger("example2")
    logger.setLevel(loglevel)
    fmt = logging.Formatter(STD_FORMAT)
    if logfile:
    ...
    mn = GtkMain.GtkMain(logger=logger)
    t = threading.Thread(target=mn.mainloop, args=[])
    t.start()
