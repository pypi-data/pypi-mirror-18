...
@click.option('--account_id', default='',
...
def cli(folder, vault_name, mode, account_id, skip, pattern):
    """Runs the CLI for the glacier sync tool"""

    start = timer()

    sync = Glacync(folder, vault_name, mode, account_id, skip, pattern)

    # Start sync run
    logging.info("Starting sync run from '%s' to vault '%s'...", folder, vault_name)
