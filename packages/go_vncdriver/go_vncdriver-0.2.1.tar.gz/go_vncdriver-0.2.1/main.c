...
go_vncdriver_VNCSession_init(go_vncdriver_VNCSession_object *self, PyObject *args, PyObject *kwds)
{
    ...
    int compress_level;
    static char *kwlist[] = {"remotes", "error_callable", "compress_level", NULL};

    error_callable = &_Py_NoneStruct;
    compress_level = 0;
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "O|OI", kwlist, &remotes, &error_callable, &compress_level))
        return -1;
    ...
    if ((args = Py_BuildValue("(OOI)", remotes, error_callable, compress_level)) == NULL) {
        return -1;
    }
