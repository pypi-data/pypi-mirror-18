...
func (c *VNCSession) Flip() (*Screen, []*vncclient.FramebufferUpdateMessage) {
	...
	if c.pauseUpdates {
		...
		c.requestUpdate()
		// Block until we get our next framebuffer update
		log.Debugf("[%s] resuming updates; blocking until next update received", c.address)
		c.updated.Wait()
		log.Debugf("[%s] next update received; proceeding with flip", c.address)
	}
	...
	for {
		...
		case update := <-updates:
			...
			if len(c.deferredUpdates) >= c.deferredUpdatesMax && !c.pauseUpdates {
				log.Debugf("[%s] update queue max of %d reached; pausing further updates", c.address, c.deferredUpdatesMax)
				c.pauseUpdates = true
			}
...
func (c *VNCSession) requestUpdate() {
	c.conn.FramebufferUpdateRequest(true, 0, 0, c.conn.FramebufferWidth, c.conn.FramebufferHeight)
}
