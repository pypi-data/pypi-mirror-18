...
FindAuth:
	...
	if auth == nil {
		if securityType == 0 {
			return fmt.Errorf("server returned an invalid security type: %#v. That means it likely rejected our connection.", securityType)
		} else {
...
func (c *ClientConn) handshake() (error, bool) {
	...
	if maxMajor < 3 {
		return fmt.Errorf("unsupported major version, less than 3: %d", maxMajor), false
	}
	if maxMinor < 3 {
		return fmt.Errorf("unsupported minor version, less than 3: %d", maxMinor), false
	}
	...
	if securityResult == 1 {
		return fmt.Errorf("security handshake failed: %s", c.readErrorReason()), false
	}
