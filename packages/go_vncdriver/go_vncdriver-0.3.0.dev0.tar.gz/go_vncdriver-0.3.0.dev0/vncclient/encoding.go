...
func (t *TightEncoding) Read(c *ClientConn, rect *Rectangle, r io.Reader) (Encoding, error) {
	...
	case 9:
		...
		t.buf.Reset()
		if err = t.readToBuf(r, length); err != nil {
			return nil, err
		}
		img, err := jpeg.Decode(t.buf)
		if err != nil {
			return nil, err
		}
		t.size += length
		...
		for y := 0; y < int(rect.Height); y++ {
			for x := 0; x < int(rect.Width); x++ {
				...
				colors[i].R = uint8(r >> 8)
				colors[i].G = uint8(g >> 8)
				colors[i].B = uint8(b >> 8)
				i++
			}
		}
		return &TightEncoding{Colors: colors, size: t.size}, nil
...
func (t *TightEncoding) readTPixels(r io.Reader, n int) ([]Color, error) {
	...
	t.size += n * 3
	c, err := (&QuickBuf{buf: t.buf.Next(n * 3)}).ReadColors(n)
	return append([]Color{}, c...), err
}
