...
static int PyArg_ParseTuple_connect(PyObject *args, PyObject *kwds, char **name, char **address, char **password, char **encoding, int *compress_level, int *fine_quality_level, int *subsample_level, unsigned long *start_timeout, PyObject **subscription) {
    static char *kwlist[] = {"name", "address", "password", "encoding", "compress_level", "fine_quality_level", "subsample_level", "start_timeout", "subscription", NULL};
    return PyArg_ParseTupleAndKeywords(args, kwds, "ss|ssiiikO", kwlist, name, address, password, encoding, compress_level, fine_quality_level, subsample_level, start_timeout, subscription);
}
...
func GoVNCDriver_VNCSession_connect(self, args, kwds *C.PyObject) *C.PyObject {
	...
	if C.PyArg_ParseTuple_connect(args, kwds, nameC, addressC, passwordC, encodingC, compressLevelC, fineQualityLevelC, subsampleLevelC, startTimeoutC, subscriptionPy) == 0 {
		return nil
	}
