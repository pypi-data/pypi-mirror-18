...
func NewVNCSession(name string, c VNCSessionConfig) *VNCSession {
	...
	if c.FineQualityLevel == 0 {
		// If someone forgets to set FineQualityLevel, just do 100
...
func (c *VNCSession) connect(updates chan *vncclient.FramebufferUpdateMessage) error {
	...
	err = conn.SetEncodings([]vncclient.Encoding{
		encoding,

		// Pseudo-encodings!
		vncclient.CompressLevel(c.config.CompressLevel),
		vncclient.FineQualityLevel(c.config.FineQualityLevel),
		vncclient.SubsampleLevel(c.config.SubsampleLevel),
	})
	if err != nil {
