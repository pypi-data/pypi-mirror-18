...
class Utils:
    ...
    @staticmethod
    def get_additionalImages():
        """ get additional images url"""

        images_url = HttpUtils.http_get_request(
            "/api/1.0/image",
            Auth.server,
            {'Authorization':'Bearer '+Auth.token},
            Auth.noCert
        ...
        if regex is not None:
            ...
            for job in tasks:
                try:
                    ...
                    ET.SubElement(single_job, "user").text = job['user']['id']
                    ET.SubElement(single_job, "container").text = job['container']['image']
                    if 'status' in job['container']:
                        ET.SubElement(single_job, "event").text = str(job['status']['secondary'] + ' ' + str(job['container']['status']))
                    else:
                        ET.SubElement(single_job, "event").text = str(job['status']['secondary'])
                    ET.SubElement(single_job, "status").text = exitCode
                    ET.SubElement(single_job, "ram").text = str(job['requirements']['ram'])
