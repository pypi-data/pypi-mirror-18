...
{content}
"""
DETAILED = """ conversation id: {conversation_id}
{repeat_block}
{fave_block}
Posted from: {source}
URL: {url}"""
...
class Status:
    ...
    def __format_repeat_block(self):
        if self.repeat_num == 0:
            return 'Has not been repeated'
        else:
            if self.repeated:
                you = 'has been repeated by you'
            else:
                you = 'has not been repeated by you'
            return 'Has been repeated by {} people ({})'.format(self.repeat_num, you)

    def __format_fave_block(self):
        if self.fave_num == 0:
            return 'Has not been favorited'
        else:
            if self.favorited:
                you = 'has been favorited by you'
            else:
                you = 'has not been favorited by you'
            return 'Has been favorited by {} people ({})'.format(self.fave_num, you)
