...
class ClientTcpCommunication(object):
    ...
    def __init__(self):
        ...
        self.__encryption_type = hz_encryption.EncryptionType.AesCbcPkcs7
        ...
        self.__Version = "1.0.0.0"     #SDKバージョン
                ...
                try:
                    ...
                    try:
                        if(self.__svr_sock is not None):
                            ...
                            self.__HzTrace.trace_comm_message("[CL]Received a data from the connection source.: Data Size[{d}]".format(rcvLen))
                            if (rcvLen <= 0):
                                #print("受信データを取得できないため、接続元から切断します。\r\n受信データサイズ[{d}]".format(rcvLen))
                                print("Disconnect from the connection source, because it can't  to receive any data.\r\nData Size[{d}]".format(rcvLen))
                        ...
                        #タイムアウト時はWait
                        if("end of file" in ex.args != True):
    ...
    def __reqStartCommProc(self, name = "", passwd = "", strEncKey = ""):
        ...
        try:
            ...
            if (strEncKey.strip() != "" and reqStartComm.cer_flg == reqStartComm.CER_FLG_ON):
                ...
                #Debug
                #decPasswd = self.__encryption.cipherDecryption(self.__encryption_type.AesCbcPkcs7, reqStartComm.passwd.decode("utf-8"), strEncKey) 
            ...
            th = threading.Thread(target=self.__wait_worker_ack_start_com)
            th.start()
            th.join(self.__ReceiveAckTimeOut)
            if(th.is_alive()):
                self.__ackStartCommAutoResetEvent.set()
    ...
    def __sendSpeechData(self):
        ...
        try:
            ...
            th = threading.Thread(target=self.__wait_worker_ack_send_speech_data)
            th.start()
            th.join(self.__ReceiveAckTimeOut)
            if(th.is_alive()):
                self.__ackSendSpeechDataAutoResetEvent.set()
    ...
    def sendSpeechDataEx(self):
        ...
        try:
            ...
            self.__HzTrace.trace_comm_message("{s}".format(ex.args)) 
            timeStamp = None
        finally:
            return timeStamp
