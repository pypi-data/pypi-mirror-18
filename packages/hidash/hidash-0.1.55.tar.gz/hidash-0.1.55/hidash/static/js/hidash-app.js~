...
hidashApp.controller('ReportsController',function($scope, $http, $window, $location, utils){
    ...
    $http.get('/hidash-api/get_group_filters.json/?group=' + $scope.group).success(function(filters) {
      ...
      for (var i = 0; i < $scope.filters.length; i++) {
        if($scope.filters[i].type === 'datetime') {
          ...
          for (var l = 0; l < tempOptions.length; l++) {
            if (tempOptions[l].active) {
              selectedDate = $scope.filters[i].selectedValue = tempOptions[l].range
              break;
            }
          }
          ...
          tempDefault["periodicDatePicker"] = selectedDate;
      ...
      $scope.$watch('parameters', function() {
        ...
        for (var  i= 0;i<$scope.parameters.length;i++) {
          if($scope.filters[i].type === 'datetime') {
            ...
            urlParamsObj["periodicDatePicker"] = $scope.filters[i].selectedValue;
          } else {
  ...
  $scope.getDateFromName = function (data, index) {
    console.log(index);
    var results = utils.getDateFromNameDate(data),
    tempDefault = {}
    ...
    $scope.filters[index].selectedValue = data;
        ...
        "<div ng-repeat='filter in filters'>"+
          "<div class='date'>"+
            '<div ng-if="filter.type===\'datetime\'">'+
              ...
              '<hi-dash-named-date-filters options="filter.filter_value" filterId="{{filter.type}}" get-date-from-name="getDateFromName(selectedValue, $index)" selected="filter.selectedValue" gridwidth="{{filter.grid_width}}" ng-model="parameters[$index].periodicDatePicker"/>'+
              '</div>'+
          "</div>"+
        "</div>"+
      "</div>"+
...
.service('utils', function() {
    var getDateFromNameDate = function(dateName) {
            let dateOptions = ['Today', 'Yesterday', 'Tomorrow', 'This Week', 'This Month', 'This Year', 'Last Week', 'Last Month', 'Last Year']
        let dateOptionsObj = {}
