...
def split_imports(s):
    ...
    imps = import_list.split(',')
    return sorted(["import " + i.strip() for i in imps if i.strip()], key=lambda s: s.lower())
...
class ReadInput():
    ...
    def store_line(self, target_map, lines):
    ...
    def process_line(self, l):
        if does_line_end_in_noqa(l):
            ...
            self.store_line(self.pre_from_import, split_from_imports(l))
        else:
            self.lines_before_import.append(l)
