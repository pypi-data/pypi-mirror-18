...
#define HALF_PIXEL ((float)(0.5))
#define DSA_ORDER  ((float)(3))  // # by default we correct for 1/cos(2th), fit2d corrects for 1/cos^3(2th)
#define R2D        ((float)(180 / M_PI))
#define M_PI2      ((float)(M_PI * M_PI))
...
static void calc_sincos(geometry *geo, geotri *geo3) {
    ...
    geo3->tth2q = (float)(4e-9 * M_PI) / geo->wavelength;
}
...
static void bbox_positions(integration *intg, geometry *geo) {
    int i, j;
    float max0, min0, delta, start, stop, maxdtth, maxdchi;
...
static void _calculate_positions(integration *intg, geometry *geo) {
    int i, j;
    ...
    for (i=0; i<=intg->pos->dim1; i++) {
        ...
        for (j=0; j<=intg->pos->dim2; j++) {
            ...
            if (geo->units == UNITS_Q)
                _dtth = geo3.tth2q * sinf(_dtth / 2);
            *dtth++ = _dtth;
            *dchi++ = atan2f(dt1, dt2);
            if (i != intg->pos->dim1 && j != intg->pos->dim2) {
                _tth = atan2f(sqrtf(t1 * t1 + t2 * t2), t3);
                if (geo->units == UNITS_Q)
                    _tth = geo3.tth2q * sinf(_tth / 2);
                *tth++ = _tth;
...
integration *calculate_positions(int dim1, int dim2, geometry *geo) {
    ...
    if (intg == NULL)
        return NULL;
    _calculate_positions(intg, geo);
    calc_corners(intg);
    bbox_positions(intg, geo);
    destroy_aux(intg);
    return intg;
}
