...
bbox_results *bbox_integrate(integration *intg, float *image, float azmin, float azmax) {
    ...
    float deltaA, deltaL, deltaR, fbin0_min, fbin0_max, max0, min0, *count, *sum;
    bbox_results *res;

    res = init_results(intg);
    count = calloc(res->bins * 2, sizeof(float));
    if (count == NULL || res == NULL) {
        destroy_results(res);
        return NULL;
    }
    ...
    for (j=0; j<intg->pos->bins; j++)
        if (count[j] > 0) {
            res->merge[j] = sum[j] / count[j];
            res->sigma[j] = sqrtf(sum[j]) / count[j];
        }
    free(count);
    return res;
}
