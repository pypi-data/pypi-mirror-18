...
class Resolver(object):
    ...
    def query(self, query, timeout=None):
        ...
        match = self._matcher.match(query.name.name)
        if not match:
            log.debug('Received incompatible query for %s' % query.name.name)
            return defer.fail(error.DomainError())
        ...
        try:
            ...
            log.failure("Failure in serving address for query %s" % query.name.name)
            return defer.fail(error.DomainError())
