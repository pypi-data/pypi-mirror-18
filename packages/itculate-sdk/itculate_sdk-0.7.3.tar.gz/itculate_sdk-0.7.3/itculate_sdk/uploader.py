...
class TimeseriesPayload(Payload):
    ...
    def __init__(self, samples, dictionary):
        # collector_id is meaningless with with timeseries reports (except for logging). Hard code to 'sdk'
        super(TimeseriesPayload, self).__init__("sdk", samples=samples, dictionary=dictionary)
...
class Mappings(PayloadProvider):
    def __init__(self):
        self.mappings = defaultdict(list)  # type: dict[tuple, list]
        self.mappings_changed = False

    def __str__(self):
        return self.__class__.__name__

    def map_counter_to_many(self, non_vertex_keys, vertex_counters):
        """
        ...
        :param collections.Iterable[Vertex|str, str, str, DataType] vertex_counters: Iterable over counter identifiers
        """
        ...
        for vertex, counter, vertex_type, data_type in vertex_counters:
            if isinstance(vertex, Vertex):
            ...
            vertex_counter_identifiers.append((vertex_key, vertex_type, counter,))

            self.mappings[non_vertex_keys] = vertex_counter_identifiers
    ...
    def flush(self):
        ...
        return MappingsPayload(collector_id="sdk",  # Collector id is not critical with timeseries
...
class AgentForwarder(Provider):
    ...
    def __init__(self, settings):
        ...
        self.server_url = settings.get("server_url", "http://127.0.0.1:8000")
    ...
    def handle_payload(self, payload):
        ...
        r = self.session.post("{}/{}".format(self.server_url, payload.agent_api), json=data_to_upload)
        r.raise_for_status()
