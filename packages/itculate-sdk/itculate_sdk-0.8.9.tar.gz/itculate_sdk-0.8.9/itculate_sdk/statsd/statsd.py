...
class ITculateStatsdClient(object):
    ...
    def _send_metric(self, metric, value, sample_rate=None, vertex=None, data_type=None):
        ...
        try:
            ...
            statsd_packet = self.encode_itculate_packet("{}:{}".format(metric, value),
                                                        vertex=vertex,
                                                        data_type=data_type)
    ...
    @staticmethod
    def encode_itculate_packet(statsd_packet, vertex=None, data_type=None):
        if vertex is None:
            return statsd_packet
        ...
        return "_it.%s.%s.%s.%s" % (len(vertex), data_type_name, vertex, statsd_packet)

    @staticmethod
    def decode_itculate_packet(encoded_packet):
        if not encoded_packet.startswith("_it."):
            # This is not an encoded vertex
            return None, None, encoded_packet

        else:
            start = 4
            end = encoded_packet.find(".", start)
            vertex_key_length = int(encoded_packet[start:end])

            start = end + 1
            end = encoded_packet.find(".", start)
            data_type = encoded_packet[start:end]

            start = end + 1
            end = start + vertex_key_length
            vertex_key = encoded_packet[start:end]

            start = end + 1
            metric_name = encoded_packet[start:]
