...
class KaptlUpdate:
    ...
    def get_version_to_update(self):
        ...
        available_versions = self.common_methods.get_versions(session=self.session, license=self.license, recipe=self.recipe, verify_request=self.verify_requests)
    ...
    def update_project(self):
        ...
        session_name = self.common_methods.parse_rules(
            self.session, self.rules, self.stack, self.kaptl_cookie,
            self.license, self.recipe, self.app_name, recipe_version=version_to_update, verify_request=self.verify_requests)
        if session_name is not None:
            self.file_info = self.common_methods.get_file_info(self.session, session_name, self.rules, self.stack,
                                                               ...
                                                               self.license, self.app_name, verify_request=self.verify_requests)
