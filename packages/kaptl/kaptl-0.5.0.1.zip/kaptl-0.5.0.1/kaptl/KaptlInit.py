...
class KaptlInit:
    ...
    def initialize_project(self):
        self.session_name = self.common_methods.parse_rules(self.session, self.rules, self.stack, None, self.licenseKey, self.recipe, self.app_name, recipe_version=self.recipe_version, verify_request=self.verify_requests)
        if self.session_name is not None:
            self.file_info = self.common_methods.get_file_info(self.session, self.session_name, self.rules, self.stack, self.recipe, "full", self.angular_only, False, self.licenseKey, self.app_name, verify_request=self.verify_requests)
