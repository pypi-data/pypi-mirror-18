...
class KMSurvival():
    ...
    H, S, T = 'hazard', 'survival', '_tenure_'
    ...
    def _check_data(self, X, group):
        """Verify the validity of data given"""
        assert isinstance(X, pd.DataFrame), "Data should be pandas DataFrame"
        assert isinstance(group, list), "Group should be a list"
        assert set(X.columns.values) >= set(group), \
                "Group name should be in columns of data"
        assert X[self.col_censored].value_counts().size == 2, \
                "Column '{}' should have two unique values" \
                .format(self.col_censored)
        return X

    def fit(self, X, cutoff=None, group=[]):
        ...
        group : list of strings, default empty list
            Column names in X for cross comparisons
        """
        X = self._check_data(X, group)
        if cutoff:
