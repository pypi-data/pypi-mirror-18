...
def make_user_secure_token(user_id, expires_in=None, **kw):
    """
    ...
    if not secret_key:
        raise exceptions.AuthError("Missing config SECRET_KEY")
...
def get_user_from_secure_token(token, **kw):
    """
    Return AuthUser of the token bearer
    :param token:
    :param kw:
    :return: AuthUser
    """
    id = get_user_id_from_secure_token(token, **kw)
    return get_user(id)


def get_user_id_from_secure_token(token, **kw):
    """
    Returns the user id from the secure token
    :param token:
    :param kw:
    :return: int
    """
    ...
    try:
        ...
        logging.warning(b)
        raise exceptions.SecureLoginError()
    return None
...
def login_with_email(email, password, require_verified_email=False, session=True):
    ...
    """
    def cb():
        userl = models.AuthUserLogin.get_by_email(email.strip())
        if userl and userl.password_hash \
                and userl.password_matched(password.strip()):
