...
class Model(QtCore.QThread):
    ...
    def addRefinementStar(self, ra, dec, refractionTemp):                                                                   # add refinement star during model run
        ...
        self.commandQueue.put('Sr{0:02d}:{1:02d}:{2:02d}'.format(h, m, s))                                                # Write jnow ra to mount
        print(h,m,s,'Sr{0:02d}:{1:02d}:{2:02d}'.format(h, m, s))
        h, m, s, sign = self.mount.decimalToDegree(self.mount.transform.DecTopocentric)                                     # convert to Jnow
        self.commandQueue.put('Sd{0}{1:02d}*{2:02d}:{3:02d}'.format(sign, h, m, s))                                       # Write jnow dec to mount
        print(sign, h, m, s, 'Sd{0}{1:02d}*{2:02d}:{3:02d}'.format(sign, h, m, s))
