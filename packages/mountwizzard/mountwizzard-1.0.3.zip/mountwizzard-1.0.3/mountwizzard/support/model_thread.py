...
class Model(QtCore.QThread):
    ...
    def prepareCaptureImageSubframes(self, scale):
    ...
    def runModel(self, modeltype, runPoints):                                                                               # model run routing
        ...
        self.numCheckPoints = 0                                                                                             # number og checkpoints done
        self.results = []                                                                                                   # error results
        if self.ui.checkDoSubframe.isChecked():
            scaleSubframe = self.ui.scaleSubframe.value() / 100
            self.sub, self.sizeX, self.sizeY, self.offX, self.offY = self.prepareCaptureImageSubframes(scaleSubframe)
        else:
            self.sub = False
            self.sizeX = 0
            self.sizeY = 0
            self.offX = 0
            self.offY = 0
        self.logger.debug('runModel-> subframe: {0}, {1}, {2}, {3}, {4}'.format(self.sub, self.sizeX, self.sizeY, self.offX, self.offY))
