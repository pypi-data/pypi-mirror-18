...
class ReliablePy(object):
    ...
    def saveDataAsPar( self, outputPrefix, N_classes = 1, particleKey = "data_" ):
        """
        ...
        uutputPrefix will be appended with "_1_rX.par", where X is the class number.
        ...
        """
        print( "WARNING: As of Relion-2.0 magnification no longer stored, set rln.star[b'data_'][b'Magnification'] = mag*np.ones_like(...)" )
        ...
        for K in np.arange( 1, N_classes+1 ):
            ...
            with open( outputName, 'w' ) as fh:
                ...
                for J in np.arange(partCount):
                    fh.write( "%7d"%(J+1) + " %8.2f"%self.star[b'data_'][b'AnglePsi'][J] 
                                + " %8.2f"%self.star[b'data_'][b'AngleTilt'][J]  
                                + " %8.2f"%self.star[b'data_'][b'AngleRot'][J] 
                                + " %8.2f"%self.star[b'data_'][b'OriginY'][J] 
                                + " %8.2f"%self.star[b'data_'][b'OriginX'][J]
        ...
        #   particleList.extend( mrcz.readMRC(uniqueName)[0] )
        ...
        #mrcz.writeMRC( particleArray, mrcName, shape=None ) # TODO: no pixelsize
        pass
    ...
    def __loadMRC( self, mrcname ):
        mrcimage, mrcheader  = mrcz.readMRC( mrcname, pixelunits=u'nm' )
