...
class MalibuService(object):
    ...
    def create_workflow_item(self, process_name,
        ...
        message, workflow_id, status = MalibuService.handle_response(response_code,
                                                                     response.content)

        return response_code, message, workflow_id
    @retry_policy()
    def cancel_workflow_item(self, workflow_id):
        ...
        response = requests.delete(url,
                                   headers=headers)
        response_code = response.status_code
        data = MalibuService.handle_response(response_code,
                                             response.content)
        #return response code and message
        return response_code, data[0]
    @retry_policy()
    ...
    def get_workflow_status(self, workflow_id):
        ...
        message, workflow_id, workflow_status = MalibuService.handle_response(response_code,
                                                                              response.text)

        return response_code, workflow_status, message
