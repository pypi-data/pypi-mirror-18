...
class WorkflowExecutor(object):
    ...
    def display_status(self, workflow_id, status_code, message=None, response_code=None):
        """ Displays status of workflow """
        display_message = "NA"
        if message is not None:
            display_message = message
        elif status_code in self.exit_status_by_workflow_status:
            display_message = self.exit_status_by_workflow_status[status_code][0]
        ...
        if response_code is not None:
            msg = msg + "Response Code: {0}".format(response_code)
    ...
    def workflow_wall_clock_endtime(cls, end_date, created_date_object):
        ...
        seconds_offset = time_delta.days * 86400
        return_end_date = ""
        ...
        base_seconds = time_delta.seconds
        base_seconds += seconds_offset
    ...
    def post_workflow_blocking(self):
        ...
        try:
            response_code, message, workflow_id = self.service.create_workflow_item(
            ...
            display_msg = "Successfully submitted"
            if message != "" and message is not None:
                display_msg = message

            self.display_status(workflow_id, None, display_msg, response_code)
            self.args_output.workflow_id = workflow_id
            return workflow_id
    ...
    def poll_workflow_status_blocking(self, workflow_id=0, success_status=20000):
        ...
        try:
            while poll:
                response_code, status_code, message = self.service.get_workflow_status(workflow_id)
                self.display_status(workflow_id, status_code, message)
                ...
                if success:
                    if self.download_output:
                        self.display_status(workflow_id, 0, "Downloading results")
    ...
    def cancel_workflow(self):
        ...
        response_code = 0
        try:
            response_code, message = self.service.cancel_workflow_item(workflow_id)
            self.set_exit_code(None, response_code)
            response_code, status_code, message = self.service.get_workflow_status(workflow_id)
            self.display_status(workflow_id, status_code, message)
            self.set_exit_code(None, response_code, status_code)
    ...
    def get_input_and_output_dict(self):
        ...
        self.add_key_value(input_dic,
                           ...
                           self.datatransfer.create_input_blob_sas_url(
                               self.args_output.input_blob_name_1),
                           True)
        self.add_key_value(input_dic,
                           "BLOBNAMES_WITH_SAS",
                           self.datatransfer.create_input_blob_sas_url(
                               self.args_output.input_blob_name_2),
                           True)
        output_dic = dict()
        ...
        self.add_key_value(output_dic, "CONTAINER_SAS",
                           self.datatransfer.create_output_sas(), False)
