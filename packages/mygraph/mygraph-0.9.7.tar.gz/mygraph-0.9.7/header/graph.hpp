...
#ifndef GRAPH_H
#define GRAPH_H
constexpr auto MAX_OUT_DEGREE = 4;
constexpr auto MAX_IN_DEGREE = 4;
...
namespace GraphException
{
    ...
    {
    public:
        virtual const char* what() const throw() { return "GRAPH_ERROR: OD pair not accessible"; }
    };

    class GraphNotSet : public std::exception
    {
    public:
        virtual const char* what() const throw() { return "GRAPH_ERROR: Graph not yet set"; }
    };
...
struct Edge {
    Edge(const std::string &_id, Vertex* _fv, Vertex* _tv) {
        id = _id;
        idx = 0;
        from_vertex = _fv;
        to_vertex = _tv;
...
private:
    ...
    Vertex** vertices;
    Edge** edges;
    std::unordered_map<std::string, int> vid_to_idx;
    std::unordered_map<std::string, int> eid_to_idx;
    int m_cnt;
    int n_cnt;
public:
    Graph(int n, int m) {
    ...
    void add_vertex(const std::string &_id) {
    ...
    void add_edge(const std::string &_id, Vertex* _fv, Vertex* _tv) {
    ...
    void add_edge(const std::string &_id, const std::string &_fv_id, const std::string &_tv_id) {
        add_vertex(_fv_id);
        add_vertex(_tv_id);
    ...
    // get vertex methods
    inline Vertex* get_vertex (const std::string &_id) const{
    ...
    // get edge methods
    inline Edge* get_edge(std::string _id) const{
    ...
    inline Edge* get_edge(std::string _fid, std::string _tid) const{
