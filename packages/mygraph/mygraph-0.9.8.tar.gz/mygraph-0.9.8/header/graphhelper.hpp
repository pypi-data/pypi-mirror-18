...
#ifndef MyGraph_graphhelper_h
#define MyGraph_graphhelper_h
...
void graph_matsim2csv(string _input, string _output)
{
    ...
    boost::property_tree::ptree *pt_network = new boost::property_tree::ptree();
    boost::property_tree::xml_parser::read_xml(_input, *pt_network);
...
const std::unordered_map<string, std::unordered_map<int, float> > get_profile(vector<int> & time_second_keys)
{
    ...
    string line("");
    int cnt = 1;
    std::unordered_map<string, std::unordered_map<int, float>> profile; // profile[long linkid][time_of_day]
    while (ins) {
