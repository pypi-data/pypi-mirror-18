...
set<string> Hyperpath_TD::get_turn_restrictions(const string& csv_path) {
    ...
    else throw std::runtime_error("Unable to open turn restrictions file");
    return restrictions;
}
...
bp::list Hyperpath_TD::wrapper_get_path_rec(const string &_oid, const string &_did) const{
    ...
    while (vis->idx != d->idx) {
        ...
        if (p_i[vis->idx] == 0) {
            const string &s = "ERROR: hyperpath not set, please conduct Hyperpath::run(string _oid, string _did, float* weights_min, float* weights_max, const float* h) first";
