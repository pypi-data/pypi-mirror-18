...
class NoteBook_Operater:
        ...
        for i, el in enumerate(_cells):
            if delete1stCell:
             if el['execution_count'] == 0:   #删除生成脚本的第一行信息
                list.append(i)
            if not el.__contains__("outputs"):
                list.append(i)
            else:
                _outputs = el["outputs"]
                if (len(_outputs) == 0):
    ...
    def generate_script(self,filepath,_moudleName,_username):
        ...
        target_path = "%s/upload/%s/tar" % (currentDir,_username)
    ...
    def gen1stCellFromspecJson(self,_notebookName,_username):
        ...
        _scpecPath = "/root/work/zetdata/r-notebook/upload/%s/tar/spec.json" % _username
