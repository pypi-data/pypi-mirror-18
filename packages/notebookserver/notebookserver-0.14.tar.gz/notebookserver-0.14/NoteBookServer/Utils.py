...
class Util:
    ...
    @staticmethod
    def tarballFiles(_username):
        '''tar package'''
        import re
        fileroot = "%s/upload/%s/tar" % (curdir,_username)
        ...
        return "%s/upload/%s/tar/%s" % (GlobalValue.currentDir, _username, filename)
...
if __name__ == "__main__":
    cfg = SafeConfigParser()
    cfg.read("/home/lj/aaa.cfg")
    cfg.add("user_id","asd","asd")
    with open("/home/lj/aaa.cfg","w") as configFile:
