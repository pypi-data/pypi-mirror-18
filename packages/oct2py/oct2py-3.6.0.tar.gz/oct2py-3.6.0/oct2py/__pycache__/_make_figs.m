...
function _make_figs(plot_dir, plot_name, plot_format, plot_res)
% Get all the figures that are currently open.
figHandles = get(0, 'children');

for _fig=1:length(_figHandles),
    _handle = _figHandles(_fig);
    _filename = sprintf('%s/%s%03d.%s', plot_dir, plot_name, _fig + %s, plot_format);
    try
