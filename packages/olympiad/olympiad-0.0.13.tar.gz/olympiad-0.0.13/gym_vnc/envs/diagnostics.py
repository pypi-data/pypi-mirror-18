...
import re
from gym_vnc import error, pyprofile, spaces
...
class MetadataDecoder(object):
    @classmethod
    def build(cls, metadata_encoding, pool, qr_pool):
        metadata_encoding = metadata_encoding.copy()
        type = metadata_encoding.pop('type')
        if type == 'qrcode':
            return QRCodeMetadataDecoder(pool=pool, qr_pool=qr_pool, **metadata_encoding)
        else:
...
class AsyncDecode(object):
    ...
    def __call__(self, img):
        ...
        if len(self.deque) > 0 and self.deque[0].ready():
            ...
            res = last.get()
            if res is not None:
                pyprofile.timing('vnc_env.diagnostics.async_decode.latency', time.time() - res['processing_start'])
        else:
            res = False
        ...
        if not match:
            ...
            async = self.qr_pool.apply_async(self.method, (self._last_img, time.time()))
...
class QRCodeMetadataDecoder(MetadataDecoder):
    def __init__(self, pool, qr_pool, x, y, width, height):
        self.x = x
        self.y = y
        self.width = width
        self.height = height

        self.decode = AsyncDecode(pool, qr_pool, self.decode, x, y, width, height)

    def decode(self, observation, start):
...
class Diagnostics(object):
    def __init__(self, n, probe_key, ignore_clock_skew=False, metadata_encoding=None, use_multiple_senses=False, disable_action_probes=False):
        ...
        self.use_multiple_senses = use_multiple_senses
    ...
    # only used in vnc-flashgames right now
    ...
    def connect(self, i, network=None):
        # This should technically be synchronized
        self.instance_n[i] = DiagnosticsInstance(i, network, self.probe_key, self.ignore_clock_skew, self.metadata_encoding, disable_action_probes=self.disable_action_probes, qr_pool=self.qr_pool, pool=self.pool)
    ...
    def _add_metadata_i(self, args):
        instance, observation, info = args
        if self.use_multiple_senses and observation is not None:
...
class DiagnosticsInstance(object):
    ...
    def __init__(self, i, network, probe_key, ignore_clock_skew, metadata_encoding, disable_action_probes, pool, qr_pool):
        '''
    ...
    def update(self, probe_key, metadata_encoding):
        ...
        if metadata_encoding is not None:
            self.metadata_decoder = MetadataDecoder.build(metadata_encoding, pool=self.pool, qr_pool=self.qr_pool)
        else:
    ...
    def add_probe(self, action):
        ...
        if self.probe_sent_at is not None and self.probe_sent_at + 10 < time.time():
            logger.warn('[%s] Probe to determine action latency timed out (was sent %s). (This is harmless, but worth knowing about.)', self.i, self.probe_sent_at)
            self.probe_sent_at = None
        if self.probe_sent_at is None:
            logger.debug('[%d] Sending out new action probe: %s', self.i, self.probe)
    ...
    def add_metadata(self, observation, info):
        ...
        if metadata is False:
            ...
            if self.could_read_metadata:
                ...
                logger.info('[%s] Stopped being able to read metadata (expected when environment resets)', self.i)
        elif not self.could_read_metadata:
            self.could_read_metadata = True
            logger.info('[%s] Started being able to read metadata', self.i)

        local_now = time.time()

        if self.network is None:
            # Assume the clock skew is zero. Should only be run on the
            # same machine as the VNC server, such as the gym-vnc
        ...
        if metadata is not None:
            ...
            if self.last_observation_timestamp == metadata['now']:
                ...
                delta = observation_now - metadata['processing_start']

                # Subtract *local* time it was received from the *remote* time
                # displayed. Negate to fix time ordering.
                info['stats.gauges.diagnostics.lag.observation'] = -(delta + clock_skew)
            ...
            if probe_received_at == 0 or self.disable_action_probes:
                ...
                logger.info('[%d] Probe is marked as received at %s, but probe_sent_at is None. This is surprising. (HINT: do you have multiple gym-vnc instances talking to the same environment?)', self.i, probe_received_at)
        ...
        if remote_score_now is not None:
            ...
            info['stats.gauges.diagnostics.lag.reward'] = -(delta + clock_skew)
        ...
        if rewarder_message_now:
            ...
            info['stats.gauges.diagnostics.lag.rewarder_message'] = -(delta + clock_skew)

    def _should_search_metadata(self):
        time.time() - self.last_search_metadata > min(1.5**self.unsuccessful_search_count, 75)

    def _find_metadata_location(self, observation):
        """Non-mutating"""
        ys, xs = np.where(np.all(observation == self.anchor[0, 0], axis=-1))
        if len(ys) == 0:
            msg = '[%d] Could not find metadata anchor pixels' % self.i
            # Use the network as a heuristic for when to suppress these messages
            # If the network is none, we assume we're on the same machine as remotes
            # Otherwise we see a lot of these while waiting for an episode to start
            if self.network is None:
                logger.debug(msg)  # suppressed (slightly)
            else:
                # Used to display this for humans/demonstrators, but
                # no real reason right now.
                logger.debug(msg)
            return False

        # TODO: handle multiple hits
        if len(ys) != 1:
            # TODO: we could be more robust by just remembering the
            # candidates.
            logger.warn("Found multiple metadata anchor pixels: ys=%s xs=%s. Going to try again.", ys, xs)
            return False
        location = (ys[0], xs[0])
...
#         """TODO: This needs be moved to gym-vnc-envs and run there. Otherwise it only works if the env and agent
