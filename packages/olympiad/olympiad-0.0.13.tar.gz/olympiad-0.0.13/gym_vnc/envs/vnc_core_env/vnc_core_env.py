...
class VNCCoreEnv(vnc_env.VNCEnv):
    ...
    def _configure(self, reuse=True, *args, **kwargs):
        super(VNCCoreEnv, self)._configure(reuse=reuse, *args, **kwargs)

        if reuse:
            self.docker_command = ['run']
        else:
            # -o is the "once" flag, meaning that the env should exit
            # -upon client disconnect.
            self.docker_command = ['run', '-o']
