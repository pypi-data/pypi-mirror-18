...
def WrappedMultiprocessingEnv(env_id):
    return RemoveNones(EpisodeID(vectorized.MultiprocessingEnv(env_id)))

class RemoveNones(vectorized.Wrapper):
    """The vectorized environment will return None for any indexes that
    are going to be ignored. For convenience, we instead return a
    plausible observation in each such slot.
    """
