...
class SelectField(StringField):
    ...
    def validate(self, val):
        ...
        if val not in self.options:
            raise FieldError("Illegal value {0!s} for <{1!r}>. Valid options "
                             "are {2!s}".format(val, self, self.options))
...
class IntField(BaseField):
    ...
    def validate(self, val):
        ...
        if self.positive and val < 0:
            raise FieldError("Input for <{0!r}> is not "
                             "positive: {1:d}".format(self, val))
...
class FloatField(BaseField):
    ...
    def validate(self, val):
        ...
        if self.positive and val < 0.0:
            raise FieldError("Input for <{0!r}> is "
                             "not positive: {1:f}".format(self, val))
...
class HlocvCurveField(MutableField):
    def from_json(self, val):
        ...
        return HlocvCurve.create_raw([d for d, v in knts],
                                     [v for d, v in knts])
...
class StructureField(MutableField):
    ...
    def validate(self, val):
        ...
        if not all(isinstance(k, str) for k in val.keys()):
            FieldError("<{0!r}>, structure "
                       "keys need to be strings".format(self))
        # --- all values should be numbers
        if not all(isinstance(v, numbers.Number) for v in val.values()):
            FieldError("<{0!r}>, structure "
                       "values need to be numbers".format(self))
