...
class Structure(collections.UserDict):
    ...
    implements algebraic manipulations such as:
        ...
        - additiona and multiplication by a scalar
    """
    # -------------------------------------------------------------------------
    def __setitem__(self, key, value):
        try:
            ...
            raise StructureError("Incorrect Structure value for "
                                 "pair {0!s}: {1!s}".format(key, value))
    ...
    def __iadd__(self, other):
        if isinstance(other, collections.MutableMapping):
    ...
    def __mul__(self, other):
        struct = self.copy()
        if isinstance(other, collections.MutableMapping):
            raise StructureError("Multiplication of two "
    ...
    def drop_zeros(self, ndigits=None):
        """
        Drop items with zero value (up to a given number of digits).
        """
        if ndigits is None:
            new = {k: v for k, v in self.items() if v == 0.0}
        else:
            new = {k: v for k, v in self.items() if v == round(v, ndigits)}
