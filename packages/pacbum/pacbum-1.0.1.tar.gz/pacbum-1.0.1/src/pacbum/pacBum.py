...
def forward_colectScreen(self):
    ...
    if self.dead == False:
        animation_update(self, 'images/simple_pc_fw.png')
...
def backward_colectScreen(self):
    ...
    if self.dead == False:
        animation_update(self, 'images/simple_pc_bw.png')
        update_Phys_pacBum(self)
...
def forward_colectScreen2(self):

    if self.dead == False:
        animation_update(self, 'images/suit_pc_fw.png')
...
#Função e detecção de colisão do movimento do personagem principal na tela colectScreen
def backward_colectScreen2(self):

    if self.dead == False:
        animation_update(self, 'images/suit_pc_bw.png')
        if self.position[0] <= 200:
            self.position[0] = 201
