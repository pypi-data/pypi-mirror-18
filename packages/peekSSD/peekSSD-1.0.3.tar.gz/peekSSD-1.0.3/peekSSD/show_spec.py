...
def iops_4k_random_read_test( device ):
    ...
    cmd = "fio --name=riops_4k --filename=%s --numjobs=4 --bs=4k --ioengine=libaio --direct=1 --randrepeat=0 --norandommap --rw=randread --group_reporting --iodepth=512 --iodepth_batch=128 --iodepth_batch_complete=128 --gtod_reduce=1 --runtime=30 > iops_4k_RR.txt" %device
    logging.info( cmd )
...
def get_average_value( filename ):
    ...
    if filename in["iops_4k_RR.txt","iops_4k_RW.txt","iops_8k_RR.txt","iops_8k_RW.txt"]:
        with open(filename) as afile:
            value = afile.readlines()[6].split(',')[2].split('=')[1]
    if filename == "bandwidth_128k_SR.txt" or filename == "bandwidth_128k_SW.txt":
        with open(filename) as afile:
            value = afile.readlines()[6].split(',')[1].split('=')[1].strip("KB/s")
    if filename == "latency_4k_RR.txt" or filename == "latency_4k_RW.txt":
        with open(filename) as afile:
            value = afile.readlines()[8].split(',')[2].split('=')[1]
    if value[-1] == 'K' or value[-1:] == 'M':
...
def workout_values():
	...
	"bandwidth_128k_SR.txt","bandwidth_128k_SW.txt","latency_4k_RR.txt","latency_4k_RW.txt"):
        if filename == "iops_4k_RR.txt":
            line = "4K Random Read IOPS:\t%sK" %(get_average_value(filename)/1000)
            print line
            logging.info( line )
	if filename == "iops_4k_RW.txt":
            line = "4K Random Write IOPS:\t%sK" %(get_average_value(filename)/1000)
	    print line
	    logging.info( line )
	if filename == "iops_8k_RR.txt":
            line = "8K Random Read IOPS:\t%sK" %(get_average_value(filename)/1000)
            print line
	    logging.info( line )
	if filename == "iops_8k_RW.txt":
            line =  "8K Random Write IOPS:\t%sK" %(get_average_value(filename)/1000)
	    print line
	    logging.info( line )
	if filename == "bandwidth_128k_SR.txt":
	    line =  "128K Sequence Read:\t%.1f MB/s" %(get_average_value(filename)/1000)
	    print line
	    logging.info( line )
	if filename == "bandwidth_128k_SW.txt":
            line = "128K Sequence Write:\t%.1f MB/s" %(get_average_value(filename)/1000)
	    print line
	    logging.info( line )
	if filename == "latency_4k_RR.txt":
	    line = "4K Random Read Latency:\t%.2f us" %(get_average_value(filename))
	    print line
	    logging.info( line )
	if filename == "latency_4k_RW.txt":
	    line = "4K Random Write Latency:%.2f us" %(get_average_value(filename))
	    print line
	    logging.info( line )
    print "-------------------End-------------------------"
...
def main():
    ...
    workout_values()
    logging.info("Show Spec Test Done!")
