...
def process_gnuplot_charts( device ):
    ...
    process = subprocess.Popen( "gnuplot " + abs_path + "/gnuplot/gnuplot_4k_RR.conf", shell=True ).wait()
    ...
    subprocess.Popen( "gnuplot " + abs_path + "/gnuplot/gnuplot_4k_RW.conf", shell=True ).wait()

    cmd = "grep %s iostat_8k_RR.log | awk '{print $4}' | sed -n '900,4499p' | awk '{print(NR, $0)}' > iostat_8k_RR.txt" %dev
    logging.info( cmd )
    process = subprocess.Popen( cmd, shell=True ).wait()
    process = subprocess.Popen( "gnuplot " + abs_path + "/gunplot/gnuplot_8k_RR.conf", shell=True ).wait()
    #gnuplot_iops_8k_random_write
    cmd = "grep %s iostat_8k_RW.log | awk '{print $5}' | sed -n '2700,6299p' | awk '{print(NR, $0)}' > iostat_8k_RW.txt" %dev
    logging.info( cmd )
    subprocess.Popen( cmd, shell=True ).wait()
    subprocess.Popen( "gnuplot " + abs_path + "/gnuplot/gnuplot_8k_RW.conf", shell=True ).wait()

    cmd = "grep %s iostat_128k_SR.log | awk '{print $6}' | sed -n '900,4499p' | awk '{print(NR, $0)}' > iostat_128k_SR.txt" %dev
    logging.info( cmd )
    process = subprocess.Popen( cmd, shell=True ).wait()
    process = subprocess.Popen( "gnuplot " + abs_path + "/gunplot/gnuplot_128k_SR.conf", shell=True ).wait()
    ...
    logging.info( cmd )
    subprocess.Popen( cmd, shell=True ).wait()
    subprocess.Popen( "gnuplot " + abs_path + "/gnuplot/gnuplot_128k_SW.conf", shell=True ).wait()
    # latency
    ...
    subprocess.Popen( "gnuplot " + abs_path + "/gnuplot/gnuplot_4k_RR_latency.conf", shell=True ).wait()
    ...
    subprocess.Popen( "gnuplot " + abs_path + "/gnuplot/gnuplot_4k_RW_latency.conf", shell=True ).wait()
def workout_values():
