...
def iops_4k_random_read_test( device ):
    cmd = "sh " + abs_path + "/sh/iops_4k_RR.sh " + device
...
def iops_8k_random_read_test( device ):
    cmd = "sh " + abs_path + "/sh/iops_8k_RR.sh " + device
    process = subprocess.Popen( cmd, shell=True )
    i = 1
    while process.poll() == None:
        time.sleep(60)
...
def iops_4k_random_write_test( device ):
    cmd = "sh " + abs_path + "/sh/iops_4k_RW.sh " + device
    process = subprocess.Popen( cmd, shell=True )
    i = 1
    while process.poll() == None:
        time.sleep(60)
...
def iops_8k_random_write_test( device ):
    cmd = "sh " + abs_path + "/sh/iops_8k_RW.sh " + device
    process = subprocess.Popen( cmd, shell=True )
    i = 1
    while process.poll() == None:
        time.sleep(60)
...
def bandwidth_128k_sequence_read_test( device ):
    cmd = "sh " + abs_path + "/sh/bandwidth_128k_SR.sh "  + device
    process = subprocess.Popen( cmd, shell=True )
    i = 1
    while process.poll() == None:
        time.sleep(60)
...
def bandwidth_128k_sequence_write_test( device ):
    cmd = "sh " + abs_path + "/sh/bandwidth_128k_SW.sh " + device
    process = subprocess.Popen( cmd, shell=True )
    i = 1
    while process.poll() == None:
        time.sleep(60)
...
def latency_4k_random_read_test( device ):
     ...
     for i in range(151):
        cmd = "sh " + abs_path + "/sh/latency_4k_RR.sh " + device + " " + str(i)
	print cmd
        process = subprocess.Popen( cmd, shell=True )
        logging.info("testing IOPS 4KB randome read, running %d seconds" %(60*i))
        process.wait()
...
def latency_4k_random_write_test( device ):
    for i in range(211):         
        cmd = "sh " + abs_path + "/sh/latency_4k_RW.sh " + device + " " + str(i)    
	process = subprocess.Popen( cmd, shell=True )
        logging.info("testing IOPS 4KB randome write, running %d seconds" %(60*i))
        process.wait()
...
def process_gnuplot_charts( device ):
    ...
    dev = device.split("/")[2]
    is_pcie_device = False
    is_sata_device = False
    if dev[:-1] == "df":
        is_pcie_device = True
    if dev[:-1] == "sd":
        is_sata_device = True
    ...
    if is_pcie_device:
        process = subprocess.Popen( "gnuplot " + abs_path + "/gnuplot/pcie/gnuplot_4k_RR.conf", shell=True ).wait()
    if is_sata_device:
        process = subprocess.Popen( "gnuplot " + abs_path + "/gnuplot/sata/gnuplot_4k_RR.conf", shell=True ).wait()
    ...
    if is_pcie_device:
        subprocess.Popen( "gnuplot " + abs_path + "/gnuplot/pcie/gnuplot_4k_RW.conf", shell=True ).wait()
    if is_sata_device:
        subprocess.Popen( "gnuplot " + abs_path + "/gnuplot/sata/gnuplot_4k_RW.conf", shell=True ).wait()

    cmd = "grep %s iostat_8k_RR.log | awk '{print $4}' | sed -n '900,4499p' | awk '{print(NR, $0)}' > iostat_8k_RR.txt" %dev
    logging.info( cmd )
    process = subprocess.Popen( cmd, shell=True ).wait()
    if is_pcie_device:
        process = subprocess.Popen( "gnuplot " + abs_path + "/gnuplot/pcie/gnuplot_8k_RR.conf", shell=True ).wait()
    if is_sata_device:
        process = subprocess.Popen( "gnuplot " + abs_path + "/gnuplot/sata/gnuplot_8k_RR.conf", shell=True ).wait()
    ...
    subprocess.Popen( cmd, shell=True ).wait()
    if is_pcie_device:
        subprocess.Popen( "gnuplot " + abs_path + "/gnuplot/pcie/gnuplot_8k_RW.conf", shell=True ).wait()
    if is_sata_device:
        subprocess.Popen( "gnuplot " + abs_path + "/gnuplot/sata/gnuplot_8k_RW.conf", shell=True ).wait()

    cmd = "grep %s iostat_128k_SR.log | awk '{print $6}' | sed -n '900,4499p' | awk '{print(NR, $0)}' > iostat_128k_SR.txt" %dev
    logging.info( cmd )
    process = subprocess.Popen( cmd, shell=True ).wait()
    if is_pcie_device:
        process = subprocess.Popen( "gnuplot " + abs_path + "/gnuplot/pcie/gnuplot_128k_SR.conf", shell=True ).wait()
    if is_sata_device:
        process = subprocess.Popen( "gnuplot " + abs_path + "/gnuplot/sata/gnuplot_128k_SR.conf", shell=True ).wait()
    ...
    subprocess.Popen( cmd, shell=True ).wait()
    if is_pcie_device:
        subprocess.Popen( "gnuplot " + abs_path + "/gnuplot/pcie/gnuplot_128k_SW.conf", shell=True ).wait()
    if is_sata_device:
        subprocess.Popen( "gnuplot " + abs_path + "/gnuplot/sata/gnuplot_128k_SW.conf", shell=True ).wait()
    # latency
    ...
    if is_pcie_device:
        subprocess.Popen( "gnuplot " + abs_path + "/gnuplot/pcie/gnuplot_4k_RR_latency.conf", shell=True ).wait()
    if is_sata_device:
        subprocess.Popen( "gnuplot " + abs_path + "/gnuplot/sata/gnuplot_4k_RR_latency.conf", shell=True ).wait()
    ...
    if is_pcie_device:
        subprocess.Popen( "gnuplot " + abs_path + "/gnuplot/pcie/gnuplot_4k_RW_latency.conf", shell=True ).wait()
    if is_sata_device:
        subprocess.Popen( "gnuplot " + abs_path + "/gnuplot/sata/gnuplot_4k_RW_latency.conf", shell=True ).wait()
def workout_values():
	...
	"iostat_128k_SR.txt","iostat_128k_SW.txt","latency_4k_RR.txt","latency_4k_RW.txt"):
        ...
        if filename == "iostat_4k_RR.txt":
            line =  "4K Random Read IOPS:\t%dK\t\t%dK" %((sum(values)/len(values)/1000),max(values)/1000)
	    logging.info( line )
	    print line
	if filename == "iostat_4k_RW.txt":
            line = "4K Random Write IOPS:\t%dK\t\t%dK" %((sum(values)/len(values)/1000),max(values)/1000)
	    logging.info( line )
	    print line
	if filename == "iostat_8k_RR.txt":
            line = "8K Random Read IOPS:\t%dK\t\t%dK" %((sum(values)/len(values)/1000),max(values)/1000)
	    logging.info( line )
	    print line
	if filename == "iostat_8k_RW.txt":
            line =  "8K Random Write IOPS:\t%dK\t\t%dK" %((sum(values)/len(values)/1000),max(values)/1000)
	    logging.info( line )
	    print line
	if filename == "iostat_128k_SR.txt":
	    line = "128K Sequence Read:\t%.1f MB/s\t%.1f MB/s" %((sum(values)/len(values)),max(values))
	    logging.info( line )
	    print line
	if filename == "iostat_128k_SW.txt":
            line = "128K Sequence Write:\t%.1f MB/s\t%.1f MB/s" %((sum(values)/len(values)),max(values))
	    logging.info( line )
	    print line
	if filename == "latency_4k_RR.txt":
	    line = "4K Random Read Latency:\t%.2f us\t%.2f us " %((sum(values)/len(values)), min(values))
	    logging.info( line )
	    print line
	if filename == "latency_4k_RW.txt":
	    line = "4K Random Write Latency:%.2f us\t%.2f us " %((sum(values)/len(values)), min(values))
	    logging.info( line )
	    print line
    print "-------------------End-------------------------"
def determine_path():
...
def main():
    ...
    parser.add_argument('-c','--compare',help="Compare two device")
