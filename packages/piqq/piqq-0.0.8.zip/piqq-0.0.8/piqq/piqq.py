...
class Piqq(object):
    ...
    async def _on_message_callback(self, channel, body, envelope, properties):
        for receiver in self.receivers:
            if envelope.routing_key in receiver.get_routing_keys():
                ...
                callbacks['nack'] = lambda requeue: asyncio.run_coroutine_threadsafe(
                    channel.basic_client_nack(envelope.delivery_tag, requeue=requeue),
                    self.event_loop)
                ...
                callbacks['reject'] = lambda: asyncio.run_coroutine_threadsafe(
                    channel.basic_reject(envelope.delivery_tag),
                    self.event_loop)
    ...
    async def _register_command(self, command_class):
        ...
        if command_class.need_consuming():
            # Consuming
    ...
    async def _publish(self, exchange, routing_key, body, channel=None, properties=None, serialize_as='text'):
        # get channel
        if channel is None:
