...
class Plancton(Daemon):
  ...
  def __init__(self, name, pidfile, logdir, confdir, socket_location='unix://var/run/docker.sock'):
    ...
    self._streamer = streamer.Streamer(host=self.conf["influxdb_address"].split(':')[0], port=self.conf["influxdb_address"].split(':')[1], \
  ...
  def main_loop(self):
    ...
    self.streamer.write_pt(db=self.conf["database_name"], name="measurament", tag={"hostname": self._hostname}, \
