...
def proxy_handler(reader, writer, protos, auth, rserver, block, auth_tables, cipher, httpget, unix_path, verbose=DUMMY, modstat=lambda r,h:lambda i:DUMMY, **kwargs):
    try:
        ...
        if viaproxy:
            verbose('{l.__name__} {}:{} -> {r.protos[0].__name__} {r.bind}'.format(host_name, port, l=lproto, r=rproto))
...
def main():
    ...
    if args.pac:
        ...
        for i, option in enumerate(args.rserver):
            pactext += ('var m{i}=/^(:?{})$/i;if(m{}.test(h))'.format(option.match.__self__.pattern, i) if option.match else '') + 'return "PROXY %(host)s";'
