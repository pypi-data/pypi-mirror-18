...
SUPPORTED_CONFIG_PATHS = map(os.path.abspath, [
    os.path.expanduser('~/.privacy.yml'),
])
...
def initialize_config(self, path, env):
    ...
    self.key_home = self.node.dir.join(os.path.expanduser(env.get('key_home') or './privacy-keyring'))
...
fallback_config_path = os.path.join(os.getcwd(), '.privacy.yml')

for config_path in SUPPORTED_CONFIG_PATHS:
    if os.path.exists(config_path):  # pragma: no cover
        fallback_config_path = config_path

privacy_config_path = os.getenv('PRIVACY_CONFIG_PATH') or fallback_config_path

setup_from_config_path(self, privacy_config_path)
