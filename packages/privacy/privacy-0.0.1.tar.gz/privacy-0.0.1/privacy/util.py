...
def temp_filename():
    return '/tmp/privacy.{0}.tmptz2'.format(os.urandom(32).encode('hex'))


def clean_temp_files():
    regex = re.compile(r'privacy[.][a-z0-9]{64}.tmptz2')
...
def binary_to_plaintext(data):
    return "\n".join([
        ...
        'Version: Privacy v{0}'.format(version),
        '',
        data.encode('base64'),
        '-----END PRIVACY MESSAGE-----',
    ])
...
class BackupManager(object):
    ...
    def generate_backup(self, path):
        ...
        tar.add(self.conf_path,
                arcname='privacy.yml', filter=set_file_info)
    ...
    def recover_backup(self, encoded, conf_path, force=False):
        ...
        conf_member = members.pop('privacy.yml')
        if not conf_member:
            raise InvalidBackupOperation('the backup file is invalid because does not contain a privacy.yml file')
