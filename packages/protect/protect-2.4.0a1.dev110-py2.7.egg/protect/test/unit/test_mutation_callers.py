...
from protect.mutation_calling.fusion import run_ericscript
...
from protect.test.unit.test_alignments import TestAlignments
...
class TestMutationCallers(ProtectTest):
    ...
    def test_ericscript(self):
        """
        Test the functionality of EricScript
        """
        univ_options = self._getTestUnivOptions()
        a = Job.wrapJobFn(TestAlignments._get_test_star_files)
        b = Job.wrapJobFn(self._get_ericscript_options).encapsulate()
        c = Job.wrapJobFn(run_ericscript, a.rv(),  univ_options, b.rv()).encapsulate()
        a.addChild(b)
        b.addChild(c)
        Job.Runner.startToil(a, self.options)

    @staticmethod
    def _get_ericscript_options(job):
        ericscript_options = {}
        call = 's3am download S3://pimmuno-test-data/CI_test_input/' \
               'ericscript_db_ensembl74.tar.gz %s' % os.path.join(os.getcwd(), 'index.tar.gz')
        subprocess.check_call(call.split(' '))
        ericscript_options['tool_index'] = job.fileStore.writeGlobalFile('index.tar.gz')
        return ericscript_options
...
_get_ericscript_options = TestMutationCallers._get_ericscript_options
