...
from protect.mutation_calling.fusion import run_ericscript, ericscript_disk
...
def pipeline_launchpad(job, fastqs, univ_options, tool_options):
    ...
    tool_options['star']['n'] = tool_options['bwa']['n'] = tool_options['phlat']['n'] = \
        tool_options['rsem']['n'] = \
        tool_options['ericscript']['n'] = ascertain_cpu_share(univ_options['max_cores'])
    ...
    ericscript = job.wrapJobFn(run_ericscript, cutadapt.rv(), univ_options,
                               tool_options['ericscript'], memory='100M',
                               cores=tool_options['ericscript']['n'],
                               disk=PromisedRequirement(ericscript_disk, cutadapt.rv())
                               ).encapsulate()
    ...
    transgene = job.wrapJobFn(run_transgene, snpeff.rv(), star.rv(), univ_options, tool_options['transgene'],
                              disk='100M', memory='100M', cores=1, fusion_calls=ericscript.rv())
    ...
    # B. cutadapt will be followed by star and ericscript
    cutadapt.addChild(star)  # Edge 2->9
    cutadapt.addChild(ericscript)
    ...
    ericscript.addChild(fastq_deletion_2)
