...
from exception.exceptions import TokenizationException
...
class RegexReplacer(Replacer):
    ...
    def __init__(self, regex_query_string, target_string_with_groups, case_sensitive, use_locale, use_unicode):
        ...
        flag_3 = re.UNICODE if use_unicode else 0
...
class RegexReplacerAdapter(RegexReplacer):
    ...
    def __init__(self, regex_query, replace_tag_with=None, case_sensitive=False, use_locale=False, use_unicode=False):
        if not replace_tag_with:
            replace_tag_with = ''
        super(RegexReplacerAdapter, self).__init__(regex_query, replace_tag_with, case_sensitive, use_locale, use_unicode)
...
class MultiWhitespaceReplacer(RegexReplacerAdapter):
    ...
    def __init__(self, replace_tag_with=' '):
        regex_query = r'\s+'
        super(MultiWhitespaceReplacer, self).__init__(regex_query, replace_tag_with)
...
class HtmlTagReplacer(RegexReplacerAdapter):
    ...
    def __init__(self, replace_tag_with=None):
        regex_query = r'<.*?>'
        super(HtmlTagReplacer, self).__init__(regex_query, replace_tag_with)
...
class VKMentionReplacer(RegexReplacerAdapter):
    ...
    def __init__(self, replace_tag_with=None):
        regex_query = ur'\[[\w_\-:]+\|.*\]'
        super(VKMentionReplacer, self).__init__(regex_query, replace_tag_with, False, False, True)


class AtReferenceReplacer(RegexReplacerAdapter):
    """Replaces the inner VK links and user mention of type '@<id>' (or '@<name>'?).
    This replaces finds only such entities which are separate words (start the line or
    have blank whitespace)"""

    def __init__(self, replace_tag_with=None):
        regex_query = ur'((?<=\s)|(?<=^))@[\w_-]+'
        super(AtReferenceReplacer, self).__init__(regex_query, replace_tag_with, False, False, True)
...
class URLReplacer(RegexReplacerAdapter):
    ...
    def __init__(self, replace_tag_with=None):
        ...
        super(URLReplacer, self).__init__(regex_query, replace_tag_with, False, False, True)
...
class EmailReplacer(RegexReplacerAdapter):
    ...
    def __init__(self, replace_tag_with=None):
        ...
        super(EmailReplacer, self).__init__(regex_query, replace_tag_with, False, False, True)
...
class DigitsReplacer(RegexReplacerAdapter):
    ...
    def __init__(self, multi, replace_tag_with=None):
        ...
        super(DigitsReplacer, self).__init__(regex_query, replace_tag_with, False)
...
class QuotesReplacer(RegexReplacerAdapter):
    ...
    def __init__(self, replace_tag_with=DEFAULT_REPLACEMENT):
        # Some of the symbols are taken from other than punctuation section
        regex_query = ur'[\'`ʹʻʼʽ՚‘’‚‛′‵]'
        # This section does not contain quotes from not-punctuation section
        # regex_query = ur'[\'‘’‚‛′‵]'
        super(QuotesReplacer, self).__init__(regex_query, replace_tag_with, False, False, True)
...
class DoubleQuotesReplacer(RegexReplacerAdapter):
    ...
    def __init__(self, replace_tag_with=DEFAULT_REPLACEMENT):
        regex_query = ur'["«»ʺ“”„‟″‶]'
        super(DoubleQuotesReplacer, self).__init__(regex_query, replace_tag_with, False, False, True)
...
class DoubleQuotesEscaper(RegexReplacerAdapter):
    ...
    def __init__(self, replace_tag_with=DEFAULT_REPLACEMENT):
        regex_query = ur'\"'
        super(DoubleQuotesEscaper, self).__init__(regex_query, replace_tag_with, False, False, True)
...
class DashAndMinusReplacer(RegexReplacerAdapter):
    ...
    def __init__(self, replace_tag_with='-'):
        regex_query = ur'[-‐‑‒–—]'
        super(DashAndMinusReplacer, self).__init__(regex_query, replace_tag_with, False, False, True)
...
class SoftHyphenReplacer(RegexReplacerAdapter):
    ...
    def __init__(self, replace_tag_with=''):
        regex_query = ur'[­]'
        super(SoftHyphenReplacer, self).__init__(regex_query, replace_tag_with, False)
...
class TripledotReplacer(RegexReplacerAdapter):
    ...
    def __init__(self, replace_tag_with='...'):
        regex_query = ur'[…]'
        super(TripledotReplacer, self).__init__(regex_query, replace_tag_with, False)
...
class BoldTagReplacer(RegexReplacerAdapter):
    ...
    def __init__(self, replace_tag_with=''):
        regex_query = ur'</?b>'
        super(BoldTagReplacer, self).__init__(regex_query, replace_tag_with, False)


class MultiPunctuationReplacer(RegexReplacerAdapter):
    """Removes all multiple punctuation (default keyboard set + some additional) usage
    according to regex ur'[`~!@#$%^&*=+\(\)\[\]{}<>/|\\?\.…,№:;_-]'."""

    def __init__(self, replace_tag_with='\\1'):
        regex_query = ur'([\'\"`~!@#$%^&*=+\(\)\[\]{}<>/|\\?\.…,№:;_-])\1+'
        super(MultiPunctuationReplacer, self).__init__(regex_query, replace_tag_with, False, False, False)


class NonAlphaNumPuncWhitespaceAllUnicodeReplacer(RegexReplacerAdapter):
    """Replaces all unicode alpha-numeric, punctuation (default keyboard set + some additional), and whitespaces
    according to regex ur'[^\s\w\'\"`~!@#$%^&*=+\(\)\[\]{}<>/|\\?\.…,№:;_-]'."""

    def __init__(self, replace_tag_with=''):
        regex_query = ur'[^\s\w\'\"`~!@#$%^&*=+\(\)\[\]{}<>/|\\?\.…,№:;_-]'
        super(NonAlphaNumPuncWhitespaceAllUnicodeReplacer, self).__init__(regex_query, replace_tag_with, False, False, True)
...
class NonWordOrNumberOrWhitespaceAllUnicodeReplacer(RegexReplacerAdapter):
    ...
    def __init__(self, replace_tag_with=' '):
        parts = [
            ur'(?<=\d)[,:\.](?!\d)|(?<!\d)[,:\.](?=\d)|(?<!\d)[,:\.](?!\d)',  # this leaves out digital sequences with , : . in middle
            ur'(?<=[\w\d])[\'\-](?![\w\d])|(?<![\w\d])[\'\-](?=[\w\d])|(?<![\w\d])[\'\-](?![\w\d])',  # this leaves out alphanumeric tokens with - and ' in middle
            ur'[^\s\w\d\'\-,:\.]+',  # all other non-whitespace, non-alpha and non-digits
            ur'_',
            ]
        regex_query = ur'(' + ur'|'.join(parts) + ur')+'
        super(NonWordOrNumberOrWhitespaceAllUnicodeReplacer, self).__init__(regex_query, replace_tag_with, False, False, True)
...
class WordTokenizer(RegexReplacerAdapter):
    ...
    def __init__(self, token_delim=TOKEN_DEFUALT_DELIM):
        token_types = [
            ur'([^\W\d_]+([\'\-][^\W\d_]+)*[^\W\d_]*)',  # tokens, also with - and ' in middle
            ur'([^\W_]+([\'\-][^\W\d_]+)+)',  # tokens, starting with number and ' - in middle ("7-fold", etc.)
            ur'(\d+([,:.]\d+)*\d*)',  # digital sequences, also with , : . in middle
            # ur'[^\W\d_]+',  # all other alpha tokens, excluding underscore
            # ur'[\d\W]+',  # all other numeric sequences
            ur'\.+',
            ur'[^\w\d\s]',  # all individual symbols that are not alphanum or whitespaces
            ur'_',  # underscore as being exluded by \w and previously by [^\W\d_]+
        ]
        regex_query = ur'(' + ur'|'.join(token_types) + ur')'
        super(WordTokenizer, self).__init__(regex_query, token_delim, False, False, True)
...
class DefaultPreprocessorStack(StackingPreprocessor):
    def __init__(self):
        ...
        super(DefaultPreprocessorStack, self).__init__(stack)
