...
class Section(BaseModel):
    """A cross section with hydraulic attributes and dimensions"""

    name = fields.CharField(unique=True)
    span = fields.FloatField()
    rise = fields.FloatField()
    ...
    shape = fields.IntegerField(default=0, choices=SHAPE_CHOICES)
    ...
    @classmethod
    def get_all_shape_display_names(cls):
        return [choice[1] for choice in cls.SHAPE_CHOICES]
    ...
    def shape_display(self):
        return dict(self.SHAPE_CHOICES)[self.shape]
        
    @shape_display.setter
    def shape_display(self, value):
        for choice in self.SHAPE_CHOICES:
            if choice[1] == value:
                self.shape = choice[0]
                return
        raise ValueError('{0} is not a valid choice name.'.format(value))
    
    def delete_related(self):
        """Delete all instances in the database that are related to the section.

        The following queries are performed and deleted:
            - :class:`Pipe` instances where Pipe.section is the section.

        """
        pipe_query = Pipe.delete().where(Pipe.section == self)
        pipe_query.execute()


class Pipe(BaseModel):
    """A link between two nodes that has hydraulic dimensions and relationships"""
    ...
    # mannings = fields.FloatField()
    section = fields.ForeignKeyField(Section)
