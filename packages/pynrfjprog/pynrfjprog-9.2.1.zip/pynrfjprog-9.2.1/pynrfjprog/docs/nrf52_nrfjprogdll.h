 ...
 *
 * @details Reads data_len characters from addr address of the external QSPI-connected memory. Reserved RAM buffer for the operation is 4096 bytes. Actual read command address and length sent to
 *          the device may be adjusted since the QSPI peripheral only accepts 4-byte aligned addresses and multiple of 4 lengths. Considering these adjustments, maximum data_len is 4096 bytes minus
 *          addr modulus 4.
 *
 * @pre     Before the execution of this function, the dll must be open. To open the dll, see NRFJPROG_open_dll() function.
 ...
 *                                              The data_len parameter is greater than 4096 minus addr modulus 4.
 ...
 *
 * @details Writes data_len characters to addr address of the external QSPI-connected memory. Reserved RAM buffer for the operation is 4096 bytes. Actual write command address and length sent to
 *          the device may be adjusted since the QSPI peripheral only accepts 4-byte aligned addresses and multiple of 4 lengths. Considering these adjustments, maximum data_len is 4096 bytes 
 *          minus addr modulus 4. Any extra bytes written to the external QSPI-connected memory will be 0xFF to avoid altering the contents of the flash. Note that NRFJPROG_qspi_write() function
 *          does not erase the addresses to be written. If target addresses are not erased, the flash contents after NRFJPROG_qspi_write() operation are therefore undefined.
 *
 * @pre     Before the execution of this function, the dll must be open. To open the dll, see NRFJPROG_open_dll() function.
 * @pre     Before the execution of this function, the QSPI must be initialized. To initialize the QSPI, see NRFJPROG_qspi_init() function.
 * @pre     Before the execution of this function, a connection to the emulator must be established. To establish a connection, see NRFJPROG_connect_to_emu_with_snr() and NRFJPROG_connect_to_emu_without_snr() functions.
