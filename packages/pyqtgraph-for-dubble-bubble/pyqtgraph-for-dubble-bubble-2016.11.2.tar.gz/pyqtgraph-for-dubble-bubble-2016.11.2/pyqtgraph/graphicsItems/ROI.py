...
class ROI(GraphicsObject):
    ...
    **Signals**
    ...
    def getArraySlice(self, data, img, axes=(0,1), returnSlice=True):
        ...
        bounds = (
            (int(min(intBounds.left(), intBounds.right())), int(max(intBounds.left(), intBounds.right()))),
            (int(min(intBounds.bottom(), intBounds.top())), int(max(intBounds.bottom(), intBounds.top())))
        )
        if axisOrder == 'row-major':
            bounds = bounds[::-1]
