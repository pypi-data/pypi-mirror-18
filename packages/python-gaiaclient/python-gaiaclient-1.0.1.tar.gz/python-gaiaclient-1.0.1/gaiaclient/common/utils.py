...
def require_fields(required_fields):
    ...
    This decorator checks that required fields are present

    On error throws CommandError exception with meaningful message.

    :param required_fields: Which fields' presence imply image data
    :type required_fields: iter
    :return: function decorator
    """
    ...
    def args_decorator(func):
        ...
        def func_wrapper(gc, args):
            ...
            if missing:
                msg = (_("error: Must provide %(req)s.") %
                       {'req': prepare_fields(missing)})
                raise exc.CommandError(msg)
            return func(gc, args)
        return func_wrapper
...
def require_dc(func):
    ...
    def func_wrapper(gc, args):
        ...
        # How many conditional requirements are missing
        missing = 'dc_name' not in fields

        if missing:
            msg = (_("error: Must provide DC_NAME or --dc-name."))
            raise exc.CommandError(msg)

        dc = find_resource(gc.dcs, args.dc_name)
        gc.set_dc(dc.id)
        return func(gc, args)
...
def find_resource(manager, name_or_id):
    ...
    matches = list(manager.list(filters={'name': encodeutils.safe_decode(name_or_id)}))
...
def import_versioned_module(version, submodule=None):
    module = 'gaiaclient.v%s' % version
    if submodule:
        module = '.'.join((module, submodule))
    return importutils.import_module(module)


def import_versioned_shell_command(version, shell=None):
    module = 'gaiaclient.v%s.shells' % version

    if shell:
        module = '.'.join((module, shell))
        for attr in (a for a in dir(module) if a.startswith('do_')):
            method = '.'.join((module, attr))
            print(method)
            importutils.import_module(method)
...
def debug_enabled(argv):
    if bool(env('GAIACLIENT_DEBUG')) is True:
        return True
    ...
    for key in keys:
        ...
        if not valid_name:
            msg = _('Invalid key: "%s". Keys may only contain letters, '
                    ...
                    'hyphens.')
            raise exc.CommandError(msg % key)
...
def do_action_on_many(action, resources, success_msg, error_msg):
    ...
    if failure_flag:
        raise exc.CommandError(error_msg)


def find_volume(gc, volume):
    """Get a volume by name or ID."""
    return find_resource(gc.volumes, volume)
