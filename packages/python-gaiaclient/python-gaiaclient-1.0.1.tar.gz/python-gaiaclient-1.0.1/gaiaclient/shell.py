"""
Command-line interface to the Zeus Gaia API.
"""
...
class ZeusGaiaShell(object):
    ...
    def _append_global_identity_args(self, parser, argv):
        ...
        parser.add_argument('--dc-name',
                            default=utils.env('DC_NAME'),
                            help='Defaults to env[DC_NAME].')
    ...
    def get_base_parser(self, argv):
        ...
        parser.add_argument('--version',
                            action='version',
                            version=gaiaclient.__version__)

        parser.add_argument('-d', '--debug',
                            default=bool(utils.env('GAIACLIENT_DEBUG')),
                            action='store_true',
                            help='Defaults to env[GAIACLIENT_DEBUG].')
        ...
        parser.add_argument('--os-gaia-url',
                            default=utils.env('OS_GAIA_URL'),
                            help=('Defaults to env[OS_GAIA_URL]. '
                                  'If the provided gaia url contains '
                                  'a version number and '
                                  '`--os-gaia-api-version` is omitted '
                                  'the version of the URL will be picked as '
                                  'the gaia api version to use.'))

        parser.add_argument('--os-gaia-api-version',
                            default=utils.env('OS_GAIA_API_VERSION',
                                              default=None),
                            help='Defaults to env[OS_GAIA_API_VERSION] or 1.')
        ...
        if osprofiler_profiler:
            parser.add_argument('--profile',
                                ...
                                'gaia, it is specified in paste '
                                'configuration file at '
                                '/etc/gaia/api-paste.ini and '
                                '/etc/gaia/registry-paste.ini. Without key '
    ...
    def get_subcommand_parser(self, version, argv=None):
        ...
        submodule_dcs = utils.import_versioned_module(version, 'dcs_shell')
        submodule_servers = utils.import_versioned_module(version, 'servers_shell')
        submodule_flavors = utils.import_versioned_module(version, 'flavors_shell')
        submodule_volumes = utils.import_versioned_module(version, 'volumes_shell')
        submodule_images = utils.import_versioned_module(version, 'images_shell')
        submodule_networks = utils.import_versioned_module(version, 'networks_shell')
        submodule_versions = utils.import_versioned_module(version, 'versions_shell')
        submodule_usages = utils.import_versioned_module(version, 'usages_shell')

        self._find_actions(subparsers, submodule_dcs)
        self._find_actions(subparsers, submodule_servers)
        self._find_actions(subparsers, submodule_flavors)
        self._find_actions(subparsers, submodule_volumes)
        self._find_actions(subparsers, submodule_images)
        self._find_actions(subparsers, submodule_networks)
        self._find_actions(subparsers, submodule_versions)
        self._find_actions(subparsers, submodule_usages)
    ...
    def _get_versioned_client(self, api_version, args):
        endpoint = self._get_gaia_url(args)
        ...
        if endpoint and auth_token:
            ...
            if endpoint is None:
                ...
                service_type = args.os_service_type or 'gaia'
                endpoint = ks_session.get_endpoint(
                    service_type=service_type,
                    interface=endpoint_type,
                    region_name=args.os_region_name)

        return gaiaclient.Client(api_version, endpoint, **kwargs)

    def _cache_schemas(self, options, client, home_dir='~/.gaiaclient'):
        ...
        if not os.path.exists(path_prefix):
            try:
                ...
                # This avoids gaiaclient to crash if it can't write to
                # ~/.gaiaclient, which may happen on some env (for me,
                # it happens in Jenkins, as gaiaclient can't write to
                # /var/lib/jenkins).
                msg = '%s' % e
                print(encodeutils.safe_decode(msg), file=sys.stderr)
        resources = ['gaia', 'metadefs/namespace', 'metadefs/resource_type']
        schema_file_paths = [os.path.join(path_prefix, x + '_schema.json')
                             for x in ['gaia', 'namespace', 'resource_type']]
    ...
    def main(self, argv):
        ...
        try:
            ...
            # gaia-url first. If no version was specified, fallback
            # to the api-gaia-version arg. If both of these fail then
            # fallback to the minimum supported one and let keystone
            # do the magic.
            endpoint = self._get_gaia_url(options)
        ...
        try:
            api_version = int(options.os_gaia_api_version or url_version or 1)
        ...
        # if not options.os_gaia_api_version and api_version == 2:
        #     switch_version = True
        ...
        LOG = logging.getLogger('gaiaclient')
        ...
        try:
            ...
            raise exc.CommandError("Invalid Zeus Identity credentials.")
        finally:
            if profile:
    ...
    def do_help(self, args, parser):
        ...
        # if not args.os_gaia_api_version or args.os_gaia_api_version == '2':
        #     # NOTE(NiallBunting) This currently assumes that the only versions
        #     # are one and two.
        #     try:
        #         if command is None:
        #             print("\nRun `gaia --os-gaia-api-version 1 help`"
        #                   " for v1 help")
        #         else:
        #             self.get_subcommand_parser(1)
        #             if command in self.subcommands:
        #                 command = ' ' + command
        #                 print(("\nRun `gaia --os-gaia-api-version 1 help%s`"
    ...
    def do_bash_completion(self, _args):
        ...
        gaia.bash_completion script doesn't have to hard code them.
        """
        commands = set()
        options = set()
...
def main():
    try:
        ...
        ZeusGaiaShell().main(argv)
    except KeyboardInterrupt:
        utils.exit('... terminating gaia client', exit_code=130)
