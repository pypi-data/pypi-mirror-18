...
class TestCase(testtools.TestCase):
    ...
    def prepare_fixtures(self, fixtures):
        self.api = FakeAPI(fixtures)

    def assert_request_id(self, request_id_mixin, request_id_list):
        self.assertEqual(request_id_list, request_id_mixin.request_ids)

    def assert_called(self, method, path):
        last_call = self.api.calls[-1]
        self.assertEqual(last_call[0], method)
        self.assertEqual(last_call[1], path)
    ...
    def assert_called_anytime(self, method, url, body=None):
        ...
        for call in self.api.calls:
            if call[0] == method and call[1] == url:
                found = True

        assert found, 'Expected %s; got %s' % (expected, self.api.calls)
