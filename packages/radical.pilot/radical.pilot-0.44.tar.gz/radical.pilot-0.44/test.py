...
def unit_cb(unit, state):
    print 'unit : %s: %s' % (cu.uid, state)

def pilot_cb(pilot, state):
    print 'pilot: %s: %s' % (pilot.uid, state)
...
if __name__ == '__main__':
    ...
    try:
        ...
        pmgr.register_callback(pilot_cb)
        ...
        pdesc1 = rp.ComputePilotDescription(pd_init)
        pd_init['runtime'] = 2
        pdesc2 = rp.ComputePilotDescription(pd_init)
