...
class Announce(Base):
    ...
    def queue_bind_header(self,service):
        try:
            ...
            def log(*args):
                self.log.info('Queue bind: %s -> %s' % (self.settings['services'][service]['queue'],self.settings['services'][service]['bind_args']))
                return True
    ...
    def queue_bind_rk(self,service):
        try:
            ...
            def log(*args):
                self.log.info('Queue bind: %s -> %s' % (self.settings['services'][service]['queue'],self.settings['services'][service]['rk']))
                return True
