...
class MakeConfig(Subcommand):
    ...
    def run(self, args):
        ...
        if not os.path.exists(template_path):
            self.stderr.write("config template not found: {}".format(template_path))
            sys.exit(1)
...
class MakeUser(Subcommand):
    ...
    def mkuser_rattail(self, args):
        ...
        auth.set_user_password(user, self.obtain_password())
        if args.admin:
    ...
    def mkuser_windows(self, args):
        ...
        return users.create_user(args.username, self.obtain_password(),
...
class TempmonServer(Subcommand):
    ...
    def run(self, args):
        from rattail.tempmon.server import make_daemon
