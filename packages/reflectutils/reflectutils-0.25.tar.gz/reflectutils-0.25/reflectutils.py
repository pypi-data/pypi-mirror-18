...
def name_boundmethod(bm):
	...
	"""
	self = getattr(bm, "__self__", None)
	if self is None:
		self = getattr(bm, "im_self", None)
	...
	if hasattr(cls, "__name__") and hasattr(bm, "__name__"):
		if cls.__name__ and bm.__name__:
			return cls.__name__ + "." + bm.__name__
	return None
...
def fullname(obj):
	...
	if not isinstance(name, str) and isboundmethod(obj):
