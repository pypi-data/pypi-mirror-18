    ...
    if regularized_loss is None:
        def regularized_loss(weights, x, y):
            ...
            if not y is None and obs_lambda > 0:
                ''' Add a parameter to balance between out log-likelihood and
                    obs log-likelihood.
                '''
                loss += obs_lambda*loglike_fun(weights, x, None)

            ''' Add a l1 regularizer to penalize large weights '''
...
def train_gru(
    ...
    def regularized_loss(weights, x, y):
        ...
        ''' Add a l1 regularizer to penalize large weights '''
        loss = -loss + l1_lambda * np.sum(np.abs(weights))

        if tree_lambda > 0:
            ''' Add a regularizer based on decision tree.

                1) use just the gru to calculate outputs (predictions)
                2) with the (observation, gru-output) pairs,
                   we can train a decision tree
                3) we can also accumulate a bunch of (gru-weights,
                   # of nodes in the tree) pairs.
            ...
            if is_autograd_array(logpreds):
                logpreds = autograd_extract(logpreds)
            tree = _train_decision_tree_from_inputs(x, np.exp(logpreds))
            # print('tree: {} nodes'.format(tree.tree_.node_count))
            loss += tree_lambda * tree.tree_.node_count
        return loss
...
def train_gru_hmm(
    ...
    # build a gru function in the background for tree regularization
    if tree_lambda > 0:
        _gru_pred_fun, _gru_loglike_fun, _gru_num_weights = gru.build(
            input_count, gru_state_count, output_count)
    ...
    def regularized_loss(weights, x, y):
        ...
        ''' Add a l1 regularizer to penalize large weights '''
        loss = -loss + l1_lambda * np.sum(np.abs(weights[:num_gru_weights]))
        ...
        if tree_lambda > 0:
            ''' Add a regularizer based on decision tree.
                ...
                5) then during training gru, we can add something like
                   `lambda*(# of predicted nodes` to the cost function.
            '''
            logpreds = _gru_pred_fun(weights, x)
            if is_autograd_array(logpreds):
                logpreds = autograd_extract(logpreds)
            tree = _train_decision_tree_from_inputs(x, np.exp(logpreds))
            print('tree: {} nodes'.format(tree.tree_.node_count))
