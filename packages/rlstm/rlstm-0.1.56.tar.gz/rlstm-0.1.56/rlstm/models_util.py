...
from timeseries_util import farray, map_3d_to_fencearray
...
def train_by_sgd(
    ...
    num_batches = int(np.ceil(obs_farray.shape[-1] / float(batch_size)))
    ...
    if regularized_loss is None:
        def regularized_loss(weights, x_farray, y_farray):
            loss = loglike_fun(weights, x_farray, y_farray)
            if not y_farray is None and obs_lambda > 0:
                ''' Add a parameter to balance between out log-likelihood and obs log-likelihood. '''
                loss += obs_lambda*loglike_fun(weights, x_farray, None)
            loss = -loss
    ...
    if batch_loss is None:
        def batch_loss(weights, iter):
            ...
            batch_obs_farray = obs_farray.slice((obs_farray.shape[0], idx))
            batch_out_farray = None if out_farray is None \
                else out_farray.slice((out_farray.shape[0], idx))
            return regularized_loss(weights, batch_obs_farray, batch_out_farray)

    # loss fnction against all data
    if collective_loss is None:
        def collective_loss(weights):
            return regularized_loss(weights, obs_farray, out_farray)

    if validation_loss is None:
        def validation_loss(weights):
            return regularized_loss(weights, va_obs_farray, va_out_farray)

    if callback is None:
        def callback(e_i, b_i, ll, va_ll, p, w):
            acc = error_func(out_farray, np.exp(logpred_fun(w, obs_farray))) \
                if not out_farray is None else None

            if not va_ll is None:
                va_acc = error_func(va_out_farray, np.exp(logpred_fun(w, va_obs_farray))) \
                    if not va_out_farray is None else None
    ...
    weights = adam(grad_fun,
                   ...
                   va_ll_fun=validation_loss if not va_obs_farray is None else None,
...
def train_lr(
    ...
    in_dim_count = obs_farray.shape[0]
    out_dim_count = out_farray.shape[0]

    num_weights = np.prod((out_dim_count, in_dim_count + 1))
    loglike_fun = lr.log_likelihood
    logpred_fun = lambda weights, obs_farray: lr.logistic_log_predictions(
        weights, obs_farray, in_dim_count, out_dim_count)
    ...
    trained_weights = train_by_sgd(
        ...
        early_stop=early_stop,
        patience=patience,
        va_obs_farray=va_obs_farray,
        va_out_farray=va_out_farray)
...
def append_lr_to_model(
    ...
    _, lprob_farray = hmm.get_state_prob_farray(
        output_fun, hmm_weights, obs_array)

    va_lprob_farray = None
    if not va_obs_farray is None:
        _, va_lprob_farray = hmm.get_state_prob_farray(
            output_fun, hmm_weights, va_obs_farray)

    # train lr
    logpred_fun, ll_fun, lr_weights = train_lr(lprob_farray,
                                               out_farray,
                                               init_weights=init_weights,
                                               min_iters=min_iters,
                                               max_iters=max_iters,
                                               ...
                                               early_stop=early_stop,
                                               patience=patience,
                                               va_obs_farray=va_lprob_farray,
                                               va_out_farray=va_out_farray)

    trained_weights = np.hstack((hmm_weights, lr_weights))

    # return functions so users can pass in obs_farray (instead of prob_farray)
    def all_logpred_fun(weights, obs_farray):
        _, lprob_farray = hmm.get_state_prob_farray(
            output_fun, weights, obs_farray)
        hmm_weights = weights[:num_hmm_weights]
        lr_weights = weights[num_hmm_weights:]
        return logpred_fun(lr_weights, lprob_farray)

    def all_loglike_fun(weights, obs_farray, out_farray=None):
        ...
        if not out_farray is None:
            _, lprob_farray = hmm.get_state_prob_farray(
                output_fun, hmm_weights, obs_farray)
            return ll_fun(lr_weights, lprob_farray, out_farray)
        else:
            return loglike_fun(hmm_weights, obs_farray)
...
def train_hmm_sgd(
    ...
    obs_count = obs_farray.shape[0]
    out_count = None
    if not obs_only:
        out_count = out_farray.shape[0]
    ...
    if init_lr_weights is None:
        init_lr_weights = init_hmm_out_weights(
            hmm_state_count, out_farray.shape[0], param_scale)
    ...
    trained_weights = train_by_sgd(
        ...
        outputs,
        num_weights,
        out_farray=None if obs_only else out_farray,
        init_weights=init_weights,
        min_iters=min_iters,
        max_iters=max_iters,
        batch_size=batch_size,
...
def train_mlp(
    ...
    input_count = obs_farray.shape[0]
    output_count = out_farray.shape[0]
...
class BlackBoxTreeRegularizer(object):
    ...
    def train_tree(self,
        ...
        num_batches = int(np.ceil(obs_farray.shape[2] / float(batch_size)))

        def bb_loss(weights, x_farray, y_farray):
            return bb_loglike_fun(weights, x_farray, y_farray)

        # train a tree + store stuff in the callback
        def callback(e_i, b_i, ll, va_ll, p, w):
            iter_i = e_i * num_batches + b_i
            tree = train_decision_tree(bb_pred_fun, w, obs_farray)
            weights_store[:, iter_i] = w
            counts_store[:, iter_i] = tree.tree_.node_count

            acc = get_accuracy(out_farray, np.exp(bb_logpred_fun(w, obs_farray))) \
                if not out_farray is None else None

            if not va_ll is None:
                va_acc = get_accuracy(va_out_farray, np.exp(bb_logpred_fun(w, va_obs_farray))) \
                    if not va_out_farray is None else None
...
def train_gru_hmm_tree(
    ...
    def regularized_loss(weights, x_farray, y_farray):
        loss = loglike_fun(weights, x_farray_farray, y_farray)
        if not y_farray is None and obs_lambda > 0:
            loss += obs_lambda*loglike_fun(weights, x_farray, None)
        loss = -loss
        if l1_lambda > 0:
        ...
        if tree_lambda > 0:
            # we need nan_to_num b/c rarely MLP could predict < 0
            loss += tree_lambda * np.sum(np.nan_to_num(tree_mlp_pred_fun(
                tree_mlp_weights, weights[:num_gru_weights])))
        return loss
