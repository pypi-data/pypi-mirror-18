    ...
    def outputs(weights, input_farray):
        ...
        for data_iter in range(data_count):
            ...
            for time_iter in range(time_count):
                curr_input_farray = input_farray.slice((xrange(feat_count), data_iter))[:, time_count]

                hiddens = update(max_conn,
                                 redun_epsilon,
                                 np.expand_dims(curr_input_farray, axis=0),
                                 hiddens,
                                 update_x_weights,
                                 update_h_weights,
                                 reset_x_weights,
            ...
            if data_iter == 0:
                ...
                # hstack here because we will treat it has a fake 2D array
        ...
        return output1_farray, output0_farray

    def prediction(weights, input_farray):
        out_farray, _ = outputs(weights, input_farray)
        return out_farray

    def log_likelihood(weights, input_farray, output_farray):
        lprobs1_farray, lprobs0_farray = outputs(weights, input_farray)
