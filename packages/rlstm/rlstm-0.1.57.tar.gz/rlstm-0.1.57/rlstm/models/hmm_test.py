...
from timeseries_util import farray
from hmm import *
...
if __name__ == '__main__':
    ...
    x_DMF = farray(x_DM, fenceposts_Np1)
    ...
    loss_x, loss_y = calc_loss_for_one_sequence(
        x_n_DT=x_DMF.slice((range(n_dims),range(10))),
    ...
    x_log_proba_T = calc_log_proba_arr_for_x(
        x_DMF.slice((range(n_dims),range(10))), eta_DK=eta_DK)
    ...
    x_log_proba_TK = calc_log_proba_arr_for_x(
        x_DMF.slice((range(n_dims),range(10))), eta_DK=eta_DK)
    ...
    x_log_proba_TK = calc_log_proba_arr_for_x(
        x_DMF.slice((range(n_dims),range(10))), eta_DK)
    prior_log_proba_K = start_log_proba_K
    mixture_loss_x = -1 * np.sum(logsumexp(
        x_log_proba_TK + prior_log_proba_K[np.newaxis, :], axis=1))
    assert np.allclose(hmm_loss_x, mixture_loss_x)
