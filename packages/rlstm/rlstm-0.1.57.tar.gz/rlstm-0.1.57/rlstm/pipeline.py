...
from timeseries_util import map_3d_to_fencearray
...
class RLSTMNode(object):
    ...
    def default_params(self):
        ...
        params['has_out_farray'] = True
        params['has_va_farray'] = False
    ...
    def run(self, x, y=None, va_x=None, va_y=None):
        ...
        if self.params['has_va_farray']:
            used_params['va_obs_farray'] = va_x
            used_params['va_out_farray'] = va_y
    ...
    def eval(self, tr_x, tr_y, te_x, te_y):
        ...
        # aucs
        tr_pr_farray = pred_fun(trained_weights, tr_x)
        te_pr_farray = pred_fun(trained_weights, te_x)

        tr_pr_farray = tr_pr_farray.reshape(tr_y.shape)
        te_pr_farray = te_pr_farray.reshape(te_y.shape)

        metrics['{}:tr_auc'.format(self.name)] = \
            get_auc(tr_y.flatten(), tr_pr_farray.flatten())
        metrics['{}:te_auc'.format(self.name)] = \
            get_auc(te_y.flatten(), te_pr_farray.flatten())

        # accuracy
        metrics['{}:tr_acc'.format(self.name)] = \
            get_accuracy(tr_y, tr_pr_farray)
        metrics['{}:te_acc'.format(self.name)] = \
            get_accuracy(te_y, te_pr_farray)
...
class RLSTMPipeline(object):
    ...
    def __init__(self, dataset, log_file=None, weights_file=None):
        ...
        (tr_obs_farray, tr_out_farray), (te_obs_farray, te_out_farray), \
            (va_obs_farray, va_out_farray) = self.load_data(dataset)

        self.data = ((tr_obs_farray, tr_out_farray),
                     (te_obs_farray, te_out_farray),
                     (va_obs_farray, va_out_farray))
        self.obs_count = tr_obs_farray.shape[0]
        self.out_count = tr_out_farray.shape[0]
    ...
    def load_data(self, dataset):
        ...
        tr_obs_farray = map_3d_to_fencearray(tr_obs_set)
        tr_out_farray = map_3d_to_fencearray(tr_out_set)
        te_obs_farray = map_3d_to_fencearray(te_obs_set)
        te_out_farray = map_3d_to_fencearray(te_out_set)
        va_obs_farray = map_3d_to_fencearray(va_obs_set)
        va_out_farray = map_3d_to_fencearray(va_out_set)

        return (tr_obs_farray, tr_out_farray), (te_obs_farray, te_out_farray), \
            (va_obs_farray, va_out_farray)
    ...
    def run_and_eval(self):
        ...
        ((tr_obs_farray, tr_out_farray),
         (te_obs_farray, te_out_farray),
         (va_obs_farray, va_out_farray)) = self.data
        ...
        while node:
            ...
            if node.params['has_out_farray']:
                node.run(tr_obs_farray, tr_out_farray, va_obs_farray, va_out_farray)
            else:
                node.run(tr_obs_farray, None, va_obs_farray, None)
        ...
        all_metrics = {}
        while node:
            node_metrics = node.eval(tr_obs_farray,
                                     tr_out_farray,
                                     te_obs_farray,
                                     te_out_farray)
