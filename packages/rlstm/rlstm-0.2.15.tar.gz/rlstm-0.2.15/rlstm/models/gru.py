...
def kdropout(weights, k):
    ...
    sort_weights = np.concatenate(
        (np.zeros((num_inputs, num_states-k)), sort_weights[:, -k:]), axis=1)
    ...
    def outputs(weights, input_set, fence_set):
        ...
        for data_iter in range(data_count):
            ...
            for time_iter in range(time_count):
                ...
                # more convoluted way with concatenate because
                ...
                if time_iter == 0:
                    ...
                    out_mat1 = np.concatenate((out_mat1, out_vec1), axis=1)
                    out_mat0 = np.concatenate((out_mat0, out_vec0), axis=1)

            if data_iter == 0:
                output_set1 = out_mat1
                output_set0 = out_mat0
            else:
                # concatenate here because we will treat it has a fake 2D array
                output_set1 = np.concatenate((output_set1, out_mat1), axis=1)
                output_set0 = np.concatenate((output_set0, out_mat0), axis=1)

        return output_set1, output_set0

    def prediction(weights, input_set, fence_set):
        output_set, _ = outputs(weights, input_set, fence_set)
        return output_set

    def log_likelihood(weights, input_set, fence_set, output_set):
        lprobs1, lprobs0 = outputs(weights, input_set, fence_set)
