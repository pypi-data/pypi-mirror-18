...
def hiddens_to_output_probs(
    ...
    state = np.concatenate((copy(lstate_norm), np.ones((1, lstate_norm.shape[1]))), axis=0)
        ...
        for data_iter in range(data_count):
            ...
            for time_iter in range(time_count):
                ...
                if time_iter == 0:
                    ...
                    lmat = np.concatenate((lmat, cur_belief_log_proba_K), axis=1)
            ...
            if data_iter == 0:
                ...
                lstate_set = np.concatenate((lstate_set, lmat), axis=1)
