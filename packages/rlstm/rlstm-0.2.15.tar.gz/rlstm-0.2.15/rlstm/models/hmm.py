...
def calc_loss_for_many_sequences(
    ...
    for n in xrange(n_seqs):
        ...
        # Use stack/concat instead of indexing for autograd-friendliness
        if n == 0:
            belief_log_proba_NTK = belief_log_proba_TK
        else:
            belief_log_proba_NTK = np.concatenate(
                (belief_log_proba_NTK, belief_log_proba_TK), axis=0)
...
def calc_loss_for_one_sequence(
    ...
    for t in range(1, n_timesteps):
        ...
        belief_log_proba_TK = np.concatenate((belief_log_proba_TK, cur_belief_log_proba_K), axis=0)
