...
def train_by_sgd(
    ...
    if callback is None:
        def callback(e_i, b_i, ll, acc, va_ll, va_acc, x):
            print('epoch {}  |  batch {}/{}  |  loss {}  |  acc {}  |  va-loss {}  |  va-acc {}'.format(
                e_i,
                b_i,
                num_batches,
                round(ll, 4),
    ...
    weights = adam(grad_fun,
                   ...
                   va_ll_fun=lambda x: regularized_loss(x, va_obs_set, va_fence_set, va_out_set),
                   va_err_fun=lambda x: error_func(va_out_set, np.exp(logpred_fun(x, batch_obs_set, batch_fence_set))),
...
class BlackBoxTreeRegularizer(object):
    ...
    def train_tree(self,
        ...
        def callback(e_i, b_i, ll, acc, va_ll, va_acc, x):
            ...
            print('epoch {}  |  batch {}  |  training loss {}  |  training acc {}  |  validation loss {}  |  validation acc {}'.format(
                e_i,
                b_i,
                round(ll, 4),
                round(acc, 4) if acc else '--',
