...
def calc_loss_for_many_sequences(
    ...
    calc_log_proba_arr_for_x : function
        ...
        Returns T x K array
    eta_DK : NumPy array, D x K (2K), of real values
    ...
    loss_x = 0.0
    loss_y = 0.0
    fenceposts_Np1_base = fenceposts_Np1[0]

    for n in xrange(n_seqs):
        # Get slice of x data corresp. to current sequence
        start_n = fenceposts_Np1[n] - fenceposts_Np1_base
        stop_n = fenceposts_Np1[n+1] - fenceposts_Np1_base
...
# given the input
# def compute_out_lprob(cur_output, lstate_norm, out_weights):
#     my_input = np.vstack((copy(lstate_norm), np.ones((1, lstate_norm.shape[1]))))
#     pred_set = sigmoid(np.dot(out_weights, my_input))
#     label_prob_set = pred_set * cur_output + \
#         (1 - pred_set) * (1 - cur_output)
#     lout = np.sum(safe_log(label_prob_set))
#     return lout

def compute_out_lprob(cur_output, lstate_norm, out_weights):
    my_input = np.hstack((copy(lstate_norm), [1]))
    pred_set = sigmoid(np.dot(out_weights, my_input))
    label_prob_set = pred_set * cur_output + \
        (1 - pred_set) * (1 - cur_output)
    lout = np.sum(safe_log(label_prob_set))
...
def get_state_prob_set(pred_fun, weights, obs_set, fence_set):
    lstate_set = pred_fun(weights, obs_set, fence_set)
    return lstate_set
...
def build(
    ...
    def log_prediction(weights, input_set, fence_set):
        ''' log probabilities '''
        out_weights = parser.get(weights, 'out_weights')
    ...
    def generative_prediction(weights, input_set, fence_set):
        _, _, lstate_set = outputs(weights, input_set, fence_set, return_lstate_set=True)
        return lstate_set
    ...
    if not output_count is None:
        return log_prediction, log_likelihood, parser.num_weights
    else:
        return generative_prediction, log_likelihood, parser.num_weights
