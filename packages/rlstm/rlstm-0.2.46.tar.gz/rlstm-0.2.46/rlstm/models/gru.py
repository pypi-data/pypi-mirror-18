...
# from debug_util import Timer, getCurMemCost_MiB
    ...
    def outputs(
        ...
        for data_iter in range(data_count):
            ...
            for time_iter in range(time_count):
                # print("[seq {}, timestep {}, memcost {} MB]".format(data_iter, time_iter, getCurMemCost_MiB()))
                hiddens = update(
                    max_conn,
                    redun_epsilon,
                ...
                if output_set is not None:
                    ...
                    out_proba = sigmoid(np.sign(output_set[:, n_i_track] - 1e-3) * np.dot(hiddens, output_h_weights))
