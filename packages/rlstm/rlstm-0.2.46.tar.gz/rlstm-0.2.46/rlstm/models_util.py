...
# import sys; sys.path.append('models'); import lr, hmm, mlp, gru, gruhmm
    ...
    if callback is None:
        def callback(e_i, b_i, ll, acc, va_ll, va_acc, x):
            ...
            if b_i == num_batches - 1:
                print('======================================================================================================')
                print('EPOCH SUMMARY - LOSS {:_<10} | ACC {:_<6} | VA_LOSS {:_<10} | VA_ACC {:_<6} | MEM {}'.format(
                    round(ll, 4),
                    round(acc, 4),
                    round(va_ll, 4) if va_ll else 'nan',
                    round(va_acc, 4) if va_acc else 'nan',
                    int(getCurMemCost_MiB())))
                print('======================================================================================================')

    grad_fun = value_and_grad(batch_loss)
...
def train_lr(
    ...
    return logpred_fun, loglike_fun, trained_weights
...
def train_hmm_sgd(
    ...
    trained_weights = train_by_sgd(
        ...
        va_obs_set=va_obs_set,
        va_fence_set=va_fence_set,
        va_out_set=None if obs_only else va_out_set)
    ...
    if obs_only:
        ...
        pred_fun = wrapper_func(outputs, np.exp)
...
class BlackBoxTreeRegularizer(object):
    ...
    def train_tree(self,
        ...
        weights_store = np.zeros((bb_num_weights, _dict_get(params, 'max_epoch', 1000)))
        counts_store = np.zeros((1, _dict_get(params, 'max_epoch', 1000)))
        batch_size = _dict_get(params, 'batch_size', 1)
        ...
        trained_weights = train_by_sgd(
            ...
            bb_num_weights,
            out_set=out_set,
            min_epoch=_dict_get(params, 'min_epoch', 1),
            max_epoch=_dict_get(params, 'max_epoch', 1000),
            batch_size=_dict_get(params, 'batch_size', 1),
