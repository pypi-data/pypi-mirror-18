...
class XyDB(object):
    """XyDB is a database-like containers for intermediate data
     ...
     Schema is accepted for input validation in the form of a NamedTuple subclass.
    
    Arguments:
        schema (default: None | NamedTuple):
        - Accepts a namedTuple that has a "key" field
        - Used for input validation for the "push" schema

        verbose (default: True | boolean)
        - If false, suppresses print commands. Including this message
        
    Attributes:
        - db (dict):   configuration data accessible by keyword
        
        - keys (list): list configuration keywords
        
        - push (func): Adds record to database
        
        - pull (func): Pulls record from database 
        ...
        - show (func): Show record from database.
                       Records are accessible via attributes by keyname 
    ...
    def __init__(self, schema = None, verbose=True):
    ...
    def push(self, key, *args, **kwargs):
        """Adds a dictionary record to the database"""
        if not(type(key)==str):
    ...
    def show(self):
        ...
        Usage: show.<config keyword>.<attribute name>          
        """
        return self._show
