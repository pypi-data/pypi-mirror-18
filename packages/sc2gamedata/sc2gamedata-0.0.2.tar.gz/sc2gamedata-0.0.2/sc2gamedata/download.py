...
def _get_league_data(access_token: str, season: int, league_id: int) -> dict:
    path = "/league/{}/{}/{}/{}".format(season, _queue_id_1v1, _team_type_arranged, league_id)
    return _get_game_data(access_token, path)


def _get_current_season_data(access_token: str) -> dict:
    return _get_game_data(access_token, "/season/current")


def _get_ladder_data(access_token: str, ladder_id: int) -> dict:
...
def _extract_ladders(access_token: str, division_index: int, division_data: dict) -> dict:
    result = _get_ladder_data(access_token, division_data["ladder_id"])
...
def download_ladder_data(access_token: str, workers: int = 10) -> GameData:
    current_season_id = _get_current_season_data(access_token)["id"]

    with multiprocessing.Pool(workers) as p:
        leagues = p.map(functools.partial(_get_league_data, access_token, current_season_id), range(len(_league_ids)))
