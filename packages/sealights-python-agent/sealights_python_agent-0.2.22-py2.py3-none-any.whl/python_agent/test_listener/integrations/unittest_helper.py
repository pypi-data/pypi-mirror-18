...
class SealightsTestResult(unittest.TestResult):
...
def context_match(frame):
    if hasattr(frame, "f_locals") and "self" in frame.f_locals.keys() and \
            isinstance(frame.f_locals["self"], unittest.TestCase):
        try:
            return frame.f_locals["self"].id()
        except:
            # __init__ method instantiates test method name, which is needed for id()
            return
