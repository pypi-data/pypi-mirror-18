...
from amino import List, LazyList, _, L, Empty, Just
...
class TorrentSearch(Thread, Logging):

    def __init__(self, monitor, requester, search_engine, min_seeders) -> None:
        Thread.__init__(self, name='seriesd torrent search')
        self.monitor = monitor
    ...
    def run(self):
        self.result = Just(self.search(self.monitor))
