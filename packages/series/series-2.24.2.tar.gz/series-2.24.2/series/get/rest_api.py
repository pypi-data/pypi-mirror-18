...
from amino import Empty, Just, __, Map, List, Either, F, _, L
...
class RestApi(RestApiBase, Tvdb):
    ...
    def add_season(self):
        ...
        def handle_show(show):
            season_name = int(data['season'])
            season = List.wrap(self.tvdb.season(show.etvdb_id, season_name))
            if season:
                ...
                aired % L(self._releases.add_season)(
                    name, season_name, _, search_name=show.search_name)
                return 'Done'
            else:
    ...
    @route('/release/purge/<int:days>')
    def purge(self, days):
        mon, rel = self._releases.purge(days)
        return dict(monitors=mon, releases=rel)
