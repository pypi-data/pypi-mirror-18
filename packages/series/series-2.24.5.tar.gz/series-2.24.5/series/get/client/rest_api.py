...
class ApiClient(Base):
    ...
    def _find_show_id(self, name):
        return (
            int(name)
            if isinstance(name, int) or (isinstance(name, str) and
                                         name.isdigit()) else
            self.get('show_id', body=dict(name=name))
        )
    ...
    def _with_show(self, name, f):
        _id = self._find_show_id(name)
    ...
    @command('[regex]', 'Display info for all releases matching the regex' +
             ' (default all)')
    def list(self, regex=''):
    ...
    @command('id || name', 'Print show info')
    def show_show(self, name):
        go = lambda _id: self.get('show/{}'.format(_id))
        return self._with_show(name, go)
    ...
    @command('id || name', 'Reset cooldown time for show')
    def activate_show(self, name):
        go = lambda _id: self.put('show/{}/activate'.format(_id))
        return self._with_show(name, go)
    ...
    @command('[days]', 'delete downloaded releases older than arg')
    def purge(self, days=30):
        self.log.info('Deleting releases older than {} days'.format(days))
        result = Map(self.get('release/purge/{}'.format(days)))
        self.log.info(
            'Deleted {} monitors, {} orphan links and {} orphan releases'
            .format(result['monitors'], result['links'], result['releases'])
        )
        return result
