...
from amino import Task, Left, Right, List, __, _, L, Empty, Just
...
class RecordCommand(Command):
    ...
    def _id_spec(self) -> Task[Left]:
        ...

    @abc.abstractproperty
    def _data_spec(self) -> Task[Right]:
        ...

    @abc.abstractproperty
    def _data_path(self) -> Task[str]:
        ...
    ...
    def _lookup(self, data):
        def check(resp):
            return (
                Task.failed('no release found')
                if resp == -1 else
                Task.now((resp, self.rest))
            )
        return Task.delay(self.client.get, '{}_id'.format(self._type),
                          body=data) // check
    ...
    def data(self):
        return (
            self.spec //
            __.lmap(lambda a: Task.now((a, self.args.tail | List())))
            .left_or_map(self._lookup)
        )

    def run(self, f: Callable[[int, List], Any]) -> Task:
    ...
    def _req(self, meth, path=Empty(), data=None):
        ...
        def task(i, body):
            return Task.delay(meth(self.client),
    ...
    def post(self, data: Callable=None):
        cb = (lambda a: Just(dict())) if data is None else data
        def assemble(path):
            return (cb(self.rest).task('invalid arguments') /
                    L(self.client.post)(path, _))
        ...
        if self.doc:
            ...
            def output():
                ...
                for name, (param_desc, desc) in data:
                    ...
                    yield '  {}'.format(desc)
            return Task.delay(output) / '\n'.join
        else:
            return Task.failed('no doc for client')
