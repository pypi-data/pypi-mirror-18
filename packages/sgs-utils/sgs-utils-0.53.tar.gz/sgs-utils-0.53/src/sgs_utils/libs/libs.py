...
#def genomic_density(genome, genes, map_gene_to_position):
...
#		if fin <= deb:
#			size = len(genome)-(deb - fin + 1) # size when the begin is before the end in the list of gene describing the circular genome.
#		else:
...
#	return (interval, len(genes)/float(len(genome)-max_size))
##end def
