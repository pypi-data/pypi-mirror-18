...
class OptionsPage(Page):
    def setupUi(self):
        ...
        self.moveAngleCheckBox = QtWidgets.QCheckBox('Move angle:')
...
class ConvertWizard(QtWidgets.QWizard):
    ...
    def workerFinished(self, errors):
        if errors:
            ...
            # noinspection PyCallByClass,PyTypeChecker,PyArgumentList
            QtWidgets.QMessageBox.critical(self, 'Errors', e)
        self.finished()
...
class CbfConverterWorker(QtCore.QRunnable):
    ...
    def convert(self):
        ...
        if isinstance(img, CbfImage) and 'changeAngleIncrement' in self.opts and not self.opts['changeAngleIncrement']:
