...
def fetchDatabase(*args, **kwargs):
    ...
    kwargs['folder'] = kwargs.get('folder',spinit.DB_FOLDER)
    url = kwargs.get('url',spinit.SPLAT_URL)+kwargs['folder']
...
# if not present at either raise error
    ...
    if folder=='':
        folder = sputil.checkOnlineFile(kwargs['folder'])
        if folder=='':
...
# locally:
    if kwargs['local']:
#        print('Reading local')
        infile = sputil.checkLocal(kwargs['filename'])
        if infile=='':
            infile = sputil.checkLocal(kwargs['folder']+'/'+kwargs['filename'])
        if infile=='':
...
#        print('Reading online')
        infile = sputil.checkOnlineFile(kwargs['filename'])
        if infile=='':
            infile = sputil.checkOnlineFile(kwargs['folder']+'/'+kwargs['filename'])
        if infile=='':
...
# check user access
    if sputil.checkAccess() == False:
        print('\nSpectra may only be imported into library by designated manager or while online; please email {}'.format(spinit.SPLAT_EMAIL))
        return
...
#        print(col,DB_SPECTRA[col].dtype)
    ...
    if verbose:
        print('\nDatabases updated; be sure to move these from {} to {}{}'.format(review_folder,spinit.SPLAT_PATH,spinit.DB_FOLDER))
