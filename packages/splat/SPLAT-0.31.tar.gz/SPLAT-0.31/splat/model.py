...
from .splat import Spectrum, classifyByIndex, compareSpectra, generateMask
...
def processModels(**kwargs):
    ...
    if sputil.checkAccess() is not True:
...
def _checkModelName(model):
    ...
    if not isinstance(model,str):
        return output
    for k in list(spinit.DEFINED_MODEL_SETS.keys()):
        if model.lower()==k.lower() or model.lower() in spinit.DEFINED_MODEL_SETS[k]:
            output = k
    return output
...
# by default assume models come from local SPLAT directory
    ...
    url = kwargs.get('url',spinit.SPLAT_URL)
...
#        kwargs['model'] = 'drift'
    kwargs['folder'] = spinit.SPLAT_PATH+spinit.SPECTRAL_MODEL_FOLDER+kwargs['model']+'/'

# preset defaults
    for ms in spinit.MODEL_PARAMETER_NAMES:
        kwargs[ms] = kwargs.get(ms,spinit.MODEL_PARAMETERS[ms])
...
# if not present at either raise error
    folder = sputil.checkLocal(kwargs['folder'])
    if folder=='':
        folder = sputil.checkOnline(kwargs['folder'])
        if folder=='':
            print('\nCould not find '+kwargs['folder']+' locally or on SPLAT website')
            print('\nAvailable model set options are:')
            for s in spinit.DEFINED_MODEL_SET:
                print('\t{}'.format(s))
            raise NameError()
        else:
...
# online:
    if kwargs['online']:
        file = sputil.checkOnline(kwargs['filename'])
        if file=='':
            if kwargs['force']:
...
# set replacements
    ...
    if mkwargs['model'].lower() == 'drift' or mkwargs['model'].lower() == 'witte' or mkwargs['model'].lower() == 'witte2011' or mkwargs['model'].lower() == 'witte11' or mkwargs['model'].lower() == 'helling':
        mkwargs['model'] = 'drift'
    mkwargs['folder'] = spinit.SPLAT_PATH+spinit.SPECTRAL_MODEL_FOLDER+kwargs['model']+'/'
    ...
    for i in numpy.arange(0,2):
        ...
        for j in numpy.arange(0,2):
            ...
            for k in numpy.arange(0,2):
                ...
                mstr = ''
                for ms in spinit.MODEL_PARAMETER_NAMES: mstr+=str(mparams[-1][ms])
...
#    mpsmall = numpy.unique(numpy.array(mparams))
    ...
    for m in mpsmall:
        ...
        mstr = ''
        for ms in spinit.MODEL_PARAMETER_NAMES: mstr+=str(m[ms])
...
# establish parameters from list of filenames
    if kwargs.get('old',False) == False:
        mfiles = glob.glob(spinit.SPLAT_PATH+spinit.SPECTRAL_MODEL_FOLDER+mset+'/'+mset+'*.txt')
        if len(mfiles) == 0:
            raise ValueError('\nCould not find any model files locally; try running without the "new" keyword')
        for mf in mfiles:
            sp = mf.replace('.txt','').split('_')[1:]
            p = {'model': mset}
            for i,ms in enumerate(spinit.MODEL_PARAMETER_NAMES):   # teff, logg, z
...
# read in parameter file - local and not local
        ...
        try:
            open(os.path.basename(spinit.TMPFILENAME), 'wb').write(requests.get(spinit.SPLAT_URL+spinit.SPECTRAL_MODEL_FOLDER+mset+'/'+pfile).content)
            p = ascii.read(os.path.basename(spinit.TMPFILENAME))
            os.remove(os.path.basename(spinit.TMPFILENAME))
        except:
            print('\n\nCannot access online models for model set {}\n'.format(mset))
#            local = True
        if (os.path.exists(pfile) == False):
            pfile = spinit.SPLAT_PATH+spinit.SPECTRAL_MODEL_FOLDER+mset+'/'+os.path.basename(pfile)
            if (os.path.exists(pfile) == False):
...
#        model[0].info()
        colors = ['k','b']
        mlegend = r'{} '.format(spinit.DEFINED_MODEL_NAMES[getattr(model[0],'modelset')])
        mlegend+='{:s}={:.0f} '.format(spinit.MODEL_PARAMETER_TITLES['teff'],getattr(model[0],'teff'))
        mlegend+='{:s}={:.2f} '.format(spinit.MODEL_PARAMETER_TITLES['logg'],getattr(model[0],'logg'))
        mlegend+='{:s}={:.1f} '.format(spinit.MODEL_PARAMETER_TITLES['z'],getattr(model[0],'z'))
...
# plot several models on top of spectrum in one plot
        ...
        for i,m in enumerate(model):
            sps.append(m)
            colors.append('grey')
            mlegend = r'{:s}={:.0f} '.format(spinit.MODEL_PARAMETER_TITLES['teff'],getattr(m,'teff'))
            mlegend+='{:s}={:.2f} '.format(spinit.MODEL_PARAMETER_TITLES['logg'],getattr(m,'logg'))
            mlegend+='{:s}={:.1f} '.format(spinit.MODEL_PARAMETER_TITLES['z'],getattr(m,'z'))
...
# plot individual panels of spectra - there must be a filename given
        ...
        for i,m in enumerate(model): 
            sps = [spec,m]
            c = ['k','b']
            mlegend = r'{}\n'.format(spinit.DEFINED_MODEL_NAMES[getattr(m,'modelset')])
            mlegend+='{:s}={:.0f} '.format(spinit.MODEL_PARAMETER_TITLES['teff'],getattr(m,'teff'))
            mlegend+='{:s}={:.2f} '.format(spinit.MODEL_PARAMETER_TITLES['logg'],getattr(m,'logg'))
            mlegend+='{:s}={:.1f} '.format(spinit.MODEL_PARAMETER_TITLES['z'],getattr(m,'z'))
            leg = [olegend,mlegend]
...
# model parameters
    ...
    if model_set == False:
        raise ValueError('\n{} is not in the SPLAT model suite; try {}'.format(kwargs['set'],' '.join(list(spinit.DEFINED_MODEL_NAMES.keys()))))
...
#        print(line)
    ...
    if kwargs.get('verbose',False): 
        print('\nBest Parameters to fit to {} models:'.format(spinit.DEFINED_MODEL_NAMES[model_set]))
        for ms in spinit.MODEL_PARAMETER_NAMES[:-1]:
            print('\t{} = {} {}'.format(spinit.MODEL_PARAMETER_TITLES[ms],parameters[0][ms],spinit.MODEL_PARAMETER_UNITS[ms]))
        if compute_radius == True:
...
# weighted means/uncertainties
    ...
    for ms in spinit.MODEL_PARAMETER_NAMES[0:3]:
        ...
        fparam[ms],fparam[ms+'_unc'] = sputil.weightedMeanVar(vals,fitweights)
        fparam[ms]*=spinit.MODEL_PARAMETER_UNITS[ms]
        fparam[ms+'_unc']*=spinit.MODEL_PARAMETER_UNITS[ms]
    if compute_radius == True:
        vals = [(p['radius']*(u.m/u.m)).value for p in parameters]
        fparam['radius'],fparam['radius_unc'] = sputil.weightedMeanVar(vals,fitweights)
    ...
    if kwargs.get('verbose',False): 
        ...
        for k in list(fparam.keys()):
            if k in spinit.MODEL_PARAMETER_NAMES[0:3]:
                print('\t{}: {}+/-{} {}'.format(spinit.MODEL_PARAMETER_TITLES[k],fparam[k].value,fparam[k+'_unc'].value,fparam[k].unit))
#        if k == 'radius':
...
# calculate statistics
    if statsFlag:
        ...
        print('\nMedian parameters:')
        for p in parameters:
            sm, mn, sp = sputil.distributionStats(t[p],sigma=sigma,weights=weights)      # +/- 1 sigma
            name = p
            if p in descrip_assoc.keys():
                name = descrip_assoc[p]
            unit = ''
...
# best fit model
    if bestfitFlag and mset in spinit.DEFINED_MODEL_SET:
...
# set initial parameters
    ...
    parameter_names = spinit.MODEL_PARAMETER_NAMES[:len(parameters0)]
    parameter_titles = [spinit.MODEL_PARAMETER_TITLES[p] for p in parameter_names]
    parameter_units = [spinit.MODEL_PARAMETER_UNITS[p] for p in parameter_names]
...
# run EMCEE with iterative saving and updates
    ...
    #        radii = ((scales*(kwargs.get('distance',10.)*u.pc.to(u.cm)/RADIUS_SUN)**2)**0.5).value.reshape(-1)
            ...
            for kkk in range(nparameters):
                resp+=' {:s}={:.2f}'.format(spinit.MODEL_PARAMETER_NAMES[kkk],bparam[kkk])
...
def _modelFitEMCEE_lnlikelihood(theta,x,y,yerr,model_params):
    ...
    for i in range(len(theta)):
        mparam[spinit.MODEL_PARAMETER_NAMES[i]] = theta[i]
    try:
        mdl = loadModel(**mparam)
    except:
        resp = '\nProblem reading in model '
        for k,v in enumerate(theta):
            resp+='{} = {}, '.format(spinit.MODEL_PARAMETER_NAMES[k],v)
        print(resp)
        return -1.e30,0.
...
#        print(yr)
        ...
        plt.plot(xr,[mn]*2,'r-')
        plt.xlabel('Steps')
        plt.ylabel(r''+spinit.MODEL_PARAMETER_TITLES[spinit.MODEL_PARAMETER_NAMES[i]]+' ('+spinit.MODEL_PARAMETER_UNITS[spinit.MODEL_PARAMETER_NAMES[i]].to_string()+')')
    try:
        plt.savefig(file)
        plt.clf()
    except:
...
# extract best fit values
    ...
    for i in range(samples.shape[-1]):
        tbl[spinit.MODEL_PARAMETER_NAMES[i]] = samples[:,i]
    tbl.sort('parameter_weights')
    tblu = tunique(tbl,keys=spinit.MODEL_PARAMETER_NAMES[:samples.shape[-1]])
    draws = numpy.min([draws,len(tblu)])
    for k in range(draws):
        mkwargs = copy.deepcopy(kwargs)
        mlegend = r''
        for i in range(samples.shape[-1]):
            mkwargs[spinit.MODEL_PARAMETER_NAMES[i]] = tblu[spinit.MODEL_PARAMETER_NAMES[i]][k]
            mlegend+='{:s}={:.2f} '.format(spinit.MODEL_PARAMETER_TITLES[spinit.MODEL_PARAMETER_NAMES[i]],mkwargs[spinit.MODEL_PARAMETER_NAMES[i]])
...
#    print(mparam)
    for i,m in enumerate(mparam):
        mkwargs[spinit.MODEL_PARAMETER_NAMES[i]] = m
        mlegend+='{:s}={:.2f} '.format(spinit.MODEL_PARAMETER_TITLES[spinit.MODEL_PARAMETER_NAMES[i]],float(m))
#    print(mkwargs)
    mdl = loadModel(**mkwargs)
#    print(mdl.teff,mdl.logg)
    stat,scl = compareSpectra(spec,mdl,**kwargs)
...
def _modelFitEMCEE_plotcorner(samples,file,**kwargs):
    ...
    labels = [r''+spinit.MODEL_PARAMETER_TITLES[spinit.MODEL_PARAMETER_NAMES[i]]+' ('+spinit.MODEL_PARAMETER_UNITS[spinit.MODEL_PARAMETER_NAMES[i]].to_string()+')' for i in range(samples.shape[-1]-1)]
...
# extract best fit values
    ...
    for i in range(nparameters):
        ...
        f.write('\n\t{} = {}+{}-{} {}'.format(spinit.MODEL_PARAMETER_TITLES[spinit.MODEL_PARAMETER_NAMES[i]],fit[1],fit[2]-fit[1],fit[1]-fit[0],spinit.MODEL_PARAMETER_UNITS[spinit.MODEL_PARAMETER_NAMES[i]].to_string()))

    mkwargs = copy.deepcopy(kwargs)
    for i in range(samples.shape[-1]):
        mkwargs[spinit.MODEL_PARAMETER_NAMES[i]] = numpy.median(samples[:,i])
    mdl = loadModel(**mkwargs)
    stat,scl = compareSpectra(spec,mdl,**kwargs)
...
# note: need to add in spectral flux uncertainty as well
        ...
        scl,scl_e = sputil.weightedMeanVar(scale,scale_unc,uncertainty=True)
