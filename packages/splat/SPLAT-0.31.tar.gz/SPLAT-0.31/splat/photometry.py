...
def filterMag(sp,filt,*args,**kwargs):
    ...
    '''
# keyword parameters
    filterFolder = kwargs.get('filterFolder',spinit.SPLAT_PATH+spinit.FILTER_FOLDER)
    if not os.path.exists(filterFolder):
        filterFolder = spinit.SPLAT_URL+spinit.FILTER_FOLDER
...
# check that requested filter is in list
    ...
    filt = filt.replace(' ','_')
    filt.upper()
    if (filt not in spinit.FILTERS.keys() and isinstance(custom,bool) and isinstance(notch,bool)):
        print('\nFilter '+filt+' not currently available for SPLAT; contact '+spinit.SPLAT_EMAIL+'\n')
        info = True
...
# Read in filter
    if isinstance(custom,bool) and isinstance(notch,bool):
        fwave,ftrans = numpy.genfromtxt(filterFolder+spinit.FILTERS[filt]['file'], comments='#', unpack=True, \
...
def filterInfo(**kwargs):
    ...
    fname = kwargs.get('filter',sorted(list(spinit.FILTERS.keys())))
    for k in fname:
        if k in list(spinit.FILTERS.keys()):
        	print('  '+k.replace('_',' ')+': '+spinit.FILTERS[k]['description'])
        else:
...
def filterProperties(filt,**kwargs):
    ...
    if (filt not in spinit.FILTERS.keys()):
...
#        filterInfo()
        ...
        report = {}
        report['name'] = filt
        report['description'] = spinit.FILTERS[filt]['description']
        report['zeropoint'] = spinit.FILTERS[filt]['zeropoint']
        fwave,ftrans = numpy.genfromtxt(filterFolder+spinit.FILTERS[filt]['file'], comments='#', unpack=True, \
            missing_values = ('NaN','nan'), filling_values = (numpy.nan))
