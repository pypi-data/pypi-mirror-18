...
# keyword check
    ...
    sputil.checkKeys(kwargs,parameters,forcekey=False)
...
# produce range of standards for plot
    stdnum = numpy.arange(numpy.floor(sputil.typeToNum(spt)-type_range),numpy.ceil(sputil.typeToNum(spt)+type_range)+1)
    if numpy.max(stdnum) > 39:
        stdnum-=(numpy.max(stdnum)-39)
    if numpy.min(stdnum) < 10:
        stdnum+=(10-numpy.min(stdnum))
    stdspt = [sputil.typeToNum(i) for i in stdnum]
