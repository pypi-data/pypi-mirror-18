...
def checkAccess(**kwargs):
    ...
    try:
	    ...
	    if home == None:
	        home = './'
	    bcode = requests.get(spinit.SPLAT_URL+spinit.ACCESS_FILE).content
	    lcode = base64.b64encode(open(home+'/'+spinit.ACCESS_FILE,'r').read().encode())
...
def checkLocal(inputfile):
    ...
    if not os.path.exists(inputfile):
        if not os.path.exists(spinit.SPLAT_PATH+inputfile):
            return ''
        else:
            return spinit.SPLAT_PATH+inputfile
    else:
        return inputfile
...
def checkOnline(*args):
    ...
    if (len(args) != 0):
        if 'http://' in args[0]:
            ...
            if requests.get(spinit.SPLAT_URL+args[0]).status_code == requests.codes.ok:
                return spinit.SPLAT_URL+args[0]
            return False
    else:
        return requests.get(spinit.SPLAT_URL).status_code == requests.codes.ok
...
def dateToCaldate(d):
    ...
    >>> splat.dateToCaldate('19940523')
        1994 May 23
    '''
    return d[:4]+' '+spinit.MONTHS[int(d[5:6])-1]+' '+d[-2:]
...
# some defaults
    if '/' in d and dformat == '':       # default American style
        if len(d) <= 8:
            ...
            dformat = 'MM/DD/YYYY'
    elif True in [c.lower() in d.lower() for c in spinit.MONTHS] and dformat == '':
        if not isNumber(d.replace(' ','')[4]):
            dformat = 'YYYY MMM DD'
        else:
            dformat = 'DD MMM YYYY'
...
# case statement for conversion to YYYY-MM-DD
    if dformat == 'YYYYMMDD':
        ...
        if len(tmp[0]) == 1:
            tmp[0] = '0'+tmp[0]
        for i,c in enumerate(spinit.MONTHS):
...
# case statement for conversion from YYYY-MM-DD to desired output format
    if oformat == 'YYYYMMDD':
        ...
        tmp = dp.split('-')
        df = tmp[2]+' '+spinit.MONTHS[int(tmp[1])-1]+' '+tmp[0]
    elif oformat == 'DD-MMM-YYYY':
        tmp = dp.split('-')
        df = tmp[2]+'-'+spinit.MONTHS[int(tmp[1])-1]+'-'+tmp[0]
    elif oformat == 'YYYY MMM DD':
        tmp = dp.split('-')
        df = tmp[0]+' '+spinit.MONTHS[int(tmp[1])-1]+' '+tmp[2]
    elif oformat == 'YYYY-MMM-DD':
        tmp = dp.split('-')
        df = tmp[0]+'-'+spinit.MONTHS[int(tmp[1])-1]+'-'+tmp[2]
    else:
