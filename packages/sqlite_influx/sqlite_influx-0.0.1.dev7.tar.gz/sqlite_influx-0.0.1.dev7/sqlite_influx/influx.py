...
def get_latest_measurement(tags):
    ...
    try:
        ...
        for r in results:
            ...
            logger.debug("last measurement in series from {}".format(time_parsed))
            last_times.append(time_parsed)

        if last_times:
            logger.debug("use {} as last measurement date".format(time_parsed))
