...
class TestActiveMQSSL(unittest.TestCase):
    def setUp(self):
        listener = TestListener('123')
        self.listener = listener
        self.timestamp = time.strftime('%Y%m%d%H%M%S')
    ...
    def test_ssl_connection(self):
        try:
            import ssl
            queuename = '/queue/ssltest-%s' % self.timestamp
            conn = stomp.Connection(get_activemq_ssl_host())
            conn.set_ssl(get_activemq_ssl_host())
            conn.set_listener('', self.listener)
            conn.start()
            conn.connect('admin', 'password', wait=True)
            conn.subscribe(destination=queuename, id=1, ack='auto')
