...
def get_activemq_ssl_host():
    host = config.get('activemq', 'host')
    port = config.get('activemq', 'ssl_port')
    return [(get_environ('ACTIVEMQ_HOST') or host, int(get_environ('ACTIVEMQ_SSL_PORT') or port))]
