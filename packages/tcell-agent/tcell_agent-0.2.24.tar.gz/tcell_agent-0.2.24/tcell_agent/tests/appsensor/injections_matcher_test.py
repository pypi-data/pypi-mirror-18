...
from tcell_agent.appsensor.params import flatten_clean, GET_PARAM, POST_PARAM, JSON_PARAM, COOKIE_PARAM, URI_PARAM
...
class InjectionsMatcherTest(unittest.TestCase):
    ...
    def one_param_of_each_type_each_injection_test(self):
        ...
        request = FakeRequest('{"xss_body_param":"<script>body"}', {'CONTENT_LENGTH': 1024, 'HTTP_USER_AGENT': None}, get_dict, post_dict, files_dict, cookie_dict)
        appsensor_meta.set_request(request)
        appsensor_meta.path_parameters_data(path_parameters)
        appsensor_meta.post_dict = flatten_clean(post_dict)
