...
from amino import Map, __, List, Right, Left, Maybe, _
...
class Magnet:
    ...
    def urn(self):
        def parse(urn):
            return Maybe(re.match('urn:(.*:.*)', urn))
        return self.params.get('xt') // _.head // parse / __.group(1)


def parse_magnet(magnet) -> Maybe[Magnet]:
    pr = urlparse(magnet)
    if pr.scheme == 'magnet':
        q = Map(parse_qs(pr.query)).valmap(List.wrap)
        name = q.get('dn') // _.head
        return Magnet(name, q)
    else:
...
class SearchResultFactory(object):
    ...
    def from_yify(self, result):
        ...
        def parse(torr):
            ...
            size = sizeof_fmt(str(td.get('size_bytes') | 0))
            seeds = td.get('seeds') | 0
            hsh = td.get('hash') | 'no_hash'
            magnet_link = yify_magnet(title, hsh)
            return SearchResult(name, size, seeds, magnet_link)
