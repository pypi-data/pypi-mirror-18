...
def lambda_configure(args):
    session = boto3.Session()
    lam.configure(session=session,
                  queries_file=args.queries_file,
                  region=args.region,
                  s3_prefix=args.s3_prefix,
                  sns_topic_arn=args.sns_topic_arn)

lambda_configure_parser = lambda_subparsers.add_parser(
    'configure',
    parents=[general_args],
    help='Configure a JSON file configuring the Tiros Lambda'
)

lambda_configure_parser.set_defaults(func=lambda_configure)

lambda_configure_parser.add_argument(
    '--queries-file',
    required=True,
    help='Path to the TQL file'
)

lambda_configure_parser.add_argument(
    '--region',
    help='Region to run Tiros queries'
)
