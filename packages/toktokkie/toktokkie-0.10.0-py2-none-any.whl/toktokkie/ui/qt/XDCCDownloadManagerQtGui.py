...
class XDCCDownloadManagerQtGui(QMainWindow, Ui_XDCCDownloadManagerWindow):
    ...
    def __init__(self, parent = None):
        ...
        """
        super(XDCCDownloadManagerQtGui, self).__init__(parent)
        self.setupUi(self)
        ...
        for pack_searcher in [u"All"] + PackSearcher.get_available_pack_searchers():
        ...
        self.directory_edit.setText(os.getcwdu())
    ...
    def start_search(self):
        ...
        def search():
            ...
            for i, result in enumerate(self.search_results):
                self.search_result_list.addTopLevelItem(QTreeWidgetItem([unicode(i),
                                                                         result.get_bot(),
    ...
    def start_download(self):
        ...
        self.progress = Progress(len(self.download_queue_list),
                                 ...
                                 self.single_progress_bar.windowTitleChanged.emit(u"Title"))

        packs = list(self.download_queue_list)

        for i, pack in enumerate(packs):
            name = u"xdcc_dl_" + unicode(i).zfill(int(len(packs) / 10) + 1)
        ...
        def handle_download():
            ...
            MultipleServerDownloader(u"random").download(packs, self.progress)
            ...
            self.downloading = False
            self.total_progress_bar.windowTitleChanged.emit(u"Title")
    ...
    def move_queue_item(self, up = False, down = False):
        u"""
    ...
    @staticmethod
    def spinner(button, check, normal_text, spinner_text):
        u"""
        ...
        if check:
            dots = button.text().count(u".")
            new_dots = (dots % 3) + 1
            button.setText(spinner_text + new_dots * u".")
        else:
            button.setText(normal_text)

    def ui_updater(self, pause_time = 0.5):
        u"""
        ...
        while self.downloading or self.searching:
            self.download_button.windowTitleChanged.emit(u"Title")
            self.search_button.windowTitleChanged.emit(u"Title")
            self.single_progress_bar.windowTitleChanged.emit(u"Title")
