...
class XDCCUpdateConfiguratorQtGui(QMainWindow, Ui_XDCCUpdateConfiguratorWindow):
    ...
    def save_json(self):
        ...
        if self.file_loaded:
            ...
            destination = QFileDialog.getSaveFileName(self, u"Save", os.getcwdu(), filter=u"*.json",
                                                      options=QFileDialog.DontConfirmOverwrite)[0]

            if destination:
                destination_file = destination if destination.endswith(u".json") else destination + u".json"
    ...
    def create_new_series(self):
        ...
        :return: None
        """
        series = Series(os.getcwdu(), u"New Series", u"1080p", u"Bot", 1, [u"nibl"], u"Plex (TVDB)", u"horriblesubs")
        self.json_handler.add_series(series)
