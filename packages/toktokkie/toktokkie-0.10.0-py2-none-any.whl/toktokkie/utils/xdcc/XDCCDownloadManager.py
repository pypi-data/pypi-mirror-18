...
class XDCCDownloadManager(object):
    ...
    def prepare_directory(directory, series_name, season):
        ...
        MetaDataManager.generate_media_directory(destination_directory, media_type=u"tv_series")

        season_directory = os.path.join(destination_directory, u"Season " + unicode(season))
    ...
    def get_max_season_and_episode_number(directory):
        ...
        if not MetaDataManager.is_media_directory(directory, u"tv_series"):
            ...
            seasons = list(ifilter(lambda x: x.startswith(u"Season "), os.listdir(directory)))

            if len(seasons) == 0:
                return 1, 1
            else:

                max_season = max(seasons, key=lambda x: int(x.split(u"Season ")[1]))
                max_episodes = len(os.listdir(os.path.join(directory, max_season))) + 1

                return int(max_season.rsplit(u"Season ", 1)[1]), max_episodes

    @staticmethod
    def auto_rename(renaming_scheme, starting_episode, packs):
        u"""
        ...
        season_number = int(os.path.basename(season_directory).rsplit(u"Season ", 1)[1])
    ...
    @staticmethod
    def get_preliminary_renaming_results(renaming_scheme, starting_episode, packs,
