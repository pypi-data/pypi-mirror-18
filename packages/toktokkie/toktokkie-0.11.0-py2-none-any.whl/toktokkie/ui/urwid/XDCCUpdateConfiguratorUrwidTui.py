...
class XDCCUpdateConfiguratorUrwidTui(object):
    ...
    def __init__(self):
        ...
        urwid.connect_signal(self.new_series_button, u'click', self.generate_new_series)
        urwid.connect_signal(self.delete_series_button, u'click', self.delete_selected_series)
        urwid.connect_signal(self.confirm_button, u'click', self.confirm_series_information)
    ...
    def lay_out(self):
        ...
        open_button = urwid.AttrMap(self.open_button, None, focus_map=u'reversed')
        save_button = urwid.AttrMap(self.save_button, None, focus_map=u'reversed')
        new_series_button = urwid.AttrMap(self.new_series_button, None, focus_map=u'reversed')
        delete_series_button = urwid.AttrMap(self.delete_series_button, None, focus_map=u'reversed')
    ...
    def load_json_file(self, button):
        ...
        if not os.path.isfile(filepath):
            self.show_message(u"The entered file path is not valid: \n\n" + filepath)
            return
        ...
        try:
            ...
            self.show_message(u"The specified file is either not a valid JSON file or not in the proper format.")
    ...
    def generate_new_series(self, button):
        ...
        self.json_handler.add_series(
            Series(os.getcwdu(), u"New Series", u"480p", u"Bot", 1, [u"nibl"], u"Plex (TVDB)", u"horriblesubs")
        )
        self.refresh_ui()
    ...
    def confirm_series_information(self, button):
        ...
        self.json_handler.add_series(
            Series(self.series_directory_edit.get_edit_text(),
                   ...
                   list(ifilter(lambda x: x.get_state(), self.quality_selectors))[0].get_label(),
                   self.bot_edit.get_edit_text(),
                   int(self.season_edit.get_edit_text()),
                   list(ifilter(lambda x: x.get_state(), self.search_engines))[0].get_label(),
                   list(ifilter(lambda x: x.get_state(), self.naming_schemes))[0].get_label(),
                   list(ifilter(lambda x: x.get_state(), self.naming_patterns))[0].get_label(),
                   )
        )
        self.refresh_ui()
        self.middle_body[len(self.middle_body) - 1].set_state(True)
