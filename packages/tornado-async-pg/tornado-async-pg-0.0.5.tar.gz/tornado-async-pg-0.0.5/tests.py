...
class BaseDataTest(BaseTest):
    ...
    def fill_int_data(self, amount=1000):
        return self.conn.transaction([
            "INSERT INTO unit_test_int_table VALUES %s" % ",".join(
                "(%s)" % i for i in range(amount)),
        ])
...
class MomokoConnectionFactoriesTest(BaseTest):
    ...
    def test_connection_factory(self):
        ...
        conn = yield asyncpg.connect(self.dsn, ioloop=self.io_loop,
...
@unittest.skipIf(psycopg2_impl == "psycopg2cffi",
...
class MomokoConnectionLogTest(BaseTest):
    @gen_test
    def test_connect(self):
        """Test that Connection can connect to the database"""
        conn = yield asyncpg.connect(good_dsn, ioloop=self.io_loop)
        self.assertIsInstance(conn, asyncpg.Connection)
        self.assertFalse(conn.connection_need_log)
        conn.connection_need_log = True
        self.assertTrue(conn.connection_need_log)
        cur = yield conn.execute('select 1')
        self.assertEqual(cur.fetchone(), (1,))
        self.assertTrue(conn.connection_need_log)
        conn.close()
        ...
        # show one time console log


class MomokoPoolLogTest(PoolBaseTest):
    @gen_test
    def test_connect(self):
        db = yield self.build_pool()
        self.assertIsInstance(db, asyncpg.Pool)
        self.assertFalse(db.pool_need_log)
        db.pool_need_log = True
        self.assertTrue(db.pool_need_log)
...
if __name__ == '__main__':
    ...
    logging.basicConfig(format=FORMAT, handler=logging.StreamHandler())
    logging.getLogger("tornado.asyncpg").setLevel(logging.DEBUG)
    unittest.main()
