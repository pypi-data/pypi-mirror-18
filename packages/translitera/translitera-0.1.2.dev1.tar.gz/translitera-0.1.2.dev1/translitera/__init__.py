    ...
    if os.path.exists(path):
        if os.path.isdir(path):
            ...
            for name in contents:
                ...
                flag, file = validate(full_path)
                if flag:
                    file_directory_renamer(full_path)
                    if file:
                        files += 1
                    else:
                        folders += 1
                else:
                    os.sys.stderr.write("Failed to rename '" + full_path + "'\n")
            os.sys.stdout.write("translitera: " + str(files) + " file(s) and " + str(folders) + " folder(s) renamed successfully\n")
        else:
