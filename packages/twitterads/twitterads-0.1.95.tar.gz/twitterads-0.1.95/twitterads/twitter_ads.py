...
class TwitterAds:
    ...
    def get_tailored_audience(self, account_id):
        assert account_id is not None, 'account_id is required'
        tailored_audience_data = []
        next_cursor = None
        first_time = True
        while first_time or next_cursor:
            if not next_cursor:
            ...
            first_time = False
            next_cursor = get_tailored_audience_response['next_cursor']
            tailored_audience_data.extend(get_tailored_audience_response['data'])
        return tailored_audience_data
    ...
    '''
    https://dev.twitter.com/ads/reference/delete/accounts/%3Aaccount_id/tailored_audiences/%3Aid
    '''
    def delete_tailored_audience(self, account_id, tailored_audience_id):
        '''
         Delete Audience id from twitter
        '''
        tailored_audience_delete_response = self.twitter_ads.request(self.base_twitter_ads_url +\
                                                    'accounts/' + account_id + '/tailored_audiences/' + tailored_audience_id,
                                                     method='DELETE')


        return tailored_audience_delete_response
