...
from ..utils import list_type, tuple_type, get_request_arg
...
class Paginator(object):
    def __init__(self, table, request, row_per_page):
        ...
        try:
            self.row_per_page = get_request_arg(request, 'row_per_page',  row_per_page)
        except ValueError:
            self.row_per_page = row_per_page

        try:
            self.page_number = max(int(get_request_arg(request, 'page', 1)), 1)
        except ValueError:
            self.page_number = 1
...
class PaginatorPlugin(BasePlugin):
    ...
    def process_request(self, table, request):
        if get_request_arg(request, 'page') != 'all':
