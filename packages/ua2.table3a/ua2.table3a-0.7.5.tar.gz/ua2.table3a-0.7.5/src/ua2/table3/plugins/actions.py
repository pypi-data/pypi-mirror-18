...
from ua2.table3.utils import get_request_arg
...
class PostAction(BasePlugin):
    ...
    def process_request(self, table, request):
        ...
        if get_request_arg(request, 'postaction', '') in state.build_actions():
            state.process(get_request_arg(request, 'postaction', ''))
