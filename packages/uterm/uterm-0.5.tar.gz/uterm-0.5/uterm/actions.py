...
class Uploader(object):
    ...
    def __init__(self, name, update_func=None, flush_every_line=False):
    ...
    def __call__(self, terminal):
        ...
        fhw = 'def _fhw(text):\r_fh.write(text)\r'
        if self.flush_every_line:
            fhw += '_fh.flush()\r'
        comms.send(fhw + '\r\r')  # Send the file handler writer function.
...
def remote(terminal):
    comms = Comms(terminal, silent=False)
    window = curses.newwin(*terminal.window.getmaxyx())
    window.keypad(1)
    curses.panel.new_panel(window)
    comms.import_module('os')
