...
class TextToSpeech(object):
    ...
    def check_error(self):
        if self.response.status_code == 200:
            if self.response.content.startswith('ERROR:'):
...
class TextToSpeechCache(BinaryFileCache):
    ...
    def filename(self,
                 text,
                 **params):
        text = file_safe(text.strip().lower())
        filename = u'{text}.{ext}'.format(text=text,
    ...
    def fetch_from_source(self,
        ...
        if requests_remaining:
            ...
            raise VoiceError(u'ERROR: ' + REQUEST_LIMIT_EXCEEDED)
