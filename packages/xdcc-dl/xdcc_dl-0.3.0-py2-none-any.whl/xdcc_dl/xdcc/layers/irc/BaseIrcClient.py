...
class BaseIrclient(irc.client.SimpleIRCClient, ConnectionStates, Variables):
    ...
    def __init__(self, server, user, logger = 0):
        u"""
        ...
        """
        super(BaseIrclient, self).__init__()
        ...
        if self.user.get_name() == u"random":
    ...
    def connect(self):
        ...
        :return: None
        """
        self.logger.log(u"Connecting to server:  " + self.server.get_address(), LOG.CONNECTION_ATTEMPT)
        self.logger.log(u"Using Port:            " + unicode(self.server.get_port()), LOG.CONNECTION_ATTEMPT)
        self.logger.log(u"As User:               " + self.user.get_name(), LOG.CONNECTION_ATTEMPT)

        try:
            super(BaseIrclient, self).connect(self.server.get_address(), int(self.server.get_port()), self.user.get_name())
            self.logger.log(u"Established Connection to Server", LOG.CONNECTION_SUCCESS)
            self.connected_to_server = True
        except irc.client.ServerConnectionError:
            self.logger.log(u"Failed to connect to Server", LOG.CONNECTION_FAILURE)
            raise NetworkError()
        except Banned:
            self.logger.log(u"Failed to connect due to a ban", LOG.BANNED)
            raise NetworkError()
