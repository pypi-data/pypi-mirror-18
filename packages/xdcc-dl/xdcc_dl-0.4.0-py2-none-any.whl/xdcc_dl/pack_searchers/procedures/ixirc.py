...
def find_ixirc_packs(search_phrase):
    u"""
    ...
    number_of_pages = 2 if u"Over" in page_analysis[0].text else 1
...
def get_page_results(page_content):
    u"""
    ...
    """
    results = []
    packs = page_content.select(u"td")
    ...
    for line_part in packs:
        ...
        if not aborted:
            if column_count == 0:
                ...
                server = u"irc." + line_part.text.lower() + u".net"  # Server Address
            elif column_count == 2:
