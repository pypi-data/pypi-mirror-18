        ...
        class Tester(IrcEventPrinter):
            ...
            def on_welcome(self, conn, event):
                ...
                for event in irc.events.all:
                    self.assertions_true = self.assertions_true and callable(getattr(self, u"on_" + event))
                raise TestException()
    ...
    def test_version_ctcp(self):
        ...
        class Tester(IrcEventPrinter):
            ...
            def on_welcome(self, conn, event):
                ...
                event.arguments = [u"VERSION"]
                self.on_ctcp(conn, event)

                raise TestException()

            def on_ctcp(self, conn, event):
                super(Tester, self).on_ctcp(conn, event)
                self.assertions_true = event.arguments[0] == u"VERSION"
