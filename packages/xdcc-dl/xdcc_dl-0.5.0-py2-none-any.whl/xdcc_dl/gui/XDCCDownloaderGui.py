...
class XDCCDownloaderGui(QMainWindow, Ui_XDCCDownloaderWindow):
    ...
    spinner_start_signal = pyqtSignal(unicode, name=u"spinner_start")
    spinner_updater_signal = pyqtSignal(QPushButton, unicode, name=u"spinner_updater")
    refresh_search_results_signal = pyqtSignal(unicode, name=u"refresh_search_results")
    refresh_download_queue_signal = pyqtSignal(unicode, name=u"refresh_download_queue")
    show_download_complete_message_signal = pyqtSignal(unicode, name=u"show_download_complete_message")
    progress_update_signal = pyqtSignal(float, float, name=u"progress_update")
    ...
    def search(self):
        ...
        if not self.searching:
            ...
            search_engines = [search_engine] if search_engine != u"All" else PackSearcher.get_available_pack_searchers()
    ...
    def refresh_search_results(self):
        ...
        for i, result in enumerate(self.search_results):
            column = [unicode(i),
                      result.get_bot(), unicode(result.get_packnumber()), unicode(result.get_size()), result.get_filename()]
        ...
        def spin():
            ...
            while self.searching or self.downloading:
                ...
                if self.searching and search:
                    new_text = u"Searching" + (self.search_button.text().count(u".") % 3 + 1) * u"."
                    self.spinner_updater_signal.emit(self.search_button, new_text)

                if self.downloading and download:
                    new_text = u"Downloading" + (self.download_button.text().count(u".") % 3 + 1) * u"."
    ...
    def download(self):
        ...
        if not self.downloading:
            ...
            if not os.path.isdir(directory):
                ...
                msg = self.generate_message(QMessageBox.Warning, u"Invalid Directory",
                                            u"The entered directory is not valid", directory)

                if not sys.argv == [sys.argv[0], u"-platform", u"minimal"]:  # pragma: no cover
                    msg.exec_()
                ...
                def do_download():
                    ...
                    results = MultipleServerDownloader(u"random").download(self.download_queue, progress)
                    self.download_queue = []
                    ...
                    for result in results:
                        list_of_downloaded_packs += result.get_filepath() + u"\n"

                    self.show_download_complete_message_signal.emit(list_of_downloaded_packs)

                Thread(target=do_download).start()

    def progress_update(self, single_percentage, total_percentage):
        u"""
    ...
    # noinspection PyMethodMayBeStatic
    def generate_message(self, icon, title, text, detailed_text):
        u"""
    ...
    def show_download_complete_message(self, details):
        ...
        :return:        None
        """
        msg = self.generate_message(QMessageBox.Information, u"Download Complete",
                                    u"The download has been completed", u"List of downloaded packs:")
