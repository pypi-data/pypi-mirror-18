...
def find_namibsun_packs(search_phrase):
    u"""
    ...
    soup = BeautifulSoup(requests.get(u"http://irc.namibsun.net:8000").text, u"html.parser")
    content = soup.select(u".content")

    all_packs = []
    for i, item in enumerate(content):
        if item.text == search_phrase:
            all_packs.append(XDCCPack(IrcServer(u"irc.namibsun.net"), u"xdcc_servbot", i + 1))
