...
class YouMadeIt:
	...
	def __init__(self, apiKey, deviceName="noName"):
		...
		client.connect("api.youmadeit.fr", 1883, 60)
	...
	def sendToMobile(self, paramName, data, subtitle=None):
		...
		try:
			...
			# Python 3 does not have 'long' type
			dataType = 'num' if isinstance(data, (int, float)) else 'str'
	...
	def sendImageToMobile(self, paramName, data, imageType, subtitle=None):
		topic = "mobile" + "/" + self.apiKey
		dataType = 'img'
		base64image = 'data:image/' + imageType + ';base64,' + base64.b64encode(data).decode('ascii')
		payloadJson = {
			'deviceName': self.deviceName,
			'paramName': paramName,
			'dataType': dataType,
