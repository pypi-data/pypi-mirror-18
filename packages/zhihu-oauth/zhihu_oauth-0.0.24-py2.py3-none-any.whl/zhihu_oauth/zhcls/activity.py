...
``answer`` 表示被收藏的答案，``collection`` 表示被收藏进的收藏夹，因为只有这个动作有两个
操作对象，所以特殊处理了一下。
...
class Activity(object):
    ...
    def _get_target(self):
        ...
        # 对收藏答案类型的特殊处理
        if self._type is ActType.COLLECT_ANSWER:
            ...
            self._target = {'answer': answer, 'collection': collection}
