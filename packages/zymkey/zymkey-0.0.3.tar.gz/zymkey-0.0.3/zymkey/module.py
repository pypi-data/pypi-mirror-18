...
zkalib = cdll.LoadLibrary(ZYMKEY_LIBRARY_PATH)
...
class Zymkey(object):
    ...
    def __init__(self):
        ...
        if ret < 0:
            raise AssertionError('bad return code {!r}'.format(c_int.value))
    ...
    def sign_digest(self, sha256):
        ...
        if ret < 0:
            raise AssertionError('bad return code {!r}'.format(ret.value))
        dc = (c_ubyte * dst_data_sz.value).from_address(dst_data.value)
        data_array = bytearray(dc)
        return data_array
